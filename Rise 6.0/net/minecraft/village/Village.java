...
public class Village {
    ...
    private void updateNumIronGolems() {
        final List<EntityIronGolem> list = this.worldObj.getEntitiesWithinAABB(EntityIronGolem.class, new AxisAlignedBB(this.center.getX() - this.villageRadius, this.center.getY() - 4, this.center.getZ() - this.villageRadius, this.center.getX() + this.villageRadius, this.center.getY() + 4, this.center.getZ() + this.villageRadius));
        this.numIronGolems = list.size();
    }

    private void updateNumVillagers() {
        final List<EntityVillager> list = this.worldObj.getEntitiesWithinAABB(EntityVillager.class, new AxisAlignedBB(this.center.getX() - this.villageRadius, this.center.getY() - 4, this.center.getZ() - this.villageRadius, this.center.getX() + this.villageRadius, this.center.getY() + 4, this.center.getZ() + this.villageRadius));
    ...
    public VillageDoorInfo getNearestDoor(final BlockPos pos) {
        ...
        for (final VillageDoorInfo villagedoorinfo1 : this.villageDoorInfoList) {
            final int j = villagedoorinfo1.getDistanceToDoorBlockSq(pos);
    ...
    public void addOrRenewAgressor(final EntityLivingBase entitylivingbaseIn) {
        for (final Village.VillageAggressor village$villageaggressor : this.villageAgressors) {
    ...
    public EntityLivingBase findNearestVillageAggressor(final EntityLivingBase entitylivingbaseIn) {
        ...
        for (int i = 0; i < this.villageAgressors.size(); ++i) {
            final Village.VillageAggressor village$villageaggressor1 = this.villageAgressors.get(i);
            final double d1 = village$villageaggressor1.agressor.getDistanceSqToEntity(entitylivingbaseIn);
        ...
        for (final String s : this.playerReputation.keySet()) {
            if (this.isPlayerReputationTooLow(s)) {
                ...
                if (entityplayer1 != null) {
                    final double d1 = entityplayer1.getDistanceSqToEntity(villageDefender);
    ...
    private void removeDeadAndOldAgressors() {
        final Iterator<Village.VillageAggressor> iterator = this.villageAgressors.iterator();

        while (iterator.hasNext()) {
            final Village.VillageAggressor village$villageaggressor = iterator.next();
    ...
    public void readVillageDataFromNBT(final NBTTagCompound p_82690_1_) {
        ...
        final NBTTagList nbttaglist1 = p_82690_1_.getTagList("Players", 10);

        for (int j = 0; j < nbttaglist1.tagCount(); ++j) {
            final NBTTagCompound nbttagcompound1 = nbttaglist1.getCompoundTagAt(j);
            ...
            if (nbttagcompound1.hasKey("UUID")) {
                ...
                final GameProfile gameprofile = playerprofilecache.getProfileByUUID(UUID.fromString(nbttagcompound1.getString("UUID")));
