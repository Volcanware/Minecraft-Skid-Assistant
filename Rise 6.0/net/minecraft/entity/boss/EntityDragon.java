...
public class EntityDragon extends EntityLiving implements IBossDisplayData, IEntityMultiPart, IMob {
    ...
    public double[] getMovementOffsets(final int p_70974_1_, float p_70974_2_) {
        ...
        final int i = this.ringBufferIndex - p_70974_1_ * 1 & 63;
        final int j = this.ringBufferIndex - p_70974_1_ * 1 - 1 & 63;
    ...
    public void onLivingUpdate() {
        if (this.worldObj.isRemote) {
            final float f = MathHelper.cos(this.animTime * (float) Math.PI * 2.0F);
            final float f1 = MathHelper.cos(this.prevAnimTime * (float) Math.PI * 2.0F);
        ...
        if (this.getHealth() <= 0.0F) {
            final float f11 = (this.rand.nextFloat() - 0.5F) * 8.0F;
            final float f13 = (this.rand.nextFloat() - 0.5F) * 4.0F;
            final float f14 = (this.rand.nextFloat() - 0.5F) * 8.0F;
            ...
            if (this.isAIDisabled()) {
                ...
                if (this.worldObj.isRemote) {
                    if (this.newPosRotationIncrements > 0) {
                        final double d10 = this.posX + (this.newPosX - this.posX) / (double) this.newPosRotationIncrements;
                        final double d0 = this.posY + (this.newPosY - this.posY) / (double) this.newPosRotationIncrements;
                        final double d1 = this.posZ + (this.newPosZ - this.posZ) / (double) this.newPosRotationIncrements;
                        final double d2 = MathHelper.wrapAngleTo180_double(this.newRotationYaw - (double) this.rotationYaw);
                    ...
                    final double d11 = this.targetX - this.posX;
                    double d12 = this.targetY - this.posY;
                    final double d13 = this.targetZ - this.posZ;
                    final double d14 = d11 * d11 + d12 * d12 + d13 * d13;

                    if (this.target != null) {
                        this.targetX = this.target.posX;
                        this.targetZ = this.target.posZ;
                        final double d3 = this.targetX - this.posX;
                        final double d5 = this.targetZ - this.posZ;
                        final double d7 = Math.sqrt(d3 * d3 + d5 * d5);
                    ...
                    this.rotationYaw = MathHelper.wrapAngleTo180_float(this.rotationYaw);
                    final double d4 = 180.0D - MathHelper.atan2(d11, d13) * 180.0D / Math.PI;
                    ...
                    final double d15 = -MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F);
                    final Vec3 vec31 = (new Vec3(MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F), this.motionY, d15)).normalize();
                    ...
                    final float f6 = MathHelper.sqrt_double(this.motionX * this.motionX + this.motionZ * this.motionZ) * 1.0F + 1.0F;
                    ...
                    final float f7 = (float) (2.0D / (d9 + 1.0D));
                    ...
                    final Vec3 vec32 = (new Vec3(this.motionX, this.motionY, this.motionZ)).normalize();
                ...
                final float f12 = (float) (this.getMovementOffsets(5, 1.0F)[1] - this.getMovementOffsets(10, 1.0F)[1]) * 10.0F / 180.0F * (float) Math.PI;
                final float f2 = MathHelper.cos(f12);
                final float f15 = -MathHelper.sin(f12);
                final float f3 = this.rotationYaw * (float) Math.PI / 180.0F;
                final float f16 = MathHelper.sin(f3);
                final float f4 = MathHelper.cos(f3);
                ...
                final double[] adouble = this.getMovementOffsets(0, 1.0F);
                final float f18 = MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F - this.randomYawVelocity * 0.01F);
                final float f19 = MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F - this.randomYawVelocity * 0.01F);
                ...
                for (int j = 0; j < 3; ++j) {
                    ...
                    final float f20 = this.rotationYaw * (float) Math.PI / 180.0F + this.simplifyAngle(adouble2[0] - adouble1[0]) * (float) Math.PI / 180.0F * 1.0F;
                    final float f21 = MathHelper.sin(f20);
                    final float f22 = MathHelper.cos(f20);
                    final float f23 = 1.5F;
                    final float f24 = (float) (j + 1) * 2.0F;
    ...
    private void updateDragonEnderCrystal() {
        ...
        if (this.rand.nextInt(10) == 0) {
            final float f = 32.0F;
            final List<EntityEnderCrystal> list = this.worldObj.getEntitiesWithinAABB(EntityEnderCrystal.class, this.getEntityBoundingBox().expand(f, f, f));
            EntityEnderCrystal entityendercrystal = null;
            double d0 = Double.MAX_VALUE;

            for (final EntityEnderCrystal entityendercrystal1 : list) {
    ...
    private void collideWithEntities(final List<Entity> p_70970_1_) {
        final double d0 = (this.dragonPartBody.getEntityBoundingBox().minX + this.dragonPartBody.getEntityBoundingBox().maxX) / 2.0D;
        final double d1 = (this.dragonPartBody.getEntityBoundingBox().minZ + this.dragonPartBody.getEntityBoundingBox().maxZ) / 2.0D;
    ...
    private void setNewTarget() {
        ...
        final List<EntityPlayer> list = Lists.newArrayList(this.worldObj.playerEntities);
        ...
        if (this.rand.nextInt(2) == 0 && !list.isEmpty()) {
            ...
            while (true) {
                ...
                final boolean flag = d0 * d0 + d1 * d1 + d2 * d2 > 100.0D;
    ...
    private boolean destroyBlocksInAABB(final AxisAlignedBB p_70972_1_) {
        ...
        final int i1 = MathHelper.floor_double(p_70972_1_.maxY);
        final int j1 = MathHelper.floor_double(p_70972_1_.maxZ);
        boolean flag = false;
        boolean flag1 = false;
        ...
        if (flag1) {
            final double d0 = p_70972_1_.minX + (p_70972_1_.maxX - p_70972_1_.minX) * (double) this.rand.nextFloat();
            final double d1 = p_70972_1_.minY + (p_70972_1_.maxY - p_70972_1_.minY) * (double) this.rand.nextFloat();
            final double d2 = p_70972_1_.minZ + (p_70972_1_.maxZ - p_70972_1_.minZ) * (double) this.rand.nextFloat();
    ...
    public boolean attackEntityFromPart(final EntityDragonPart dragonPart, final DamageSource source, float p_70965_3_) {
        ...
        final float f = this.rotationYaw * (float) Math.PI / 180.0F;
    ...
    protected void onDeathUpdate() {
        ...
        final boolean flag = this.worldObj.getGameRules().getGameRuleBooleanValue("doMobLoot");
    ...
    private void generatePortal(final BlockPos pos) {
        ...
        final BlockPos blockpos1 = pos.up(2);
