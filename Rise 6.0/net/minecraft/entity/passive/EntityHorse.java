...
public class EntityHorse extends EntityAnimal implements IInvBasic {
    ...
    public void fall(final float distance, final float damageMultiplier) {
        ...
        if (i > 0) {
            ...
            final Block block = this.worldObj.getBlockState(new BlockPos(this.posX, this.posY - 0.2D - (double) this.prevRotationYaw, this.posZ)).getBlock();
    ...
    protected EntityHorse getClosestHorse(final Entity entityIn, final double distance) {
        ...
        for (final Entity entity1 : this.worldObj.getEntitiesInAABBexcluding(entityIn, entityIn.getEntityBoundingBox().addCoord(distance, distance, distance), horseBreedingSelector)) {
            final double d1 = entity1.getDistanceSq(entityIn.posX, entityIn.posY, entityIn.posZ);
    ...
    private void setHorseTexturePaths() {
        ...
        final int i1 = this.getHorseArmorIndexSynced();
    ...
    public void moveEntityWithHeading(float strafe, float forward) {
        if (this.riddenByEntity != null && this.riddenByEntity instanceof EntityLivingBase && this.isHorseSaddled()) {
            ...
            if (this.jumpPower > 0.0F && !this.isHorseJumping() && this.onGround) {
                ...
                if (forward > 0.0F) {
                    final float f = MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F);
                    final float f1 = MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F);
    ...
    public void readEntityFromNBT(final NBTTagCompound tagCompund) {
        ...
        if (tagCompund.hasKey("OwnerUUID", 8)) {
            ...
            final String s1 = tagCompund.getString("Owner");
        ...
        if (tagCompund.hasKey("SaddleItem", 10)) {
            final ItemStack itemstack1 = ItemStack.loadItemStackFromNBT(tagCompund.getCompoundTag("SaddleItem"));
    ...
    public EntityAgeable createChild(final EntityAgeable ageable) {
        ...
        if (k == 0) {
            final int i1 = this.rand.nextInt(9);
            int l;
        ...
        final double d1 = this.getEntityAttribute(SharedMonsterAttributes.maxHealth).getBaseValue() + ageable.getEntityAttribute(SharedMonsterAttributes.maxHealth).getBaseValue() + (double) this.getModifiedMaxHealth();
        entityhorse1.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(d1 / 3.0D);
        final double d2 = this.getEntityAttribute(horseJumpStrength).getBaseValue() + ageable.getEntityAttribute(horseJumpStrength).getBaseValue() + this.getModifiedJumpStrength();
        entityhorse1.getEntityAttribute(horseJumpStrength).setBaseValue(d2 / 3.0D);
        final double d0 = this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).getBaseValue() + ageable.getEntityAttribute(SharedMonsterAttributes.movementSpeed).getBaseValue() + this.getModifiedMovementSpeed();
