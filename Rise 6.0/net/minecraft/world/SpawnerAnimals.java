...
public final class SpawnerAnimals {
    ...
    public int findChunksForSpawning(final WorldServer p_77192_1_, final boolean spawnHostileMobs, final boolean spawnPeacefulMobs, final boolean p_77192_4_) {
        if (!spawnHostileMobs && !spawnPeacefulMobs) {
            ...
            if (flag) {
                ...
                for (final EntityPlayer entityplayer1 : p_77192_1_.playerEntities) {
                    if (!entityplayer1.isSpectator()) {
                        final int j = MathHelper.floor_double(entityplayer1.posX / 16.0D);
                        final int k = MathHelper.floor_double(entityplayer1.posZ / 16.0D);
                        final int l = 8;

                        for (int i1 = -l; i1 <= l; ++i1) {
                            for (int j1 = -l; j1 <= l; ++j1) {
                                final boolean flag1 = i1 == -l || i1 == l || j1 == -l || j1 == l;
                                final ChunkCoordIntPair chunkcoordintpair = new ChunkCoordIntPair(i1 + j, j1 + k);
            ...
            for (final EnumCreatureType enumcreaturetype : EnumCreatureType.values()) {
                if ((!enumcreaturetype.getPeacefulCreature() || spawnPeacefulMobs) && (enumcreaturetype.getPeacefulCreature() || spawnHostileMobs) && (!enumcreaturetype.getAnimal() || p_77192_4_)) {
                    final int k4 = Reflector.ForgeWorld_countEntities.exists() ? Reflector.callInt(p_77192_1_, Reflector.ForgeWorld_countEntities, enumcreaturetype, Boolean.valueOf(true)) : p_77192_1_.countEntities(enumcreaturetype.getCreatureClass());
                    final int l4 = enumcreaturetype.getMaxNumberOfCreature() * this.countChunkPos / MOB_COUNT_DIV;

                    if (k4 <= l4) {
                        Collection<ChunkCoordIntPair> collection = this.eligibleChunksForSpawning;

                        if (Reflector.ForgeHooksClient.exists()) {
                            final ArrayList<ChunkCoordIntPair> arraylist = Lists.newArrayList(collection);
                        ...
                        for (final ChunkCoordIntPair chunkcoordintpair1 : collection) {
                            final BlockPos blockpos = getRandomChunkPosition(p_77192_1_, chunkcoordintpair1.chunkXPos, chunkcoordintpair1.chunkZPos, blockposm);
                            final int k1 = blockpos.getX();
                            final int l1 = blockpos.getY();
                            final int i2 = blockpos.getZ();
                            final Block block = p_77192_1_.getBlockState(blockpos).getBlock();
                            ...
                            if (!block.isNormalCube()) {
                                ...
                                for (int k2 = 0; k2 < 3; ++k2) {
                                    ...
                                    for (int l3 = 0; l3 < 4; ++l3) {
                                        ...
                                        final BlockPos blockpos1 = new BlockPos(l2, i3, j3);
                                        final float f = (float) l2 + 0.5F;
                                        final float f1 = (float) j3 + 0.5F;
                                        ...
                                        if (!p_77192_1_.isAnyPlayerWithinRangeAt(f, i3, f1, 24.0D) && blockpos2.distanceSq(f, i3, f1) >= 576.0D) {
                                            ...
                                            if (p_77192_1_.canCreatureTypeSpawnHere(enumcreaturetype, biomegenbase$spawnlistentry, blockpos1) && canCreatureTypeSpawnAtLocation(EntitySpawnPlacementRegistry.getPlacementForEntity(biomegenbase$spawnlistentry.entityClass), p_77192_1_, blockpos1)) {
                                                ...
                                                final boolean flag2 = Reflector.ForgeEventFactory_canEntitySpawn.exists() ? ReflectorForge.canEntitySpawn(entityliving, p_77192_1_, f, (float) i3, f1) : entityliving.getCanSpawnHere() && entityliving.isNotColliding();
                                                ...
                                                if (flag2) {
                                                    ...
                                                    final int i4 = Reflector.ForgeEventFactory_getMaxSpawnPackSize.exists() ? Reflector.callInt(Reflector.ForgeEventFactory_getMaxSpawnPackSize, entityliving) : entityliving.getMaxSpawnedInChunk();
    ...
    private static BlockPosM getRandomChunkPosition(final World p_getRandomChunkPosition_0_, final int p_getRandomChunkPosition_1_, final int p_getRandomChunkPosition_2_, final BlockPosM p_getRandomChunkPosition_3_) {
        ...
        final int k = MathHelper.func_154354_b(chunk.getHeightValue(i & 15, j & 15) + 1, 16);
        ...
        if (!worldIn.getWorldBorder().contains(pos)) {
            ...
            if (p_180267_0_ == EntityLiving.SpawnPlacementType.IN_WATER) {
                ...
                if (!flag) {
                    ...
                    final boolean flag1 = block1 != Blocks.bedrock && block1 != Blocks.barrier;
                    ...
                    for (int k1 = 0; !flag && k1 < 4; ++k1) {
                        ...
                        if (canCreatureTypeSpawnAtLocation(EntityLiving.SpawnPlacementType.ON_GROUND, worldIn, blockpos)) {
                            ...
                            if (Reflector.ForgeEventFactory_canEntitySpawn.exists()) {
                                final Object object = Reflector.call(Reflector.ForgeEventFactory_canEntitySpawn, entityliving, worldIn, Float.valueOf((float) j + 0.5F), Integer.valueOf(blockpos.getY()), Float.valueOf((float) k + 0.5F));
