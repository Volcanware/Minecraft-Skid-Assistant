...
public class ChunkProviderHell implements IChunkProvider {
        ...
        for (int j1 = 0; j1 < i; ++j1) {
            for (int k1 = 0; k1 < i; ++k1) {
                for (int l1 = 0; l1 < 16; ++l1) {
                    ...
                    final double d5 = (this.noiseField[((j1 + 0) * i1 + k1 + 0) * l + l1 + 1] - d1) * d0;
                    final double d6 = (this.noiseField[((j1 + 0) * i1 + k1 + 1) * l + l1 + 1] - d2) * d0;
                    final double d7 = (this.noiseField[((j1 + 1) * i1 + k1 + 0) * l + l1 + 1] - d3) * d0;
                    final double d8 = (this.noiseField[((j1 + 1) * i1 + k1 + 1) * l + l1 + 1] - d4) * d0;
                    ...
                    for (int i2 = 0; i2 < 8; ++i2) {
                        ...
                        for (int j2 = 0; j2 < 4; ++j2) {
                            ...
                            for (int k2 = 0; k2 < 4; ++k2) {
                                ...
                                final int l2 = j2 + j1 * 4;
                                final int i3 = i2 + l1 * 8;
                                final int j3 = k2 + k1 * 4;
    ...
    public void func_180516_b(final int p_180516_1_, final int p_180516_2_, final ChunkPrimer p_180516_3_) {
        ...
        for (int j = 0; j < 16; ++j) {
            for (int k = 0; k < 16; ++k) {
                final boolean flag = this.slowsandNoise[j + k * 16] + this.hellRNG.nextDouble() * 0.2D > 0.0D;
                final boolean flag1 = this.gravelNoise[j + k * 16] + this.hellRNG.nextDouble() * 0.2D > 0.0D;
                final int l = (int) (this.netherrackExclusivityNoise[j + k * 16] / 3.0D + 3.0D + this.hellRNG.nextDouble() * 0.25D);
                int i1 = -1;
                ...
                for (int j1 = 127; j1 >= 0; --j1) {
                    if (j1 < 127 - this.hellRNG.nextInt(5) && j1 > this.hellRNG.nextInt(5)) {
                        final IBlockState iblockstate2 = p_180516_3_.getBlockState(k, j1, j);
        ...
        for (int l = 0; l < p_73164_5_; ++l) {
            for (int i1 = 0; i1 < p_73164_7_; ++i1) {
                ...
                for (int k = 0; k < p_73164_6_; ++k) {
                    ...
                    final double d6 = this.noiseData2[i] / 512.0D;
                    final double d7 = this.noiseData3[i] / 512.0D;
                    final double d8 = (this.noiseData1[i] / 10.0D + 1.0D) / 2.0D;
                    ...
                    if (k > p_73164_6_ - 4) {
                        final double d9 = (float) (k - (p_73164_6_ - 4)) / 3.0F;
