...
public class WorldGenBigTree extends WorldGenAbstractTree {
    ...
    void generateLeafNodeList() {
        ...
        for (; k >= 0; --k) {
            ...
            if (f >= 0.0F) {
                for (int l = 0; l < i; ++l) {
                    final double d0 = this.scaleWidth * (double) f * ((double) this.rand.nextFloat() + 0.328D);
                    final double d1 = (double) (this.rand.nextFloat() * 2.0F) * Math.PI;
                    final double d2 = d0 * Math.sin(d1) + 0.5D;
                    final double d3 = d0 * Math.cos(d1) + 0.5D;
                    final BlockPos blockpos = this.basePos.add(d2, k - 1, d3);
                    final BlockPos blockpos1 = blockpos.up(this.leafDistanceLimit);

                    if (this.checkBlockLine(blockpos, blockpos1) == -1) {
                        final int i1 = this.basePos.getX() - blockpos.getX();
                        final int j1 = this.basePos.getZ() - blockpos.getZ();
                        final double d4 = (double) blockpos.getY() - Math.sqrt(i1 * i1 + j1 * j1) * this.branchSlope;
                        final int k1 = d4 > (double) j ? j : (int) d4;
                        final BlockPos blockpos2 = new BlockPos(this.basePos.getX(), k1, this.basePos.getZ());
    ...
    void func_181631_a(final BlockPos p_181631_1_, final float p_181631_2_, final IBlockState p_181631_3_) {
        final int i = (int) ((double) p_181631_2_ + 0.618D);
    ...
    float layerSize(final int p_76490_1_) {
        if ((float) p_76490_1_ < (float) this.heightLimit * 0.3F) {
            ...
            final float f1 = f - (float) p_76490_1_;
    ...
    void func_175937_a(final BlockPos p_175937_1_, final BlockPos p_175937_2_, final Block p_175937_3_) {
        ...
        final float f = (float) blockpos.getX() / (float) i;
        final float f1 = (float) blockpos.getY() / (float) i;
        final float f2 = (float) blockpos.getZ() / (float) i;

        for (int j = 0; j <= i; ++j) {
            final BlockPos blockpos1 = p_175937_1_.add(0.5F + (float) j * f, 0.5F + (float) j * f1, 0.5F + (float) j * f2);
            final BlockLog.EnumAxis blocklog$enumaxis = this.func_175938_b(p_175937_1_, blockpos1);
     ...
     */
    void generateLeaves() {
        for (final WorldGenBigTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.field_175948_j) {
    ...
    void generateTrunk() {
        ...
        final BlockPos blockpos1 = this.basePos.up(this.height);
    ...
    int checkBlockLine(final BlockPos posOne, final BlockPos posTwo) {
        ...
        if (i == 0) {
            ...
            for (int j = 0; j <= i; ++j) {
                final BlockPos blockpos1 = posOne.add(0.5F + (float) j * f, 0.5F + (float) j * f1, 0.5F + (float) j * f2);
