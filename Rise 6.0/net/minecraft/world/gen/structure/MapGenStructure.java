...
public abstract class MapGenStructure extends MapGenBase {
    ...
    protected StructureStart func_175797_c(final BlockPos pos) {
        ...
        for (final StructureStart structurestart : this.structureMap.values()) {
            if (structurestart.isSizeableStructure() && structurestart.getBoundingBox().isVecInside(pos)) {
                final Iterator<StructureComponent> iterator = structurestart.getComponents().iterator();
    ...
    public BlockPos getClosestStrongholdPos(final World worldIn, final BlockPos pos) {
        ...
        final long k = (long) (pos.getX() >> 4) * i;
        final long l = (long) (pos.getZ() >> 4) * j;
        ...
        for (final StructureStart structurestart : this.structureMap.values()) {
            if (structurestart.isSizeableStructure()) {
                ...
                final BlockPos blockpos1 = structurecomponent.getBoundingBoxCenter();
                final double d1 = blockpos1.distanceSq(pos);
        ...
        if (blockpos != null) {
            ...
            if (list != null) {
                ...
                for (final BlockPos blockpos3 : list) {
                    final double d2 = blockpos3.distanceSq(pos);
