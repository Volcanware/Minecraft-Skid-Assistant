    ...
    public void run() {
        ...
        try {
            ...
            if (Config.getGameSettings().snooperEnabled) {
                ...
                httpurlconnection.setRequestProperty("OF-Java-Version", "" + System.getProperty((String)"java.version"));
            ...
            try {
                ...
                String s = Config.readInputStream((InputStream)inputstream);
                inputstream.close();
                String[] astring = Config.tokenize((String)s, (String)"\n\r");
                if (astring.length >= 1) {
                    String s1 = astring[0].trim();
                    Config.dbg((String)("Version found: " + s1));
                    if (Config.compareRelease((String)s1, (String)"M5") <= 0) {
                        return;
                    }
                    Config.setNewRelease((String)s1);
                    return;
                }
            }
            finally {
