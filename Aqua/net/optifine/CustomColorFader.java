...
public class CustomColorFader {
    ...
    public Vec3 getColor(double x, double y, double z) {
        ...
        this.timeUpdate = i;
        if (Math.abs((double)(x - this.color.xCoord)) < 0.004 && Math.abs((double)(y - this.color.yCoord)) < 0.004 && Math.abs((double)(z - this.color.zCoord)) < 0.004) {
            return this.color;
        }
        double d0 = (double)j * 0.001;
        d0 = Config.limit((double)d0, (double)0.0, (double)1.0);
