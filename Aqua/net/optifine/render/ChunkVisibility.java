...
public class ChunkVisibility {
    ...
    public static int getMaxChunkY(World world, Entity viewEntity, int renderDistanceChunks) {
        int i = MathHelper.floor_double((double)viewEntity.posX) >> 4;
        int j = MathHelper.floor_double((double)viewEntity.posY) >> 4;
        int k = MathHelper.floor_double((double)viewEntity.posZ) >> 4;
        ...
        for (int i2 = l; i2 < i1; ++i2) {
            block9: for (int j2 = j1; j2 < k1; ++j2) {
                ...
                try {
                    Map map = chunk1.getTileEntityMap();
                    if (!map.isEmpty()) {
                ...
                for (int i3 = classinheritancemultimap.length - 1; i3 > l1; --i3) {
                    ClassInheritanceMultiMap classinheritancemultimap1 = classinheritancemultimap[i3];
    ...
    private static EnumFacing[][] makeEnumFacingArrays(boolean opposite) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            aenumfacing[j] = aenumfacing1 = (EnumFacing[])list.toArray((Object[])new EnumFacing[list.size()]);
        }
        return aenumfacing;
    }
