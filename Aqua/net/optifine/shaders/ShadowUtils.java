...
public class ShadowUtils {
    public static Iterator<RenderChunk> makeShadowChunkIterator(WorldClient world, double partialTicks, Entity viewEntity, int renderDistanceChunks, ViewFrustum viewFrustum) {
        ...
        if (f > 0.0f && f < (float)((renderDistanceChunks - 1) * 16)) {
            int i = MathHelper.ceiling_float_int((float)(f / 16.0f)) + 1;
            ...
            float f3 = -MathHelper.sin((float)f2);
            float f4 = MathHelper.cos((float)f2) * MathHelper.cos((float)f1);
            float f5 = -MathHelper.cos((float)f2) * MathHelper.sin((float)f1);
            BlockPos blockpos = new BlockPos(MathHelper.floor_double((double)viewEntity.posX) >> 4, MathHelper.floor_double((double)viewEntity.posY) >> 4, MathHelper.floor_double((double)viewEntity.posZ) >> 4);
        ...
        List list = Arrays.asList((Object[])viewFrustum.renderChunks);
