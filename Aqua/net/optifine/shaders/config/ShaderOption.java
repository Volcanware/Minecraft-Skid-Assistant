...
public abstract class ShaderOption {
    ...
    public String getDescriptionText() {
        String s = Config.normalize((String)this.description);
        s = StrUtils.removePrefix((String)s, (String)"//");
        s = Shaders.translate((String)("option." + this.getName() + ".comment"), (String)s);
        return s;
    }
    ...
    private static int getIndex(String str, String[] strs) {
        for (int i = 0; i < strs.length; ++i) {
            String s = strs[i];
            if (!s.equals((Object)str)) continue;
            return i;
        }
        return -1;
    }
    ...
    public void addPaths(String[] newPaths) {
        List list = Arrays.asList((Object[])this.paths);
        for (int i = 0; i < newPaths.length; ++i) {
            String s = newPaths[i];
            if (list.contains((Object)s)) continue;
            this.paths = (String[])Config.addObjectToArray((Object[])this.paths, (Object)s);
        }
    }
    ...
    public boolean isChanged() {
        return !Config.equals((Object)this.value, (Object)this.valueDefault);
    }
    ...
    public String getNameText() {
        return Shaders.translate((String)("option." + this.name), (String)this.name);
    }

    public String getValueText(String val) {
        return Shaders.translate((String)("value." + this.name + "." + val), (String)val);
    }
    ...
    public void setIndexNormalized(float f) {
        if (this.values.length > 1) {
            f = Config.limit((float)f, (float)0.0f, (float)1.0f);
            int i = Math.round((float)(f * (float)(this.values.length - 1)));
            this.value = this.values[i];
        }
    }
