...
public class LightMap {
    ...
    public boolean updateLightmap(World world, float torchFlickerX, int[] lmColors, boolean nightvision) {
        ...
        f = Config.limitTo1((float)f);
        float f1 = f * (float)(j - 1);
        float f2 = Config.limitTo1((float)(torchFlickerX + 0.5f)) * (float)(j - 1);
        float f3 = Config.limitTo1((float)Config.getGameSettings().gammaSetting);
        ...
        for (int l = 0; l < 16; ++l) {
            for (int i1 = 0; i1 < 16; ++i1) {
                for (int j1 = 0; j1 < 3; ++j1) {
                    float f4 = Config.limitTo1((float)(this.sunRgbs[l][j1] + this.torchRgbs[i1][j1]));
                    if (flag) {
                        float f5 = 1.0f - f4;
                        f5 = 1.0f - f5 * f5 * f5 * f5;
                        f4 = f3 * f5 + (1.0f - f3) * f4;
