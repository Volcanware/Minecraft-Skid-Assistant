...
implements ARBDebugOutputCallback.Handler {
    ...
    public void handleMessage(int source, int type, int id, int severity, String message) {
        if (!(message.contains((CharSequence)"glBindFramebuffer") || message.contains((CharSequence)"Wide lines") || message.contains((CharSequence)"shader recompiled"))) {
            Config.dbg((String)("[LWJGL] source: " + this.getSource(source) + ", type: " + this.getType(type) + ", id: " + id + ", severity: " + this.getSeverity(severity) + ", message: " + message));
    ...
    private String getUnknown(int token) {
        return "Unknown (0x" + Integer.toHexString((int)token).toUpperCase() + ")";
    }
}
