...
public class TextureUtils {
    ...
    public static BufferedImage fixTextureDimensions(String name, BufferedImage bi) {
        ...
        if ((name.startsWith("/mob/zombie") || name.startsWith("/mob/pigzombie")) && (i = bi.getWidth()) == (j = bi.getHeight()) * 2) {
            ...
            graphics2d.drawImage((Image)bi, 0, 0, i, j, (ImageObserver)null);
    ...
    public static ITextureObject getTexture(ResourceLocation loc) {
        ...
        Config.getTextureManager().loadTexture(loc, (ITextureObject)simpletexture);
    ...
    public static void registerResourceListener() {
        ...
        if (iresourcemanager instanceof IReloadableResourceManager) {
            ...
            1 iresourcemanagerreloadlistener = new /* Unavailable Anonymous Inner Class!! */;
            ireloadableresourcemanager.registerReloadListener((IResourceManagerReloadListener)iresourcemanagerreloadlistener);
        }
        2 itickabletextureobject = new /* Unavailable Anonymous Inner Class!! */;
        ResourceLocation resourcelocation = new ResourceLocation("optifine/TickableTextures");
        Config.getTextureManager().loadTickableTexture(resourcelocation, (ITickableTextureObject)itickabletextureobject);
    }

    public static ResourceLocation fixResourceLocation(ResourceLocation loc, String basePath) {
        if (!loc.getResourceDomain().equals((Object)"minecraft")) {
            return loc;
        }
    ...
    public static boolean isPowerOfTwo(int x) {
        int i = MathHelper.roundUpToPowerOfTwo((int)x);
        return i == x;
    }
    ...
    public static BufferedImage scaleImage(BufferedImage bi, int w2) {
        ...
        graphics2d.drawImage((Image)bi, 0, 0, w2, k, (ImageObserver)null);
        return bufferedimage;
    }
    ...
    public static Dimension getImageSize(InputStream in, String suffix) {
        Iterator iterator = ImageIO.getImageReadersBySuffix((String)suffix);
        while (iterator.hasNext()) {
            Dimension dimension;
            ImageReader imagereader = (ImageReader)iterator.next();
            try {
                ImageInputStream imageinputstream = ImageIO.createImageInputStream((Object)in);
    ...
    public static void dbgMipmaps(TextureAtlasSprite textureatlassprite) {
        ...
        for (int i = 0; i < aint.length; ++i) {
            int[] aint1 = aint[i];
            if (aint1 == null) {
                Config.dbg((String)("" + i + ": " + aint1));
                continue;
            }
            Config.dbg((String)("" + i + ": " + aint1.length));
        }
    }
    ...
    public static void saveGlTexture(String name, int textureId, int mipmapLevels, int width, int height) {
        ...
        for (int i1 = 0; i1 <= mipmapLevels; ++i1) {
            ...
            try {
                ImageIO.write((RenderedImage)bufferedimage, (String)"png", (File)file4);
                Config.dbg((String)("Exported: " + file4));
                continue;
            }
            catch (Exception exception) {
                Config.warn((String)("Error writing: " + file4));
    ...
    public static void generateCustomMipmaps(TextureAtlasSprite tas, int mipmaps) {
        ...
        if (tas.getFrameCount() < 1) {
            ...
            aint[0] = aint1;
            list.add((Object)aint);
            tas.setFramesTextureData((List)list);
        }
        ...
        for (int i1 = 0; i1 < l; ++i1) {
            ...
            if (aint2.length != i * j) {
                int k = (int)Math.round((double)Math.sqrt((double)aint2.length));
                if (k * k != aint2.length) {
                    aint2 = new int[1];
                    k = 1;
                }
    ...
    public static int getGLMaximumTextureSize() {
        for (int i = 65536; i > 0; i >>= 1) {
            GlStateManager.glTexImage2D((int)32868, (int)0, (int)6408, (int)i, (int)i, (int)0, (int)6408, (int)5121, (IntBuffer)null);
            int j = GL11.glGetError();
            int k = GlStateManager.glGetTexLevelParameteri((int)32868, (int)0, (int)4096);
            if (k == 0) continue;
            return i;
        }
        return -1;
    ...
    static {
        staticBuffer = GLAllocation.createDirectIntBuffer((int)256);
    }
}
