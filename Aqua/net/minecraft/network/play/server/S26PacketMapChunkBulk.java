...
implements Packet<INetHandlerPlayClient> {
    ...
    public S26PacketMapChunkBulk(List<Chunk> chunks) {
        ...
        this.isOverworld = !((Chunk)chunks.get((int)0)).getWorld().provider.getHasNoSky();
        for (int j = 0; j < i; ++j) {
            Chunk chunk = (Chunk)chunks.get(j);
            S21PacketChunkData.Extracted s21packetchunkdata$extracted = S21PacketChunkData.getExtractedData((Chunk)chunk, (boolean)true, (boolean)this.isOverworld, (int)65535);
    ...
    public void readPacketData(PacketBuffer buf) throws IOException {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            this.chunksData[j].data = new byte[S21PacketChunkData.func_180737_a((int)Integer.bitCount((int)this.chunksData[j].dataSize), (boolean)this.isOverworld, (boolean)true)];
        }
        for (int k = 0; k < i; ++k) {
    ...
    public void writePacketData(PacketBuffer buf) throws IOException {
        ...
        for (int i = 0; i < this.xPositions.length; ++i) {
            ...
            buf.writeShort((int)((short)(this.chunksData[i].dataSize & 0xFFFF)));
        }
