...
public class ChatComponentProcessor {
    public static IChatComponent processComponent(ICommandSender commandSender, IChatComponent component, Entity entityIn) throws CommandException {
        ...
        if (component instanceof ChatComponentScore) {
            ...
            if (PlayerSelector.hasArguments((String)s)) {
                List list = PlayerSelector.matchEntities((ICommandSender)commandSender, (String)s, Entity.class);
                if (list.size() != 1) {
                    throw new EntityNotFoundException();
                }
                s = ((Entity)list.get(0)).getName();
            }
            ichatcomponent = entityIn != null && s.equals((Object)"*") ? new ChatComponentScore(entityIn.getName(), chatcomponentscore.getObjective()) : new ChatComponentScore(s, chatcomponentscore.getObjective());
            ...
            ichatcomponent = PlayerSelector.matchEntitiesToChatComponent((ICommandSender)commandSender, (String)s1);
