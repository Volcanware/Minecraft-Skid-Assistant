...
public class DifficultyInstance {
    ...
    private float calculateAdditionalDifficulty(EnumDifficulty difficulty, long worldTime, long chunkInhabitedTime, float moonPhaseFactor) {
        ...
        float f = 0.75f;
        float f1 = MathHelper.clamp_float((float)(((float)worldTime + -72000.0f) / 1440000.0f), (float)0.0f, (float)1.0f) * 0.25f;
        f += f1;
        float f2 = 0.0f;
        f2 += MathHelper.clamp_float((float)((float)chunkInhabitedTime / 3600000.0f), (float)0.0f, (float)1.0f) * (flag ? 1.0f : 0.75f);
        f2 += MathHelper.clamp_float((float)(moonPhaseFactor * 0.25f), (float)0.0f, (float)f1);
