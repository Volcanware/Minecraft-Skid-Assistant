...
implements IBlockAccess {
    ...
    public BiomeGenBase getBiomeGenForCoords(BlockPos pos) {
        if (this.isBlockLoaded(pos)) {
            ...
            try {
                ...
                CrashReport crashreport = CrashReport.makeCrashReport((Throwable)throwable, (String)"Getting biome");
                CrashReportCategory crashreportcategory = crashreport.makeCategory("Coordinates of biome request");
                crashreportcategory.addCrashSectionCallable("Location", (Callable)new /* Unavailable Anonymous Inner Class!! */);
    ...
    public boolean destroyBlock(BlockPos pos, boolean dropBlock) {
        ...
        this.playAuxSFX(2001, pos, Block.getStateId((IBlockState)iblockstate));
        if (dropBlock) {
    ...
    public void notifyBlockOfStateChange(BlockPos pos, Block blockIn) {
        if (!this.isRemote) {
            ...
            try {
                ...
                crashreportcategory.addCrashSectionCallable("Source block type", (Callable)new /* Unavailable Anonymous Inner Class!! */);
                CrashReportCategory.addBlockInfo((CrashReportCategory)crashreportcategory, (BlockPos)pos, (IBlockState)iblockstate);
                throw new ReportedException(crashreport);
            }
        }
    }
    ...
    public MovingObjectPosition rayTraceBlocks(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        if (!(Double.isNaN((double)vec31.xCoord) || Double.isNaN((double)vec31.yCoord) || Double.isNaN((double)vec31.zCoord))) {
            if (!(Double.isNaN((double)vec32.xCoord) || Double.isNaN((double)vec32.yCoord) || Double.isNaN((double)vec32.zCoord))) {
    ...
    public boolean addWeatherEffect(Entity entityIn) {
        this.weatherEffects.add((Object)entityIn);
        return true;
    }

    public boolean spawnEntityInWorld(Entity entityIn) {
        int i = MathHelper.floor_double((double)(entityIn.posX / 16.0));
        int j = MathHelper.floor_double((double)(entityIn.posZ / 16.0));
        ...
        if (entityIn instanceof EntityPlayer) {
            ...
            this.playerEntities.add((Object)entityplayer);
            this.updateAllPlayersSleepingFlag();
        }
        this.getChunkFromChunkCoords(i, j).addEntity(entityIn);
        this.loadedEntityList.add((Object)entityIn);
    ...
    public void removeEntity(Entity entityIn) {
        ...
        entityIn.setDead();
        if (entityIn instanceof EntityPlayer) {
            this.playerEntities.remove((Object)entityIn);
            this.updateAllPlayersSleepingFlag();
    ...
    public void removePlayerEntityDangerously(Entity entityIn) {
        ...
        this.loadedEntityList.remove((Object)entityIn);
        this.onEntityRemoved(entityIn);
    }

    public void addWorldAccess(IWorldAccess worldAccess) {
        this.worldAccesses.add((Object)worldAccess);
    }

    public void removeWorldAccess(IWorldAccess worldAccess) {
        this.worldAccesses.remove((Object)worldAccess);
    }

    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb) {
        ArrayList list = Lists.newArrayList();
        int i = MathHelper.floor_double((double)bb.minX);
        int j = MathHelper.floor_double((double)(bb.maxX + 1.0));
        int k = MathHelper.floor_double((double)bb.minY);
        int l = MathHelper.floor_double((double)(bb.maxY + 1.0));
        int i1 = MathHelper.floor_double((double)bb.minZ);
        int j1 = MathHelper.floor_double((double)(bb.maxZ + 1.0));
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = i1; l1 < j1; ++l1) {
                if (!this.isBlockLoaded((BlockPos)blockpos$mutableblockpos.set(k1, 64, l1))) continue;
                for (int i2 = k - 1; i2 < l; ++i2) {
                    ...
                    if (worldborder.contains((BlockPos)blockpos$mutableblockpos) || !flag1) {
                        iblockstate1 = this.getBlockState((BlockPos)blockpos$mutableblockpos);
                    }
                    iblockstate1.getBlock().addCollisionBoxesToList(this, (BlockPos)blockpos$mutableblockpos, iblockstate1, bb, (List)list, entityIn);
                }
            }
        }
        double d0 = 0.25;
        ...
        for (int j2 = 0; j2 < list1.size(); ++j2) {
            ...
            if (axisalignedbb != null && axisalignedbb.intersectsWith(bb)) {
                list.add((Object)axisalignedbb);
            }
            if ((axisalignedbb = entityIn.getCollisionBox((Entity)list1.get(j2))) == null || !axisalignedbb.intersectsWith(bb)) continue;
            list.add((Object)axisalignedbb);
        }
        return list;
    }
    ...
    public List<AxisAlignedBB> func_147461_a(AxisAlignedBB bb) {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = i1; l1 < j1; ++l1) {
                ...
                for (int i2 = k - 1; i2 < l; ++i2) {
                    blockpos$mutableblockpos.set(k1, i2, l1);
                    IBlockState iblockstate = k1 >= -30000000 && k1 < 30000000 && l1 >= -30000000 && l1 < 30000000 ? this.getBlockState((BlockPos)blockpos$mutableblockpos) : Blocks.bedrock.getDefaultState();
                    iblockstate.getBlock().addCollisionBoxesToList(this, (BlockPos)blockpos$mutableblockpos, iblockstate, bb, (List)list, (Entity)null);
                }
            }
        }
        return list;
    ...
    public int calculateSkylightSubtracted(float p_72967_1_) {
        ...
        float f1 = 1.0f - (MathHelper.cos((float)(f * (float)Math.PI * 2.0f)) * 2.0f + 0.5f);
    ...
    public float getSunBrightness(float p_72971_1_) {
        ...
        float f1 = 1.0f - (MathHelper.cos((float)(f * (float)Math.PI * 2.0f)) * 2.0f + 0.2f);
        f1 = MathHelper.clamp_float((float)f1, (float)0.0f, (float)1.0f);
        f1 = 1.0f - f1;
    ...
    public Vec3 getSkyColor(Entity entityIn, float partialTicks) {
        ...
        float f1 = MathHelper.cos((float)(f * (float)Math.PI * 2.0f)) * 2.0f + 0.5f;
        f1 = MathHelper.clamp_float((float)f1, (float)0.0f, (float)1.0f);
        int i = MathHelper.floor_double((double)entityIn.posX);
        int j = MathHelper.floor_double((double)entityIn.posY);
    ...
    public float getStarBrightness(float partialTicks) {
        float f = this.getCelestialAngle(partialTicks);
        float f1 = 1.0f - (MathHelper.cos((float)(f * (float)Math.PI * 2.0f)) * 2.0f + 0.25f);
        f1 = MathHelper.clamp_float((float)f1, (float)0.0f, (float)1.0f);
        return f1 * f1 * 0.5f;
    }
    ...
    public void updateEntities() {
        ...
        for (int i = 0; i < this.weatherEffects.size(); ++i) {
            ...
            try {
                ...
                CrashReport crashreport = CrashReport.makeCrashReport((Throwable)throwable2, (String)"Ticking entity");
                CrashReportCategory crashreportcategory = crashreport.makeCategory("Entity being ticked");
                if (entity == null) {
                    crashreportcategory.addCrashSection("Entity", (Object)"~~NULL~~");
                } else {
        ...
        for (int i1 = 0; i1 < this.loadedEntityList.size(); ++i1) {
            ...
            if (!entity2.isDead || Minecraft.getMinecraft().thePlayer.isDead && entity2 instanceof EntityPlayerSP) {
                try {
                    ...
                    CrashReport crashreport1 = CrashReport.makeCrashReport((Throwable)throwable1, (String)"Ticking entity");
        ...
        while (iterator.hasNext()) {
            ...
            if (!tileentity.isInvalid() && tileentity.hasWorldObj() && this.isBlockLoaded(blockpos = tileentity.getPos()) && this.worldBorder.contains(blockpos)) {
                try {
                    ...
                    CrashReport crashreport2 = CrashReport.makeCrashReport((Throwable)throwable, (String)"Ticking block entity");
            ...
            iterator.remove();
            this.loadedTileEntityList.remove((Object)tileentity);
        ...
        if (!this.addedTileEntityList.isEmpty()) {
            for (int j1 = 0; j1 < this.addedTileEntityList.size(); ++j1) {
                ...
                if (!this.loadedTileEntityList.contains((Object)tileentity1)) {
    ...
    public boolean addTileEntity(TileEntity tile) {
        boolean flag = this.loadedTileEntityList.add((Object)tile);
        if (flag && tile instanceof ITickable) {
            this.tickableTileEntities.add((Object)tile);
        }
        return flag;
    }
    ...
    public void addTileEntities(Collection<TileEntity> tileEntityCollection) {
        if (this.processingLoadedTiles) {
            ...
            for (TileEntity tileentity : tileEntityCollection) {
                this.loadedTileEntityList.add((Object)tileentity);
                if (!(tileentity instanceof ITickable)) continue;
                this.tickableTileEntities.add((Object)tileentity);
            }
        }
    }
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
        int i = MathHelper.floor_double((double)entityIn.posX);
        int j = MathHelper.floor_double((double)entityIn.posZ);
        int k = 32;
        if (!forceUpdate || this.isAreaLoaded(i - k, 0, j - k, i + k, 0, j + k, true)) {
            ...
            if (Double.isNaN((double)entityIn.posX) || Double.isInfinite((double)entityIn.posX)) {
                entityIn.posX = entityIn.lastTickPosX;
            }
            if (Double.isNaN((double)entityIn.posY) || Double.isInfinite((double)entityIn.posY)) {
                entityIn.posY = entityIn.lastTickPosY;
            }
            if (Double.isNaN((double)entityIn.posZ) || Double.isInfinite((double)entityIn.posZ)) {
            ...
            int l = MathHelper.floor_double((double)(entityIn.posX / 16.0));
            int i1 = MathHelper.floor_double((double)(entityIn.posY / 16.0));
            int j1 = MathHelper.floor_double((double)(entityIn.posZ / 16.0));
    ...
    public boolean checkBlockCollision(AxisAlignedBB bb) {
        int i = MathHelper.floor_double((double)bb.minX);
        int j = MathHelper.floor_double((double)bb.maxX);
        int k = MathHelper.floor_double((double)bb.minY);
        int l = MathHelper.floor_double((double)bb.maxY);
        int i1 = MathHelper.floor_double((double)bb.minZ);
        int j1 = MathHelper.floor_double((double)bb.maxZ);
        BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
        for (int k1 = i; k1 <= j; ++k1) {
            for (int l1 = k; l1 <= l; ++l1) {
                for (int i2 = i1; i2 <= j1; ++i2) {
                    Block block = this.getBlockState((BlockPos)blockpos$mutableblockpos.set(k1, l1, i2)).getBlock();
    ...
    public boolean isFlammableWithin(AxisAlignedBB bb) {
        ...
        int l = MathHelper.floor_double((double)(bb.maxY + 1.0));
        int i1 = MathHelper.floor_double((double)bb.minZ);
        if (this.isAreaLoaded(i, k, i1, j, l, j1 = MathHelper.floor_double((double)(bb.maxZ + 1.0)), true)) {
            BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
            for (int k1 = i; k1 < j; ++k1) {
                for (int l1 = k; l1 < l; ++l1) {
                    for (int i2 = i1; i2 < j1; ++i2) {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    ...
                    IBlockState iblockstate = this.getBlockState((BlockPos)blockpos$mutableblockpos);
                    Block block = iblockstate.getBlock();
                    if (block.getMaterial() != materialIn || !((double)l >= (d0 = (double)((float)(l1 + 1) - BlockLiquid.getLiquidHeightPercent((int)((Integer)iblockstate.getValue((IProperty)BlockLiquid.LEVEL))))))) continue;
                    flag = true;
    ...
    public boolean isMaterialInBB(AxisAlignedBB bb, Material materialIn) {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    if (this.getBlockState((BlockPos)blockpos$mutableblockpos.set(k1, l1, i2)).getBlock().getMaterial() != materialIn) continue;
                    return true;
                }
            }
        }
    ...
    public boolean isAABBInMaterial(AxisAlignedBB bb, Material materialIn) {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    IBlockState iblockstate = this.getBlockState((BlockPos)blockpos$mutableblockpos.set(k1, l1, i2));
                    Block block = iblockstate.getBlock();
                    if (block.getMaterial() != materialIn) continue;
                    int j2 = (Integer)iblockstate.getValue((IProperty)BlockLiquid.LEVEL);
                    double d0 = l1 + 1;
                    if (j2 < 8) {
    ...
    public float getBlockDensity(Vec3 vec, AxisAlignedBB bb) {
        ...
        double d3 = (1.0 - Math.floor((double)(1.0 / d0)) * d0) / 2.0;
        double d4 = (1.0 - Math.floor((double)(1.0 / d2)) * d2) / 2.0;
    ...
    public TileEntity getTileEntity(BlockPos pos) {
        ...
        if (this.processingLoadedTiles) {
            for (int i = 0; i < this.addedTileEntityList.size(); ++i) {
                ...
                if (tileentity1.isInvalid() || !tileentity1.getPos().equals((Object)pos)) continue;
        ...
        if (tileentity == null) {
            for (int j = 0; j < this.addedTileEntityList.size(); ++j) {
                ...
                if (tileentity2.isInvalid() || !tileentity2.getPos().equals((Object)pos)) continue;
    ...
    public void setTileEntity(BlockPos pos, TileEntity tileEntityIn) {
        if (tileEntityIn != null && !tileEntityIn.isInvalid()) {
            if (this.processingLoadedTiles) {
                ...
                this.addedTileEntityList.add((Object)tileEntityIn);
            } else {
    ...
    public void removeTileEntity(BlockPos pos) {
        ...
        if (tileentity != null && this.processingLoadedTiles) {
            tileentity.invalidate();
            this.addedTileEntityList.remove((Object)tileentity);
        } else {
            if (tileentity != null) {
                this.addedTileEntityList.remove((Object)tileentity);
                this.loadedTileEntityList.remove((Object)tileentity);
                this.tickableTileEntities.remove((Object)tileentity);
            }
    ...
    public void markTileEntityForRemoval(TileEntity tileEntityIn) {
        this.tileEntitiesToBeRemoved.add((Object)tileEntityIn);
    }
    ...
    public static boolean doesBlockHaveSolidTopSurface(IBlockAccess blockAccess, BlockPos pos) {
        ...
        Block block = iblockstate.getBlock();
        return block.getMaterial().isOpaque() && block.isFullCube() ? true : (block instanceof BlockStairs ? iblockstate.getValue((IProperty)BlockStairs.HALF) == BlockStairs.EnumHalf.TOP : (block instanceof BlockSlab ? iblockstate.getValue((IProperty)BlockSlab.HALF) == BlockSlab.EnumBlockHalf.TOP : (block instanceof BlockHopper ? true : (block instanceof BlockSnow ? (Integer)iblockstate.getValue((IProperty)BlockSnow.LAYERS) == 7 : false))));
    }
    ...
    protected void updateWeather() {
        if (!this.provider.getHasNoSky() && !this.isRemote) {
            ...
            this.thunderingStrength = MathHelper.clamp_float((float)this.thunderingStrength, (float)0.0f, (float)1.0f);
            ...
            this.rainingStrength = MathHelper.clamp_float((float)this.rainingStrength, (float)0.0f, (float)1.0f);
        }
    }
    ...
    protected void setActivePlayerChunksAndCheckLight() {
        ...
        for (int i = 0; i < this.playerEntities.size(); ++i) {
            ...
            int j = MathHelper.floor_double((double)(entityplayer.posX / 16.0));
            int k = MathHelper.floor_double((double)(entityplayer.posZ / 16.0));
            int l = this.getRenderDistanceChunks();
            for (int i1 = -l; i1 <= l; ++i1) {
                for (int j1 = -l; j1 <= l; ++j1) {
                    this.activeChunkSet.add((Object)new ChunkCoordIntPair(i1 + j, j1 + k));
                }
            }
        }
        this.theProfiler.endSection();
        ...
        if (!this.playerEntities.isEmpty()) {
            ...
            int l1 = MathHelper.floor_double((double)entityplayer1.posX) + this.rand.nextInt(11) - 5;
            int i2 = MathHelper.floor_double((double)entityplayer1.posY) + this.rand.nextInt(11) - 5;
            int j2 = MathHelper.floor_double((double)entityplayer1.posZ) + this.rand.nextInt(11) - 5;
    ...
    public boolean canBlockFreeze(BlockPos pos, boolean noWaterAdj) {
        ...
        if (pos.getY() >= 0 && pos.getY() < 256 && this.getLightFor(EnumSkyBlock.BLOCK, pos) < 10 && ((block = (iblockstate = this.getBlockState(pos)).getBlock()) == Blocks.water || block == Blocks.flowing_water) && (Integer)iblockstate.getValue((IProperty)BlockLiquid.LEVEL) == 0) {
            boolean flag;
            if (!noWaterAdj) {
                return true;
            }
    ...
    public boolean checkLightFor(EnumSkyBlock lightType, BlockPos pos) {
        ...
        if (l > k) {
            ...
            while (i < j) {
                ...
                if (l2 <= 0 || (j3 = MathHelper.abs_int((int)(i2 - i1))) + (k3 = MathHelper.abs_int((int)(j2 - j1))) + (l3 = MathHelper.abs_int((int)(k2 - k1))) >= 17) continue;
                BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
                for (EnumFacing enumfacing : EnumFacing.values()) {
                    ...
                    int l4 = Math.max((int)1, (int)this.getBlockState((BlockPos)blockpos$mutableblockpos).getBlock().getLightOpacity());
                    i3 = this.getLightFor(lightType, (BlockPos)blockpos$mutableblockpos);
        ...
        while (i < j) {
            ...
            if (j6 <= i6) continue;
            int k6 = Math.abs((int)(j5 - i1));
            int l6 = Math.abs((int)(k5 - j1));
            int i7 = Math.abs((int)(l5 - k1));
    ...
    public List<Entity> getEntitiesWithinAABBExcludingEntity(Entity entityIn, AxisAlignedBB bb) {
        return this.getEntitiesInAABBexcluding(entityIn, bb, (Predicate<? super Entity>)EntitySelectors.NOT_SPECTATING);
    }

    public List<Entity> getEntitiesInAABBexcluding(Entity entityIn, AxisAlignedBB boundingBox, Predicate<? super Entity> predicate) {
        ArrayList list = Lists.newArrayList();
        int i = MathHelper.floor_double((double)((boundingBox.minX - 2.0) / 16.0));
        int j = MathHelper.floor_double((double)((boundingBox.maxX + 2.0) / 16.0));
        int k = MathHelper.floor_double((double)((boundingBox.minZ - 2.0) / 16.0));
        int l = MathHelper.floor_double((double)((boundingBox.maxZ + 2.0) / 16.0));
        for (int i1 = i; i1 <= j; ++i1) {
            for (int j1 = k; j1 <= l; ++j1) {
                if (!this.isChunkLoaded(i1, j1, true)) continue;
                this.getChunkFromChunkCoords(i1, j1).getEntitiesWithinAABBForEntity(entityIn, boundingBox, (List)list, predicate);
            }
        }
        return list;
    }
    ...
    public <T extends Entity> List<T> getEntitiesWithinAABB(Class<? extends T> clazz, AxisAlignedBB aabb, Predicate<? super T> filter) {
        int i = MathHelper.floor_double((double)((aabb.minX - 2.0) / 16.0));
        int j = MathHelper.floor_double((double)((aabb.maxX + 2.0) / 16.0));
        int k = MathHelper.floor_double((double)((aabb.minZ - 2.0) / 16.0));
        int l = MathHelper.floor_double((double)((aabb.maxZ + 2.0) / 16.0));
        ArrayList list = Lists.newArrayList();
        for (int i1 = i; i1 <= j; ++i1) {
            for (int j1 = k; j1 <= l; ++j1) {
                if (!this.isChunkLoaded(i1, j1, true)) continue;
                this.getChunkFromChunkCoords(i1, j1).getEntitiesOfTypeWithinAAAB(clazz, aabb, (List)list, filter);
            }
        }
        return list;
    }
    ...
    public <T extends Entity> T findNearestEntityWithinAABB(Class<? extends T> entityType, AxisAlignedBB aabb, T closestTo) {
        List<? extends T> list = this.getEntitiesWithinAABB(entityType, aabb);
        Entity t = null;
    ...
    public int getStrongPower(BlockPos pos, EnumFacing direction) {
        IBlockState iblockstate = this.getBlockState(pos);
        return iblockstate.getBlock().getStrongPower((IBlockAccess)this, pos, iblockstate, direction);
    }
    ...
    public int getStrongPower(BlockPos pos) {
        int i = 0;
        if ((i = Math.max((int)i, (int)this.getStrongPower(pos.down(), EnumFacing.DOWN))) >= 15) {
            return i;
        }
        if ((i = Math.max((int)i, (int)this.getStrongPower(pos.up(), EnumFacing.UP))) >= 15) {
            return i;
        }
        if ((i = Math.max((int)i, (int)this.getStrongPower(pos.north(), EnumFacing.NORTH))) >= 15) {
            return i;
        }
        if ((i = Math.max((int)i, (int)this.getStrongPower(pos.south(), EnumFacing.SOUTH))) >= 15) {
            return i;
        }
        if ((i = Math.max((int)i, (int)this.getStrongPower(pos.west(), EnumFacing.WEST))) >= 15) {
            return i;
        }
        return (i = Math.max((int)i, (int)this.getStrongPower(pos.east(), EnumFacing.EAST))) >= 15 ? i : i;
    }
    ...
    public int getRedstonePower(BlockPos pos, EnumFacing facing) {
        IBlockState iblockstate = this.getBlockState(pos);
        Block block = iblockstate.getBlock();
        return block.isNormalCube() ? this.getStrongPower(pos) : block.getWeakPower((IBlockAccess)this, pos, iblockstate, facing);
    }
    ...
    public EntityPlayer getPlayerEntityByName(String name) {
        for (int i = 0; i < this.playerEntities.size(); ++i) {
            EntityPlayer entityplayer = (EntityPlayer)this.playerEntities.get(i);
            if (!name.equals((Object)entityplayer.getName())) continue;
            return entityplayer;
        }
        return null;
    }
    ...
    public EntityPlayer getPlayerEntityByUUID(UUID uuid) {
        for (int i = 0; i < this.playerEntities.size(); ++i) {
            EntityPlayer entityplayer = (EntityPlayer)this.playerEntities.get(i);
            if (!uuid.equals((Object)entityplayer.getUniqueID())) continue;
            return entityplayer;
        }
        return null;
    }
    ...
    public void joinEntityInSurroundings(Entity entityIn) {
        ...
        if (!this.loadedEntityList.contains((Object)entityIn)) {
            this.loadedEntityList.add((Object)entityIn);
        }
    }
    ...
    public void playAuxSFXAtEntity(EntityPlayer player, int sfxType, BlockPos pos, int p_180498_4_) {
        try {
            ...
            CrashReport crashreport = CrashReport.makeCrashReport((Throwable)throwable, (String)"Playing level event");
            CrashReportCategory crashreportcategory = crashreport.makeCategory("Level event being played");
            crashreportcategory.addCrashSection("Block coordinates", (Object)CrashReportCategory.getCoordinateInfo((BlockPos)pos));
            crashreportcategory.addCrashSection("Event source", (Object)player);
            crashreportcategory.addCrashSection("Event type", (Object)sfxType);
            crashreportcategory.addCrashSection("Event data", (Object)p_180498_4_);
            throw new ReportedException(crashreport);
        }
    }
    ...
    public CrashReportCategory addWorldInfoToCrashReport(CrashReport report) {
        ...
        crashreportcategory.addCrashSection("Level name", (Object)(this.worldInfo == null ? "????" : this.worldInfo.getWorldName()));
        crashreportcategory.addCrashSectionCallable("All players", (Callable)new /* Unavailable Anonymous Inner Class!! */);
        crashreportcategory.addCrashSectionCallable("Chunk stats", (Callable)new /* Unavailable Anonymous Inner Class!! */);
        try {
