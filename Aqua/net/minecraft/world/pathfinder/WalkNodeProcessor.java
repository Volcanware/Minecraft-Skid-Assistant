...
extends NodeProcessor {
    ...
    public PathPoint getPathPointTo(Entity entityIn) {
        ...
        if (this.canSwim && entityIn.isInWater()) {
            ...
            BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos(MathHelper.floor_double((double)entityIn.posX), i, MathHelper.floor_double((double)entityIn.posZ));
            Block block = this.blockaccess.getBlockState((BlockPos)blockpos$mutableblockpos).getBlock();
            while (block == Blocks.flowing_water || block == Blocks.water) {
                blockpos$mutableblockpos.set(MathHelper.floor_double((double)entityIn.posX), ++i, MathHelper.floor_double((double)entityIn.posZ));
                block = this.blockaccess.getBlockState((BlockPos)blockpos$mutableblockpos).getBlock();
            }
            this.avoidsWater = false;
        } else {
            i = MathHelper.floor_double((double)(entityIn.getEntityBoundingBox().minY + 0.5));
        }
        return this.openPoint(MathHelper.floor_double((double)entityIn.getEntityBoundingBox().minX), i, MathHelper.floor_double((double)entityIn.getEntityBoundingBox().minZ));
    }

    public PathPoint getPathPointToCoords(Entity entityIn, double x, double y, double target) {
        return this.openPoint(MathHelper.floor_double((double)(x - (double)(entityIn.width / 2.0f))), MathHelper.floor_double((double)y), MathHelper.floor_double((double)(target - (double)(entityIn.width / 2.0f))));
    }
        ...
        for (int i = x; i < x + sizeX; ++i) {
            for (int j = y; j < y + sizeY; ++j) {
                for (int k = z; k < z + sizeZ; ++k) {
                    ...
                    if (entityIn.worldObj.getBlockState((BlockPos)blockpos$mutableblockpos).getBlock() instanceof BlockRailBase) {
                    ...
                    if (block.isPassable(blockaccessIn, (BlockPos)blockpos$mutableblockpos) || breakDoors && block instanceof BlockDoor && block.getMaterial() == Material.wood) continue;
