...
extends WorldGenAbstractTree {
    ...
    private static final IBlockState field_181649_b = Blocks.leaves.getDefaultState().withProperty((IProperty)BlockOldLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.OAK).withProperty((IProperty)BlockOldLeaf.CHECK_DECAY, (Comparable)Boolean.valueOf((boolean)false));
            ...
            for (int j = position.getY(); j <= position.getY() + 1 + i; ++j) {
                ...
                for (int l = position.getX() - k; l <= position.getX() + k && flag; ++l) {
                    for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1) {
                        if (j >= 0 && j < 256) {
                            Block block = worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.set(l, j, i1)).getBlock();
                ...
                for (int l1 = position.getY() - 3 + i; l1 <= position.getY() + i; ++l1) {
                    ...
                    for (int k3 = position.getX() - i3; k3 <= position.getX() + i3; ++k3) {
                        ...
                        for (int j1 = position.getZ() - i3; j1 <= position.getZ() + i3; ++j1) {
                            ...
                            if (Math.abs((int)l3) == i3 && Math.abs((int)k1) == i3 && (rand.nextInt(2) == 0 || k2 == 0) || worldIn.getBlockState(blockpos = new BlockPos(k3, l1, j1)).getBlock().isFullBlock()) continue;
    ...
    private void func_181647_a(World p_181647_1_, BlockPos p_181647_2_, PropertyBool p_181647_3_) {
        IBlockState iblockstate = Blocks.vine.getDefaultState().withProperty((IProperty)p_181647_3_, (Comparable)Boolean.valueOf((boolean)true));
