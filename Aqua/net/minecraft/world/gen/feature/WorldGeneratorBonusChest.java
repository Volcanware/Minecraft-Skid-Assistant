...
extends WorldGenerator {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (!worldIn.isAirBlock(blockpos) || !World.doesBlockHaveSolidTopSurface((IBlockAccess)worldIn, (BlockPos)blockpos.down())) continue;
            worldIn.setBlockState(blockpos, Blocks.chest.getDefaultState(), 2);
            TileEntity tileentity = worldIn.getTileEntity(blockpos);
            if (tileentity instanceof TileEntityChest) {
                WeightedRandomChestContent.generateChestContents((Random)rand, this.chestItems, (IInventory)((TileEntityChest)tileentity), (int)this.itemsToGenerateInBonusChest);
            }
            BlockPos blockpos1 = blockpos.east();
            BlockPos blockpos2 = blockpos.west();
            BlockPos blockpos3 = blockpos.north();
            BlockPos blockpos4 = blockpos.south();
            if (worldIn.isAirBlock(blockpos2) && World.doesBlockHaveSolidTopSurface((IBlockAccess)worldIn, (BlockPos)blockpos2.down())) {
                worldIn.setBlockState(blockpos2, Blocks.torch.getDefaultState(), 2);
            }
            if (worldIn.isAirBlock(blockpos1) && World.doesBlockHaveSolidTopSurface((IBlockAccess)worldIn, (BlockPos)blockpos1.down())) {
                worldIn.setBlockState(blockpos1, Blocks.torch.getDefaultState(), 2);
            }
            if (worldIn.isAirBlock(blockpos3) && World.doesBlockHaveSolidTopSurface((IBlockAccess)worldIn, (BlockPos)blockpos3.down())) {
                worldIn.setBlockState(blockpos3, Blocks.torch.getDefaultState(), 2);
            }
            if (worldIn.isAirBlock(blockpos4) && World.doesBlockHaveSolidTopSurface((IBlockAccess)worldIn, (BlockPos)blockpos4.down())) {
