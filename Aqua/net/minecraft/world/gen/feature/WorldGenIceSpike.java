...
extends WorldGenerator {
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        for (int k = 0; k < i; ++k) {
            ...
            int l = MathHelper.ceiling_float_int((float)f);
            for (int i1 = -l; i1 <= l; ++i1) {
                float f1 = (float)MathHelper.abs_int((int)i1) - 0.25f;
                for (int j1 = -l; j1 <= l; ++j1) {
                    float f2 = (float)MathHelper.abs_int((int)j1) - 0.25f;
        ...
        for (int l1 = -k1; l1 <= k1; ++l1) {
            for (int i2 = -k1; i2 <= k1; ++i2) {
                ...
                int j2 = 50;
                if (Math.abs((int)l1) == 1 && Math.abs((int)i2) == 1) {
                    j2 = rand.nextInt(5);
                }
