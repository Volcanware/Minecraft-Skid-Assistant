...
extends WorldGenAbstractTree {
    ...
    private static final IBlockState field_181646_b = Blocks.leaves.getDefaultState().withProperty((IProperty)BlockOldLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.SPRUCE).withProperty((IProperty)BlockLeaves.CHECK_DECAY, (Comparable)Boolean.valueOf((boolean)false));
            ...
            for (int i1 = position.getY(); i1 <= position.getY() + 1 + i && flag; ++i1) {
                ...
                for (int k1 = position.getX() - j1; k1 <= position.getX() + j1 && flag; ++k1) {
                    for (int l1 = position.getZ() - j1; l1 <= position.getZ() + j1 && flag; ++l1) {
                        if (i1 >= 0 && i1 < 256) {
                            Block block = worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.set(k1, i1, l1)).getBlock();
                ...
                for (int l3 = 0; l3 <= k; ++l3) {
                    ...
                    for (int i2 = position.getX() - i3; i2 <= position.getX() + i3; ++i2) {
                        ...
                        for (int k2 = position.getZ() - i3; k2 <= position.getZ() + i3; ++k2) {
                            ...
                            if (Math.abs((int)j2) == i3 && Math.abs((int)l2) == i3 && i3 > 0 || worldIn.getBlockState(blockpos = new BlockPos(i2, j4, k2)).getBlock().isFullBlock()) continue;
