        ...
        while (true) {
            ...
            if (position.getY() <= 3 || !worldIn.isAirBlock(position.down()) && ((block = worldIn.getBlockState(position.down()).getBlock()) == Blocks.grass || block == Blocks.dirt || block == Blocks.stone)) {
                ...
                for (int i = 0; i1 >= 0 && i < 3; ++i) {
                    ...
                    for (BlockPos blockpos : BlockPos.getAllInBox((BlockPos)position.add(-j, -k, -l), (BlockPos)position.add(j, k, l))) {
                        if (!(blockpos.distanceSq((Vec3i)position) <= (double)(f * f))) continue;
