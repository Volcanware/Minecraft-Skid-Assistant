...
extends WorldGenAbstractTree {
    ...
    void generateLeafNodeList() {
        ...
        if ((i = (int)(1.382 + Math.pow((double)(this.leafDensity * (double)this.heightLimit / 13.0), (double)2.0))) < 1) {
            i = 1;
        }
        int j = this.basePos.getY() + this.height;
        this.field_175948_j = Lists.newArrayList();
        this.field_175948_j.add((Object)new FoliageCoordinates(this.basePos.up(k), j));
        for (k = this.heightLimit - this.leafDistanceLimit; k >= 0; --k) {
            ...
            for (int l = 0; l < i; ++l) {
                ...
                double d2 = d0 * Math.sin((double)(d1 = (double)(this.rand.nextFloat() * 2.0f) * Math.PI)) + 0.5;
                BlockPos blockpos = this.basePos.add(d2, (double)(k - 1), d3 = d0 * Math.cos((double)d1) + 0.5);
                ...
                this.field_175948_j.add((Object)new FoliageCoordinates(blockpos, blockpos2.getY()));
            }
        }
    }
    ...
    void func_181631_a(BlockPos p_181631_1_, float p_181631_2_, IBlockState p_181631_3_) {
        ...
        for (int j = -i; j <= i; ++j) {
            for (int k = -i; k <= i; ++k) {
                BlockPos blockpos;
                Material material;
                if (!(Math.pow((double)((double)Math.abs((int)j) + 0.5), (double)2.0) + Math.pow((double)((double)Math.abs((int)k) + 0.5), (double)2.0) <= (double)(p_181631_2_ * p_181631_2_)) || (material = this.world.getBlockState(blockpos = p_181631_1_.add(j, 0, k)).getBlock().getMaterial()) != Material.air && material != Material.leaves) continue;
    ...
    void generateLeafNode(BlockPos pos) {
        for (int i = 0; i < this.leafDistanceLimit; ++i) {
            this.func_181631_a(pos.up(i), this.leafSize(i), Blocks.leaves.getDefaultState().withProperty((IProperty)BlockLeaves.CHECK_DECAY, (Comparable)Boolean.valueOf((boolean)false)));
        }
    }
    ...
    void func_175937_a(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_) {
        ...
        for (int j = 0; j <= i; ++j) {
            ...
            this.setBlockAndNotifyAdequately(this.world, blockpos1, p_175937_3_.getDefaultState().withProperty((IProperty)BlockLog.LOG_AXIS, (Comparable)blocklog$enumaxis));
        }
    }

    private int getGreatestDistance(BlockPos posIn) {
        int i = MathHelper.abs_int((int)posIn.getX());
        int j = MathHelper.abs_int((int)posIn.getY());
        int k = MathHelper.abs_int((int)posIn.getZ());
    ...
    private BlockLog.EnumAxis func_175938_b(BlockPos p_175938_1_, BlockPos p_175938_2_) {
        ...
        int i = Math.abs((int)(p_175938_2_.getX() - p_175938_1_.getX()));
        int k = Math.max((int)i, (int)(j = Math.abs((int)(p_175938_2_.getZ() - p_175938_1_.getZ()))));
        if (k > 0) {
            if (i == k) {
    ...
    void generateLeaves() {
        for (FoliageCoordinates worldgenbigtree$foliagecoordinates : this.field_175948_j) {
            this.generateLeafNode((BlockPos)worldgenbigtree$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases() {
        for (FoliageCoordinates worldgenbigtree$foliagecoordinates : this.field_175948_j) {
            ...
            if (blockpos.equals((Object)worldgenbigtree$foliagecoordinates) || !this.leafNodeNeedsBase(i - this.basePos.getY())) continue;
            this.func_175937_a(blockpos, (BlockPos)worldgenbigtree$foliagecoordinates, Blocks.log);
        }
    }
