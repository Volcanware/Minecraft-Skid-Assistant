...
extends WorldGenHugeTrees {
    ...
    private static final IBlockState field_181634_f = Blocks.leaves.getDefaultState().withProperty((IProperty)BlockOldLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.SPRUCE).withProperty((IProperty)BlockLeaves.CHECK_DECAY, (Comparable)Boolean.valueOf((boolean)false));
    ...
    private void func_150541_c(World worldIn, int p_150541_2_, int p_150541_3_, int p_150541_4_, int p_150541_5_, Random p_150541_6_) {
        ...
        for (int k = p_150541_4_ - i; k <= p_150541_4_; ++k) {
            int l = p_150541_4_ - k;
            int i1 = p_150541_5_ + MathHelper.floor_float((float)((float)l / (float)i * 3.5f));
    ...
    private void func_175933_b(World worldIn, BlockPos p_175933_2_) {
        for (int i = -2; i <= 2; ++i) {
            for (int j = -2; j <= 2; ++j) {
                if (Math.abs((int)i) == 2 && Math.abs((int)j) == 2) continue;
