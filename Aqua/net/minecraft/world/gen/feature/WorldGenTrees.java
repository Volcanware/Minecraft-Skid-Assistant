...
extends WorldGenAbstractTree {
    ...
    private static final IBlockState field_181654_b = Blocks.leaves.getDefaultState().withProperty((IProperty)BlockOldLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.OAK).withProperty((IProperty)BlockLeaves.CHECK_DECAY, (Comparable)Boolean.valueOf((boolean)false));
            ...
            for (int j = position.getY(); j <= position.getY() + 1 + i; ++j) {
                ...
                for (int l = position.getX() - k; l <= position.getX() + k && flag; ++l) {
                    for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1) {
                        if (j >= 0 && j < 256) {
                            if (this.func_150523_a(worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.set(l, j, i1)).getBlock())) continue;
                            flag = false;
                            continue;
                        }
                        flag = false;
                ...
                for (int i3 = position.getY() - k2 + i; i3 <= position.getY() + i; ++i3) {
                    ...
                    for (int k1 = position.getX() - j1; k1 <= position.getX() + j1; ++k1) {
                        ...
                        for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
                            ...
                            if (Math.abs((int)l1) == j1 && Math.abs((int)j2) == j1 && (rand.nextInt(2) == 0 || i4 == 0) || (block = worldIn.getBlockState(blockpos = new BlockPos(k1, i3, i2)).getBlock()).getMaterial() != Material.air && block.getMaterial() != Material.leaves && block.getMaterial() != Material.vine) continue;
                    ...
                    for (int k3 = position.getY() - 3 + i; k3 <= position.getY() + i; ++k3) {
                        ...
                        for (int l4 = position.getX() - k4; l4 <= position.getX() + k4; ++l4) {
                            for (int i5 = position.getZ() - k4; i5 <= position.getZ() + k4; ++i5) {
                                ...
                                if (worldIn.getBlockState((BlockPos)blockpos$mutableblockpos1).getBlock().getMaterial() != Material.leaves) continue;
    ...
    private void func_181652_a(World p_181652_1_, int p_181652_2_, BlockPos p_181652_3_, EnumFacing p_181652_4_) {
        this.setBlockAndNotifyAdequately(p_181652_1_, p_181652_3_, Blocks.cocoa.getDefaultState().withProperty((IProperty)BlockCocoa.AGE, (Comparable)Integer.valueOf((int)p_181652_2_)).withProperty((IProperty)BlockCocoa.FACING, (Comparable)p_181652_4_));
    }

    private void func_181651_a(World p_181651_1_, BlockPos p_181651_2_, PropertyBool p_181651_3_) {
        this.setBlockAndNotifyAdequately(p_181651_1_, p_181651_2_, Blocks.vine.getDefaultState().withProperty((IProperty)p_181651_3_, (Comparable)Boolean.valueOf((boolean)true)));
    }
