...
public abstract class StructureComponent {
    ...
    public NBTTagCompound createStructureBaseNBT() {
        ...
        nbttagcompound.setString("id", MapGenStructureIO.getStructureComponentName((StructureComponent)this));
    ...
    public void readStructureBaseNBT(World worldIn, NBTTagCompound tagCompound) {
        ...
        this.coordBaseMode = (i = tagCompound.getInteger("O")) == -1 ? null : EnumFacing.getHorizontal((int)i);
    ...
    protected boolean isLiquidInStructureBoundingBox(World worldIn, StructureBoundingBox boundingboxIn) {
        int i = Math.max((int)(this.boundingBox.minX - 1), (int)boundingboxIn.minX);
        int j = Math.max((int)(this.boundingBox.minY - 1), (int)boundingboxIn.minY);
        int k = Math.max((int)(this.boundingBox.minZ - 1), (int)boundingboxIn.minZ);
        int l = Math.min((int)(this.boundingBox.maxX + 1), (int)boundingboxIn.maxX);
        int i1 = Math.min((int)(this.boundingBox.maxY + 1), (int)boundingboxIn.maxY);
        int j1 = Math.min((int)(this.boundingBox.maxZ + 1), (int)boundingboxIn.maxZ);
        ...
        for (int k1 = i; k1 <= l; ++k1) {
            for (int l1 = k; l1 <= j1; ++l1) {
                ...
                if (!worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.set(k1, i1, l1)).getBlock().getMaterial().isLiquid()) continue;
                return true;
            }
        }
        for (int i2 = i; i2 <= l; ++i2) {
            for (int k2 = j; k2 <= i1; ++k2) {
                ...
                if (!worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.set(i2, k2, j1)).getBlock().getMaterial().isLiquid()) continue;
                return true;
            }
        }
        for (int j2 = k; j2 <= j1; ++j2) {
            for (int l2 = j; l2 <= i1; ++l2) {
                ...
                if (!worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.set(l, l2, j2)).getBlock().getMaterial().isLiquid()) continue;
                return true;
            }
        }
        return false;
                ...
                if (blockIn == Blocks.piston || blockIn == Blocks.sticky_piston || blockIn == Blocks.lever || blockIn == Blocks.dispenser) {
                    if (this.coordBaseMode == EnumFacing.SOUTH) {
                        if (meta == EnumFacing.NORTH.getIndex() || meta == EnumFacing.SOUTH.getIndex()) {
                            return EnumFacing.getFront((int)meta).getOpposite().getIndex();
                        }
                    } else if (this.coordBaseMode == EnumFacing.WEST) {
                        if (meta == EnumFacing.NORTH.getIndex()) {
                            return EnumFacing.WEST.getIndex();
    ...
    protected void replaceAirAndLiquidDownwards(World worldIn, IBlockState blockstateIn, int x, int y, int z, StructureBoundingBox boundingboxIn) {
        ...
        int j;
        int i = this.getXWithOffset(x, z);
        if (boundingboxIn.isVecInside((Vec3i)new BlockPos(i, j = this.getYWithOffset(y), k = this.getZWithOffset(x, z)))) {
    ...
    protected boolean generateChestContents(World worldIn, StructureBoundingBox boundingBoxIn, Random rand, int x, int y, int z, List<WeightedRandomChestContent> listIn, int max) {
        ...
        if (boundingBoxIn.isVecInside((Vec3i)blockpos) && worldIn.getBlockState(blockpos).getBlock() != Blocks.chest) {
            ...
            if (tileentity instanceof TileEntityChest) {
                WeightedRandomChestContent.generateChestContents((Random)rand, listIn, (IInventory)((TileEntityChest)tileentity), (int)max);
            }
            return true;
        }
        return false;
    ...
    protected boolean generateDispenserContents(World worldIn, StructureBoundingBox boundingBoxIn, Random rand, int x, int y, int z, int meta, List<WeightedRandomChestContent> listIn, int max) {
        ...
        if (boundingBoxIn.isVecInside((Vec3i)blockpos) && worldIn.getBlockState(blockpos).getBlock() != Blocks.dispenser) {
            ...
            if (tileentity instanceof TileEntityDispenser) {
                WeightedRandomChestContent.generateDispenserContents((Random)rand, listIn, (TileEntityDispenser)((TileEntityDispenser)tileentity), (int)max);
            }
            return true;
        }
        return false;
    ...
    protected void placeDoorCurrentPosition(World worldIn, StructureBoundingBox boundingBoxIn, Random rand, int x, int y, int z, EnumFacing facing) {
        BlockPos blockpos = new BlockPos(this.getXWithOffset(x, z), this.getYWithOffset(y), this.getZWithOffset(x, z));
        if (boundingBoxIn.isVecInside((Vec3i)blockpos)) {
            ItemDoor.placeDoor((World)worldIn, (BlockPos)blockpos, (EnumFacing)facing.rotateYCCW(), (Block)Blocks.oak_door);
        }
    }
