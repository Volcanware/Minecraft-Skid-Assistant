...
extends SaveFormatOld {
    ...
    public List<SaveFormatComparator> getSaveList() throws AnvilConverterException {
        if (this.savesDirectory != null && this.savesDirectory.exists() && this.savesDirectory.isDirectory()) {
            ...
            for (File file1 : afile = this.savesDirectory.listFiles()) {
                ...
                long i = 0L;
                list.add((Object)new SaveFormatComparator(s, s1, worldinfo.getLastTimePlayed(), i, worldinfo.getGameType(), flag, worldinfo.isHardcoreModeEnabled(), worldinfo.areCommandsAllowed()));
            }
            return list;
        }
    ...
    public boolean convertMapFormat(String filename, IProgressUpdate progressCallback) {
        ...
        this.addRegionFilesToCollection(file1, (Collection<File>)list);
        if (file2.exists()) {
            this.addRegionFilesToCollection(file2, (Collection<File>)list1);
        }
        if (file3.exists()) {
            this.addRegionFilesToCollection(file3, (Collection<File>)list2);
        }
        ...
        this.convertFile(new File(file1, "region"), (Iterable<File>)list, (WorldChunkManager)worldchunkmanager, 0, i, progressCallback);
        this.convertFile(new File(file2, "region"), (Iterable<File>)list1, (WorldChunkManager)new WorldChunkManagerHell(BiomeGenBase.hell, 0.0f), list.size(), i, progressCallback);
        this.convertFile(new File(file3, "region"), (Iterable<File>)list2, (WorldChunkManager)new WorldChunkManagerHell(BiomeGenBase.sky, 0.0f), list.size() + list1.size(), i, progressCallback);
    ...
    private void convertFile(File p_75813_1_, Iterable<File> p_75813_2_, WorldChunkManager p_75813_3_, int p_75813_4_, int p_75813_5_, IProgressUpdate p_75813_6_) {
        for (File file1 : p_75813_2_) {
            ...
            int i = (int)Math.round((double)(100.0 * (double)(++p_75813_4_) / (double)p_75813_5_));
        ...
        try {
            ...
            for (int i = 0; i < 32; ++i) {
                for (int j = 0; j < 32; ++j) {
                    ...
                    ChunkLoader.AnvilConverterData chunkloader$anvilconverterdata = ChunkLoader.load((NBTTagCompound)nbttagcompound1);
                    NBTTagCompound nbttagcompound2 = new NBTTagCompound();
                    NBTTagCompound nbttagcompound3 = new NBTTagCompound();
                    nbttagcompound2.setTag("Level", (NBTBase)nbttagcompound3);
                    ChunkLoader.convertToAnvilFormat((ChunkLoader.AnvilConverterData)chunkloader$anvilconverterdata, (NBTTagCompound)nbttagcompound3, (WorldChunkManager)p_75811_3_);
                    DataOutputStream dataoutputstream = regionfile1.getChunkDataOutputStream(i, j);
                    CompressedStreamTools.write((NBTTagCompound)nbttagcompound2, (DataOutput)dataoutputstream);
                    dataoutputstream.close();
                }
                int k = (int)Math.round((double)(100.0 * (double)(p_75811_4_ * 1024) / (double)(p_75811_5_ * 1024)));
                int l = (int)Math.round((double)(100.0 * (double)((i + 1) * 32 + p_75811_4_ * 1024) / (double)(p_75811_5_ * 1024)));
                if (l <= k) continue;
    ...
    private void addRegionFilesToCollection(File worldDir, Collection<File> collection) {
        ...
        Object[] afile = file1.listFiles((FilenameFilter)new /* Unavailable Anonymous Inner Class!! */);
        if (afile != null) {
            Collections.addAll(collection, (Object[])afile);
        }
    }
}
