...
public abstract class WorldProvider {
    ...
    protected void registerWorldChunkManager() {
        ...
        if (worldtype == WorldType.FLAT) {
            FlatGeneratorInfo flatgeneratorinfo = FlatGeneratorInfo.createFlatGeneratorFromString((String)this.worldObj.getWorldInfo().getGeneratorOptions());
            this.worldChunkMgr = new WorldChunkManagerHell(BiomeGenBase.getBiomeFromBiomeList((int)flatgeneratorinfo.getBiome(), (BiomeGenBase)BiomeGenBase.field_180279_ad), 0.5f);
        } else {
    ...
    public float calculateCelestialAngle(long worldTime, float partialTicks) {
        ...
        f = 1.0f - (float)((Math.cos((double)((double)f * Math.PI)) + 1.0) / 2.0);
        f += (f - f) / 3.0f;
        return f;
    }
    ...
    public float[] calcSunriseSunsetColors(float celestialAngle, float partialTicks) {
        float f2;
        float f = 0.4f;
        float f1 = MathHelper.cos((float)(celestialAngle * (float)Math.PI * 2.0f)) - 0.0f;
        if (f1 >= (f2 = -0.0f) - f && f1 <= f2 + f) {
            float f3 = (f1 - f2) / f * 0.5f + 0.5f;
            float f4 = 1.0f - (1.0f - MathHelper.sin((float)(f3 * (float)Math.PI))) * 0.99f;
            f4 *= f4;
    ...
    public Vec3 getFogColor(float p_76562_1_, float p_76562_2_) {
        ...
        float f3 = 1.0f;
        return new Vec3((double)(f1 *= f * 0.94f + 0.06f), (double)(f2 *= f * 0.94f + 0.06f), (double)(f3 *= f * 0.91f + 0.09f));
    }
