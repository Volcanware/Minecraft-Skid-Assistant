...
extends CommandBase {
    public static final Pattern field_147211_a = Pattern.compile((String)"^([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])$");
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (args.length >= 1 && args[0].length() > 1) {
            IChatComponent ichatcomponent = args.length >= 2 ? CommandBanIp.getChatComponentFromNthArg((ICommandSender)sender, (String[])args, (int)1) : null;
            Matcher matcher = field_147211_a.matcher((CharSequence)args[0]);
            if (matcher.matches()) {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        return args.length == 1 ? CommandBanIp.getListOfStringsMatchingLastWord((String[])args, (String[])MinecraftServer.getServer().getAllUsernames()) : null;
    }
    ...
    protected void func_147210_a(ICommandSender sender, String address, String reason) {
        ...
        List list = MinecraftServer.getServer().getConfigurationManager().getPlayersMatchingAddress(address);
        ...
        if (list.isEmpty()) {
            CommandBanIp.notifyOperators((ICommandSender)sender, (ICommand)this, (String)"commands.banip.success", (Object[])new Object[]{address});
        } else {
            CommandBanIp.notifyOperators((ICommandSender)sender, (ICommand)this, (String)"commands.banip.success.players", (Object[])new Object[]{address, CommandBanIp.joinNiceString((Object[])astring)});
        }
    }
}
