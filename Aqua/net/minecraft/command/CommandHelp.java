...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        ...
        int k = 0;
        try {
            k = args.length == 0 ? 0 : CommandHelp.parseInt((String)args[0], (int)1, (int)(j + 1)) - 1;
        }
        catch (NumberInvalidException numberinvalidexception) {
            Map<String, ICommand> map = this.getCommands();
            ICommand icommand = (ICommand)map.get((Object)args[0]);
            if (icommand != null) {
                throw new WrongUsageException(icommand.getCommandUsage(sender), new Object[0]);
            }
            if (MathHelper.parseIntWithDefault((String)args[0], (int)-1) != -1) {
                throw numberinvalidexception;
            }
            throw new CommandNotFoundException();
        }
        int l = Math.min((int)((k + 1) * 7), (int)list.size());
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length == 1) {
            ...
            return CommandHelp.getListOfStringsMatchingLastWord((String[])args, (String[])((String[])set.toArray((Object[])new String[set.size()])));
        }
        return null;
    }
}
