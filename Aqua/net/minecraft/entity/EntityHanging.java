...
extends Entity {
    ...
    public boolean onValidSurface() {
        if (!this.worldObj.getCollidingBoundingBoxes((Entity)this, this.getEntityBoundingBox()).isEmpty()) {
            return false;
        }
        int i = Math.max((int)1, (int)(this.getWidthPixels() / 16));
        int j = Math.max((int)1, (int)(this.getHeightPixels() / 16));
        ...
        for (int k = 0; k < i; ++k) {
            for (int l = 0; l < j; ++l) {
                ...
                if (block.getMaterial().isSolid() || BlockRedstoneDiode.isRedstoneRepeaterBlockID((Block)block)) continue;
                return false;
            }
        }
        for (Entity entity : this.worldObj.getEntitiesWithinAABBExcludingEntity((Entity)this, this.getEntityBoundingBox())) {
    ...
    public boolean hitByEntity(Entity entityIn) {
        return entityIn instanceof EntityPlayer ? this.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer)((EntityPlayer)entityIn)), 0.0f) : false;
    }
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund) {
        ...
        if (tagCompund.hasKey("Direction", 99)) {
            enumfacing = EnumFacing.getHorizontal((int)tagCompund.getByte("Direction"));
            this.hangingPosition = this.hangingPosition.offset(enumfacing);
        } else {
            enumfacing = tagCompund.hasKey("Facing", 99) ? EnumFacing.getHorizontal((int)tagCompund.getByte("Facing")) : EnumFacing.getHorizontal((int)tagCompund.getByte("Dir"));
        }
    ...
    public void setPosition(double x, double y, double z) {
        ...
        if (!this.hangingPosition.equals((Object)blockpos)) {
            this.updateBoundingBox();
