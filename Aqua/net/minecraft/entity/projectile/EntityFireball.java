...
extends Entity {
    ...
    public EntityFireball(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
        ...
        double d0 = MathHelper.sqrt_double((double)(accelX * accelX + accelY * accelY + accelZ * accelZ));
    ...
    public EntityFireball(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ) {
        ...
        this.motionY = 0.0;
        this.motionX = 0.0;
        double d0 = MathHelper.sqrt_double((double)((accelX += this.rand.nextGaussian() * 0.4) * accelX + (accelY += this.rand.nextGaussian() * 0.4) * accelY + (accelZ += this.rand.nextGaussian() * 0.4) * accelZ));
        this.accelerationX = accelX / d0 * 0.1;
        this.accelerationY = accelY / d0 * 0.1;
        this.accelerationZ = accelZ / d0 * 0.1;
    }
    ...
    public void onUpdate() {
        if (this.worldObj.isRemote || (this.shootingEntity == null || !this.shootingEntity.isDead) && this.worldObj.isBlockLoaded(new BlockPos((Entity)this))) {
            super.onUpdate();
            this.setFire(1);
            if (this.inGround) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (!entity1.canBeCollidedWith() || entity1.isEntityEqual((Entity)this.shootingEntity) && this.ticksInAir < 25) continue;
                float f = 0.3f;
            ...
            this.rotationYaw = (float)(MathHelper.atan2((double)this.motionZ, (double)this.motionX) * 180.0 / Math.PI) + 90.0f;
            this.rotationPitch = (float)(MathHelper.atan2((double)f1, (double)this.motionY) * 180.0 / Math.PI) - 90.0f;
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound) {
        ...
        tagCompound.setTag("direction", (NBTBase)this.newDoubleNBTList(new double[]{this.motionX, this.motionY, this.motionZ}));
    }
