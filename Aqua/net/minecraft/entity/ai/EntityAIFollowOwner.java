...
extends EntityAIBase {
    ...
    public void updateTask() {
        ...
        if (!this.thePet.isSitting() && --this.field_75343_h <= 0) {
            ...
            if (!this.petPathfinder.tryMoveToEntityLiving((Entity)this.theOwner, this.followSpeed) && !this.thePet.getLeashed() && this.thePet.getDistanceSqToEntity((Entity)this.theOwner) >= 144.0) {
                int i = MathHelper.floor_double((double)this.theOwner.posX) - 2;
                int j = MathHelper.floor_double((double)this.theOwner.posZ) - 2;
                int k = MathHelper.floor_double((double)this.theOwner.getEntityBoundingBox().minY);
                for (int l = 0; l <= 4; ++l) {
                    for (int i1 = 0; i1 <= 4; ++i1) {
                        if (l >= 1 && i1 >= 1 && l <= 3 && i1 <= 3 || !World.doesBlockHaveSolidTopSurface((IBlockAccess)this.theWorld, (BlockPos)new BlockPos(i + l, k - 1, j + i1)) || !this.func_181065_a(new BlockPos(i + l, k, j + i1)) || !this.func_181065_a(new BlockPos(i + l, k + 1, j + i1))) continue;
