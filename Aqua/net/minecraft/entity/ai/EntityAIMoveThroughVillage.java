...
extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        Vec3 vec3 = RandomPositionGenerator.findRandomTargetBlockTowards((EntityCreature)this.theEntity, (int)10, (int)7, (Vec3)new Vec3((double)this.doorInfo.getDoorBlockPos().getX(), (double)this.doorInfo.getDoorBlockPos().getY(), (double)this.doorInfo.getDoorBlockPos().getZ()));
        if (vec3 == null) {
            return false;
        }
        pathnavigateground.setBreakDoors(false);
    ...
    public void resetTask() {
        if (this.theEntity.getNavigator().noPath() || this.theEntity.getDistanceSq(this.doorInfo.getDoorBlockPos()) < 16.0) {
            this.doorList.add((Object)this.doorInfo);
        }
    }
    ...
    private VillageDoorInfo findNearestDoor(Village villageIn) {
        ...
        for (VillageDoorInfo villagedoorinfo1 : villageIn.getVillageDoorInfoList()) {
            int j = villagedoorinfo1.getDistanceSquared(MathHelper.floor_double((double)this.theEntity.posX), MathHelper.floor_double((double)this.theEntity.posY), MathHelper.floor_double((double)this.theEntity.posZ));
    ...
    private boolean doesDoorListContain(VillageDoorInfo doorInfoIn) {
        for (VillageDoorInfo villagedoorinfo : this.doorList) {
            if (!doorInfoIn.getDoorBlockPos().equals((Object)villagedoorinfo.getDoorBlockPos())) continue;
            return true;
        }
        return false;
    }
