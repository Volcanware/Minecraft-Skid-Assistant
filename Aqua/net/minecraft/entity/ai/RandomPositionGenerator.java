...
public class RandomPositionGenerator {
    ...
    private static Vec3 findRandomTargetBlock(EntityCreature entitycreatureIn, int xz, int y, Vec3 targetVec3) {
        ...
        int k = 0;
        float f = -99999.0f;
        boolean flag1 = entitycreatureIn.hasHome() ? (d0 = entitycreatureIn.getHomePosition().distanceSq((double)MathHelper.floor_double((double)entitycreatureIn.posX), (double)MathHelper.floor_double((double)entitycreatureIn.posY), (double)MathHelper.floor_double((double)entitycreatureIn.posZ)) + 4.0) < (d1 = (double)(entitycreatureIn.getMaximumHomeDistance() + (float)xz)) * d1 : false;
        for (int j1 = 0; j1 < 10; ++j1) {
            ...
            BlockPos blockpos1 = new BlockPos(l += MathHelper.floor_double((double)entitycreatureIn.posX), k1 += MathHelper.floor_double((double)entitycreatureIn.posY), i1 += MathHelper.floor_double((double)entitycreatureIn.posZ));
