...
extends EntityAIBase {
    ...
    public static boolean isSuitableTarget(EntityLiving attacker, EntityLivingBase target, boolean includeInvincibles, boolean checkSight) {
        ...
        if (attacker instanceof IEntityOwnable && StringUtils.isNotEmpty((CharSequence)((IEntityOwnable)attacker).getOwnerId())) {
            if (target instanceof IEntityOwnable && ((IEntityOwnable)attacker).getOwnerId().equals((Object)((IEntityOwnable)target).getOwnerId())) {
                return false;
            }
    ...
    protected boolean isSuitableTarget(EntityLivingBase target, boolean includeInvincibles) {
        if (!EntityAITarget.isSuitableTarget((EntityLiving)this.taskOwner, target, includeInvincibles, this.shouldCheckSight)) {
            return false;
        }
    ...
    private boolean canEasilyReach(EntityLivingBase target) {
        ...
        int i = pathpoint.xCoord - MathHelper.floor_double((double)target.posX);
        return (double)(i * i + (j = pathpoint.zCoord - MathHelper.floor_double((double)target.posZ)) * j) <= 2.25;
    }
}
