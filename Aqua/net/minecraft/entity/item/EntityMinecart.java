...
implements IWorldNameable {
    ...
    public static EntityMinecart getMinecart(World worldIn, double x, double y, double z, EnumMinecartType type) {
        switch (1.$SwitchMap$net$minecraft$entity$item$EntityMinecart$EnumMinecartType[type.ordinal()]) {
            case 1: {
    ...
    protected void entityInit() {
        ...
        this.dataWatcher.addObject(21, (Object)new Integer(6));
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.isRemote) {
            if (this.turnProgress > 0) {
                ...
                double d1 = MathHelper.wrapAngleTo180_double((double)(this.minecartYaw - (double)this.rotationYaw));
            ...
            int k = MathHelper.floor_double((double)this.posX);
            if (BlockRailBase.isRailBlock((World)this.worldObj, (BlockPos)new BlockPos(k, (l = MathHelper.floor_double((double)this.posY)) - 1, i1 = MathHelper.floor_double((double)this.posZ)))) {
                --l;
            }
            if (BlockRailBase.isRailBlock((IBlockState)(iblockstate = this.worldObj.getBlockState(blockpos = new BlockPos(k, l, i1))))) {
                this.func_180460_a(blockpos, iblockstate);
                if (iblockstate.getBlock() == Blocks.activator_rail) {
                    this.onActivatorRailPass(k, l, i1, (Boolean)iblockstate.getValue((IProperty)BlockRailPowered.POWERED));
                }
            } else {
            ...
            if (d0 * d0 + d2 * d2 > 0.001) {
                this.rotationYaw = (float)(MathHelper.atan2((double)d2, (double)d0) * 180.0 / Math.PI);
                if (this.isInReverse) {
                    this.rotationYaw += 180.0f;
                }
            }
            if ((d3 = (double)MathHelper.wrapAngleTo180_float((float)(this.rotationYaw - this.prevRotationYaw))) < -170.0 || d3 >= 170.0) {
                this.rotationYaw += 180.0f;
                this.isInReverse = !this.isInReverse;
            }
            this.setRotation(this.rotationYaw, this.rotationPitch);
            for (Entity entity : this.worldObj.getEntitiesWithinAABBExcludingEntity((Entity)this, this.getEntityBoundingBox().expand((double)0.2f, 0.0, (double)0.2f))) {
    ...
    protected void moveDerailedMinecart() {
        ...
        this.motionX = MathHelper.clamp_double((double)this.motionX, (double)(-d0), (double)d0);
        this.motionZ = MathHelper.clamp_double((double)this.motionZ, (double)(-d0), (double)d0);
        if (this.onGround) {
            this.motionX *= 0.5;
            this.motionY *= 0.5;
            this.motionZ *= 0.5;
    ...
    protected void func_180460_a(BlockPos p_180460_1_, IBlockState p_180460_2_) {
        ...
        if (blockrailbase == Blocks.golden_rail) {
            flag = (Boolean)p_180460_2_.getValue((IProperty)BlockRailPowered.POWERED);
            flag1 = !flag;
        }
        double d0 = 0.0078125;
        ...
        double d3 = Math.sqrt((double)(d1 * d1 + d2 * d2));
        double d4 = this.motionX * d1 + this.motionZ * d2;
        if (d4 < 0.0) {
            d1 = -d1;
            d2 = -d2;
        }
        if ((d5 = Math.sqrt((double)(this.motionX * this.motionX + this.motionZ * this.motionZ))) > 2.0) {
            d5 = 2.0;
        }
        ...
        if (flag1) {
            double d17 = Math.sqrt((double)(this.motionX * this.motionX + this.motionZ * this.motionZ));
            if (d17 < 0.03) {
                this.motionX *= 0.0;
                this.motionY *= 0.0;
                this.motionZ *= 0.0;
        ...
        d22 = MathHelper.clamp_double((double)d22, (double)(-d13), (double)d13);
        d23 = MathHelper.clamp_double((double)d23, (double)(-d13), (double)d13);
        this.moveEntity(d22, 0.0, d23);
        if (aint[0][1] != 0 && MathHelper.floor_double((double)this.posX) - p_180460_1_.getX() == aint[0][0] && MathHelper.floor_double((double)this.posZ) - p_180460_1_.getZ() == aint[0][2]) {
            this.setPosition(this.posX, this.posY + (double)aint[0][1], this.posZ);
        } else if (aint[1][1] != 0 && MathHelper.floor_double((double)this.posX) - p_180460_1_.getX() == aint[1][0] && MathHelper.floor_double((double)this.posZ) - p_180460_1_.getZ() == aint[1][2]) {
        ...
        if (vec31 != null && vec3 != null) {
            ...
            d5 = Math.sqrt((double)(this.motionX * this.motionX + this.motionZ * this.motionZ));
            if (d5 > 0.0) {
        ...
        int i = MathHelper.floor_double((double)this.posZ);
        ...
        if (flag) {
            double d15 = Math.sqrt((double)(this.motionX * this.motionX + this.motionZ * this.motionZ));
            if (d15 > 0.01) {
                double d16 = 0.06;
    ...
    public Vec3 func_70495_a(double p_70495_1_, double p_70495_3_, double p_70495_5_, double p_70495_7_) {
        ...
        int k;
        int j;
        int i = MathHelper.floor_double((double)p_70495_1_);
        if (BlockRailBase.isRailBlock((World)this.worldObj, (BlockPos)new BlockPos(i, (j = MathHelper.floor_double((double)p_70495_3_)) - 1, k = MathHelper.floor_double((double)p_70495_5_)))) {
            --j;
        }
        if (BlockRailBase.isRailBlock((IBlockState)(iblockstate = this.worldObj.getBlockState(new BlockPos(i, j, k))))) {
            ...
            double d2 = Math.sqrt((double)(d0 * d0 + d1 * d1));
            if (aint[0][1] != 0 && MathHelper.floor_double((double)(p_70495_1_ += (d0 /= d2) * p_70495_7_)) - i == aint[0][0] && MathHelper.floor_double((double)(p_70495_5_ += (d1 /= d2) * p_70495_7_)) - k == aint[0][2]) {
                p_70495_3_ += (double)aint[0][1];
            } else if (aint[1][1] != 0 && MathHelper.floor_double((double)p_70495_1_) - i == aint[1][0] && MathHelper.floor_double((double)p_70495_5_) - k == aint[1][2]) {
    ...
    public Vec3 func_70489_a(double p_70489_1_, double p_70489_3_, double p_70489_5_) {
        ...
        int k;
        int j;
        int i = MathHelper.floor_double((double)p_70489_1_);
        if (BlockRailBase.isRailBlock((World)this.worldObj, (BlockPos)new BlockPos(i, (j = MathHelper.floor_double((double)p_70489_3_)) - 1, k = MathHelper.floor_double((double)p_70489_5_)))) {
            --j;
        }
        if (BlockRailBase.isRailBlock((IBlockState)(iblockstate = this.worldObj.getBlockState(new BlockPos(i, j, k))))) {
            BlockRailBase.EnumRailDirection blockrailbase$enumraildirection = (BlockRailBase.EnumRailDirection)iblockstate.getValue(((BlockRailBase)iblockstate.getBlock()).getShapeProperty());
    ...
    protected void readEntityFromNBT(NBTTagCompound tagCompund) {
        if (tagCompund.getBoolean("CustomDisplayTile")) {
            ...
            if (tagCompund.hasKey("DisplayTile", 8)) {
                Block block = Block.getBlockFromName((String)tagCompund.getString("DisplayTile"));
                if (block == null) {
                ...
                Block block1 = Block.getBlockById((int)tagCompund.getInteger("DisplayTile"));
                if (block1 == null) {
                    this.func_174899_a(Blocks.air.getDefaultState());
                } else {
    ...
    protected void writeEntityToNBT(NBTTagCompound tagCompound) {
        if (this.hasDisplayTile()) {
            ...
            ResourceLocation resourcelocation = (ResourceLocation)Block.blockRegistry.getNameForObject((Object)iblockstate.getBlock());
    ...
    public void applyEntityCollision(Entity entityIn) {
        if (!(this.worldObj.isRemote || entityIn.noClip || this.noClip || entityIn == this.riddenByEntity)) {
            ...
            if (entityIn instanceof EntityLivingBase && !(entityIn instanceof EntityPlayer) && !(entityIn instanceof EntityIronGolem) && this.getMinecartType() == EnumMinecartType.RIDEABLE && this.motionX * this.motionX + this.motionZ * this.motionZ > 0.01 && this.riddenByEntity == null && entityIn.ridingEntity == null) {
                entityIn.mountEntity((Entity)this);
            }
            if ((d2 = (d0 = entityIn.posX - this.posX) * d0 + (d1 = entityIn.posZ - this.posZ) * d1) >= (double)1.0E-4f) {
                ...
                if (entityIn instanceof EntityMinecart) {
                    ...
                    double d6 = Math.abs((double)vec3.dotProduct(vec31 = new Vec3((double)MathHelper.cos((float)(this.rotationYaw * (float)Math.PI / 180.0f)), 0.0, (double)MathHelper.sin((float)(this.rotationYaw * (float)Math.PI / 180.0f))).normalize()));
                    if (d6 < (double)0.8f) {
                        return;
                    }
    ...
    public void setDamage(float p_70492_1_) {
        this.dataWatcher.updateObject(19, (Object)Float.valueOf((float)p_70492_1_));
    }
    ...
    public void setRollingAmplitude(int p_70497_1_) {
        this.dataWatcher.updateObject(17, (Object)p_70497_1_);
    }
    ...
    public void setRollingDirection(int p_70494_1_) {
        this.dataWatcher.updateObject(18, (Object)p_70494_1_);
    }
    ...
    public IBlockState getDisplayTile() {
        return !this.hasDisplayTile() ? this.getDefaultDisplayTile() : Block.getStateById((int)this.getDataWatcher().getWatchableObjectInt(20));
    }
    ...
    public void func_174899_a(IBlockState p_174899_1_) {
        this.getDataWatcher().updateObject(20, (Object)Block.getStateId((IBlockState)p_174899_1_));
        this.setHasDisplayTile(true);
    }

    public void setDisplayTileOffset(int p_94086_1_) {
        this.getDataWatcher().updateObject(21, (Object)p_94086_1_);
        this.setHasDisplayTile(true);
    }
    ...
    public void setHasDisplayTile(boolean p_94096_1_) {
        this.getDataWatcher().updateObject(22, (Object)((byte)(p_94096_1_ ? 1 : 0)));
    }
