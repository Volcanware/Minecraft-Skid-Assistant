...
extends Entity {
                ...
                if (this.onGround) {
                    ...
                    if (this.worldObj.getBlockState(blockpos1).getBlock() != Blocks.piston_extension) {
                        ...
                        if (!this.canSetAsBlock) {
                            if (this.worldObj.canBlockBePlaced(block, blockpos1, true, EnumFacing.UP, (Entity)null, (ItemStack)null) && !BlockFalling.canFallInto((World)this.worldObj, (BlockPos)blockpos1.down()) && this.worldObj.setBlockState(blockpos1, this.fallTile, 3)) {
    ...
    public void fall(float distance, float damageMultiplier) {
        int i;
        Block block = this.fallTile.getBlock();
        if (this.hurtEntities && (i = MathHelper.ceiling_float_int((float)(distance - 1.0f))) > 0) {
            ArrayList list = Lists.newArrayList((Iterable)this.worldObj.getEntitiesWithinAABBExcludingEntity((Entity)this, this.getEntityBoundingBox()));
            boolean flag = block == Blocks.anvil;
            DamageSource damagesource = flag ? DamageSource.anvil : DamageSource.fallingBlock;
            for (Entity entity : list) {
                entity.attackEntityFrom(damagesource, (float)Math.min((int)MathHelper.floor_float((float)((float)i * this.fallHurtAmount)), (int)this.fallHurtMax));
            }
            if (flag && (double)this.rand.nextFloat() < (double)0.05f + (double)i * 0.05) {
                int j = (Integer)this.fallTile.getValue((IProperty)BlockAnvil.DAMAGE);
                if (++j > 2) {
                    this.canSetAsBlock = true;
                } else {
                    this.fallTile = this.fallTile.withProperty((IProperty)BlockAnvil.DAMAGE, (Comparable)Integer.valueOf((int)j));
                }
            }
        }
    }
    ...
    protected void readEntityFromNBT(NBTTagCompound tagCompund) {
        ...
        this.fallTile = tagCompund.hasKey("Block", 8) ? Block.getBlockFromName((String)tagCompund.getString("Block")).getStateFromMeta(i) : (tagCompund.hasKey("TileID", 99) ? Block.getBlockById((int)tagCompund.getInteger("TileID")).getStateFromMeta(i) : Block.getBlockById((int)(tagCompund.getByte("Tile") & 0xFF)).getStateFromMeta(i));
    ...
    public void addEntityCrashInfo(CrashReportCategory category) {
        ...
        if (this.fallTile != null) {
            Block block = this.fallTile.getBlock();
            category.addCrashSection("Immitating block ID", (Object)Block.getIdFromBlock((Block)block));
            category.addCrashSection("Immitating block data", (Object)block.getMetaFromState(this.fallTile));
        }
    }
