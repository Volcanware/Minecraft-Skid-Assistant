...
extends EntityLiving {
    public static final UUID FLEEING_SPEED_MODIFIER_UUID = UUID.fromString((String)"E199AD21-BA8A-4C53-8D13-6182D5C69D3A");
    ...
    public boolean isWithinHomeDistanceFromPosition(BlockPos pos) {
        return this.maximumHomeDistance == -1.0f ? true : this.homePosition.distanceSq((Vec3i)pos) < (double)(this.maximumHomeDistance * this.maximumHomeDistance);
    }
    ...
    protected void updateLeashedState() {
        ...
        if (this.getLeashed() && this.getLeashedToEntity() != null && this.getLeashedToEntity().worldObj == this.worldObj) {
            ...
            if (f > 6.0f) {
                ...
                this.motionX += d0 * Math.abs((double)d0) * 0.4;
                this.motionY += d1 * Math.abs((double)d1) * 0.4;
                this.motionZ += d2 * Math.abs((double)d2) * 0.4;
            }
            if (f > 10.0f) {
                this.clearLeashed(true, true);
            }
