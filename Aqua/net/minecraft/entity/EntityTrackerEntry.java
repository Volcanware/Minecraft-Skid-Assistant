...
public class EntityTrackerEntry {
    ...
    public EntityTrackerEntry(Entity trackedEntityIn, int trackingDistanceThresholdIn, int updateFrequencyIn, boolean sendVelocityUpdatesIn) {
        ...
        this.encodedPosX = MathHelper.floor_double((double)(trackedEntityIn.posX * 32.0));
        this.encodedPosY = MathHelper.floor_double((double)(trackedEntityIn.posY * 32.0));
        this.encodedPosZ = MathHelper.floor_double((double)(trackedEntityIn.posZ * 32.0));
        this.encodedRotationYaw = MathHelper.floor_float((float)(trackedEntityIn.rotationYaw * 256.0f / 360.0f));
        this.encodedRotationPitch = MathHelper.floor_float((float)(trackedEntityIn.rotationPitch * 256.0f / 360.0f));
        this.lastHeadMotion = MathHelper.floor_float((float)(trackedEntityIn.getRotationYawHead() * 256.0f / 360.0f));
    ...
    public void updatePlayerList(List<EntityPlayer> players) {
        ...
        if (this.field_85178_v != this.trackedEntity.ridingEntity || this.trackedEntity.ridingEntity != null && this.updateCounter % 60 == 0) {
            ...
            this.sendPacketToTrackedPlayers((Packet)new S1BPacketEntityAttach(0, this.trackedEntity, this.trackedEntity.ridingEntity));
        }
        ...
        if (this.updateCounter % this.updateFrequency == 0 || this.trackedEntity.isAirBorne || this.trackedEntity.getDataWatcher().hasObjectChanged()) {
            if (this.trackedEntity.ridingEntity == null) {
                ...
                int k = MathHelper.floor_double((double)(this.trackedEntity.posX * 32.0));
                int j1 = MathHelper.floor_double((double)(this.trackedEntity.posY * 32.0));
                int k1 = MathHelper.floor_double((double)(this.trackedEntity.posZ * 32.0));
                int l1 = MathHelper.floor_float((float)(this.trackedEntity.rotationYaw * 256.0f / 360.0f));
                int i2 = MathHelper.floor_float((float)(this.trackedEntity.rotationPitch * 256.0f / 360.0f));
                int j2 = k - this.encodedPosX;
                int k2 = j1 - this.encodedPosY;
                int i = k1 - this.encodedPosZ;
                S18PacketEntityTeleport packet1 = null;
                boolean flag = Math.abs((int)j2) >= 4 || Math.abs((int)k2) >= 4 || Math.abs((int)i) >= 4 || this.updateCounter % 60 == 0;
                boolean bl = flag1 = Math.abs((int)(l1 - this.encodedRotationYaw)) >= 4 || Math.abs((int)(i2 - this.encodedRotationPitch)) >= 4;
                ...
                if (this.sendVelocityUpdates && ((d4 = (d0 = this.trackedEntity.motionX - this.lastTrackedEntityMotionX) * d0 + (d1 = this.trackedEntity.motionY - this.lastTrackedEntityMotionY) * d1 + (d2 = this.trackedEntity.motionZ - this.motionZ) * d2) > (d3 = 0.02) * d3 || d4 > 0.0 && this.trackedEntity.motionX == 0.0 && this.trackedEntity.motionY == 0.0 && this.trackedEntity.motionZ == 0.0)) {
                    ...
                    this.sendPacketToTrackedPlayers((Packet)new S12PacketEntityVelocity(this.trackedEntity.getEntityId(), this.lastTrackedEntityMotionX, this.lastTrackedEntityMotionY, this.motionZ));
                }
                if (packet1 != null) {
                ...
                boolean flag2;
                int j = MathHelper.floor_float((float)(this.trackedEntity.rotationYaw * 256.0f / 360.0f));
                int i1 = MathHelper.floor_float((float)(this.trackedEntity.rotationPitch * 256.0f / 360.0f));
                boolean bl = flag2 = Math.abs((int)(j - this.encodedRotationYaw)) >= 4 || Math.abs((int)(i1 - this.encodedRotationPitch)) >= 4;
                if (flag2) {
                    this.sendPacketToTrackedPlayers((Packet)new S14PacketEntity.S16PacketEntityLook(this.trackedEntity.getEntityId(), (byte)j, (byte)i1, this.trackedEntity.onGround));
                    this.encodedRotationYaw = j;
                    this.encodedRotationPitch = i1;
                }
                this.encodedPosX = MathHelper.floor_double((double)(this.trackedEntity.posX * 32.0));
                this.encodedPosY = MathHelper.floor_double((double)(this.trackedEntity.posY * 32.0));
                this.encodedPosZ = MathHelper.floor_double((double)(this.trackedEntity.posZ * 32.0));
                this.sendMetadataToAllAssociatedPlayers();
                this.ridingEntity = true;
            }
            int l = MathHelper.floor_float((float)(this.trackedEntity.getRotationYawHead() * 256.0f / 360.0f));
            if (Math.abs((int)(l - this.lastHeadMotion)) >= 4) {
                this.sendPacketToTrackedPlayers((Packet)new S19PacketEntityHeadLook(this.trackedEntity, (byte)l));
        ...
        if (this.trackedEntity.velocityChanged) {
            this.func_151261_b((Packet)new S12PacketEntityVelocity(this.trackedEntity));
    ...
    private void sendMetadataToAllAssociatedPlayers() {
        ...
        if (datawatcher.hasObjectChanged()) {
            this.func_151261_b((Packet)new S1CPacketEntityMetadata(this.trackedEntity.getEntityId(), datawatcher, false));
        }
        if (this.trackedEntity instanceof EntityLivingBase) {
            ...
            if (!set.isEmpty()) {
                this.func_151261_b((Packet)new S20PacketEntityProperties(this.trackedEntity.getEntityId(), (Collection)set));
            }
            set.clear();
        }
    }
    ...
    public void removeFromTrackedPlayers(EntityPlayerMP playerMP) {
        if (this.trackingPlayers.contains((Object)playerMP)) {
            playerMP.removeEntity(this.trackedEntity);
            this.trackingPlayers.remove((Object)playerMP);
        }
    }
    ...
    public void updatePlayerEntity(EntityPlayerMP playerMP) {
        if (playerMP != this.trackedEntity) {
            if (this.func_180233_c(playerMP)) {
                if (!this.trackingPlayers.contains((Object)playerMP) && (this.isPlayerWatchingThisChunk(playerMP) || this.trackedEntity.forceSpawn)) {
                    EntityPlayer entityplayer;
                    ServersideAttributeMap serversideattributemap;
                    Collection collection;
                    NBTTagCompound nbttagcompound;
                    this.trackingPlayers.add((Object)playerMP);
            ...
            } else if (this.trackingPlayers.contains((Object)playerMP)) {
                this.trackingPlayers.remove((Object)playerMP);
                playerMP.removeEntity(this.trackedEntity);
            }
        }
    ...
    private Packet createSpawnPacket() {
        ...
        if (this.trackedEntity instanceof IAnimals) {
            this.lastHeadMotion = MathHelper.floor_float((float)(this.trackedEntity.getRotationYawHead() * 256.0f / 360.0f));
        ...
        if (this.trackedEntity instanceof EntityFallingBlock) {
            ...
            return new S0EPacketSpawnObject(this.trackedEntity, 70, Block.getStateId((IBlockState)entityfallingblock.getBlock()));
        }
