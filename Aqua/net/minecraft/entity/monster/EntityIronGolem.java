...
extends EntityGolem {
    ...
    public EntityIronGolem(World worldIn) {
        ...
        this.targetTasks.addTask(3, (EntityAIBase)new AINearestAttackableTargetNonCreeper((EntityCreature)this, EntityLiving.class, 10, false, true, IMob.VISIBLE_MOB_SELECTOR));
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.motionX * this.motionX + this.motionZ * this.motionZ > 2.500000277905201E-7 && this.rand.nextInt(5) == 0 && (block = (iblockstate = this.worldObj.getBlockState(new BlockPos(i = MathHelper.floor_double((double)this.posX), j = MathHelper.floor_double((double)(this.posY - (double)0.2f)), k = MathHelper.floor_double((double)this.posZ)))).getBlock()).getMaterial() != Material.air) {
            this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double)this.rand.nextFloat() - 0.5) * (double)this.width, this.getEntityBoundingBox().minY + 0.1, this.posZ + ((double)this.rand.nextFloat() - 0.5) * (double)this.width, 4.0 * ((double)this.rand.nextFloat() - 0.5), 0.5, ((double)this.rand.nextFloat() - 0.5) * 4.0, new int[]{Block.getStateId((IBlockState)iblockstate)});
        }
    }
    ...
    public boolean attackEntityAsMob(Entity entityIn) {
        ...
        boolean flag = entityIn.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), (float)(7 + this.rand.nextInt(15)));
        if (flag) {
    ...
    protected void dropFewItems(boolean wasRecentlyHit, int lootingModifier) {
        ...
        for (int j = 0; j < i; ++j) {
            this.dropItemWithOffset(Item.getItemFromBlock((Block)Blocks.red_flower), 1, BlockFlower.EnumFlowerType.POPPY.getMeta());
        }
