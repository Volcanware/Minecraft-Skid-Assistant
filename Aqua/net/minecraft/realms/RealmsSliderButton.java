...
extends RealmsButton {
    ...
    public float toPct(float p_toPct_1_) {
        return MathHelper.clamp_float((float)((this.clamp(p_toPct_1_) - this.minValue) / (this.maxValue - this.minValue)), (float)0.0f, (float)1.0f);
    }

    public float toValue(float p_toValue_1_) {
        return this.clamp(this.minValue + (this.maxValue - this.minValue) * MathHelper.clamp_float((float)p_toValue_1_, (float)0.0f, (float)1.0f));
    }

    public float clamp(float p_clamp_1_) {
        p_clamp_1_ = this.clampSteps(p_clamp_1_);
        return MathHelper.clamp_float((float)p_clamp_1_, (float)this.minValue, (float)this.maxValue);
    }

    protected float clampSteps(float p_clampSteps_1_) {
        if (this.steps > 0) {
            p_clampSteps_1_ = this.steps * Math.round((float)(p_clampSteps_1_ / (float)this.steps));
        }
