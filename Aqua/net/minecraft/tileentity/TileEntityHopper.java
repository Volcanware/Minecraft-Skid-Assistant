...
ITickable {
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        for (int i = 0; i < nbttaglist.tagCount(); ++i) {
            ...
            this.inventory[j] = ItemStack.loadItemStackFromNBT((NBTTagCompound)nbttagcompound);
        }
    }
    ...
    public boolean updateHopper() {
        if (this.worldObj != null && !this.worldObj.isRemote) {
            if (!this.isOnTransferCooldown() && BlockHopper.isEnabled((int)this.getBlockMetadata())) {
                boolean flag = false;
                if (!this.isEmpty()) {
    ...
    private boolean transferItemsOut() {
        ...
        EnumFacing enumfacing = BlockHopper.getFacing((int)this.getBlockMetadata()).getOpposite();
    ...
    public static boolean captureDroppedItems(IHopper p_145891_0_) {
        ...
        if (iinventory != null) {
            ...
            for (EntityItem entityitem : TileEntityHopper.func_181556_a(p_145891_0_.getWorld(), p_145891_0_.getXPos(), p_145891_0_.getYPos() + 1.0, p_145891_0_.getZPos())) {
                if (!TileEntityHopper.putDropInInventoryAllSlots((IInventory)p_145891_0_, entityitem)) continue;
                return true;
            }
        }
        return false;
    ...
    private static boolean pullItemFromSlot(IHopper hopper, IInventory inventoryIn, int index, EnumFacing direction) {
        ...
        if (itemstack != null && TileEntityHopper.canExtractItemFromSlot(inventoryIn, itemstack, index, direction)) {
            ...
            ItemStack itemstack2 = TileEntityHopper.putStackInInventoryAllSlots((IInventory)hopper, inventoryIn.decrStackSize(index, 1), null);
    ...
    private static ItemStack insertStack(IInventory inventoryIn, ItemStack stack, int index, EnumFacing side) {
        ...
        if (TileEntityHopper.canInsertItemInSlot(inventoryIn, stack, index, side)) {
            ...
            if (itemstack == null) {
                ...
                int j = Math.min((int)stack.stackSize, (int)i);
                stack.stackSize -= j;
    ...
    private IInventory getInventoryForHopperTransfer() {
        EnumFacing enumfacing = BlockHopper.getFacing((int)this.getBlockMetadata());
    ...
    public static IInventory getInventoryAtPosition(World worldIn, double x, double y, double z) {
        ...
        BlockPos blockpos = new BlockPos(i, j = MathHelper.floor_double((double)y), k = MathHelper.floor_double((double)z));
        Block block = worldIn.getBlockState(blockpos).getBlock();
        if (block.hasTileEntity() && (tileentity = worldIn.getTileEntity(blockpos)) instanceof IInventory && (iinventory = (IInventory)tileentity) instanceof TileEntityChest && block instanceof BlockChest) {
            iinventory = ((BlockChest)block).getLockableContainer(worldIn, blockpos);
        }
        if (iinventory == null && (list = worldIn.getEntitiesInAABBexcluding((Entity)null, new AxisAlignedBB(x - 0.5, y - 0.5, z - 0.5, x + 0.5, y + 0.5, z + 0.5), EntitySelectors.selectInventories)).size() > 0) {
    ...
    private static boolean canCombine(ItemStack stack1, ItemStack stack2) {
        return stack1.getItem() != stack2.getItem() ? false : (stack1.getMetadata() != stack2.getMetadata() ? false : (stack1.stackSize > stack1.getMaxStackSize() ? false : ItemStack.areItemStackTagsEqual((ItemStack)stack1, (ItemStack)stack2)));
    }
