...
extends Block {
    ...
    private void checkFall(World worldIn, BlockPos pos) {
        if (BlockFalling.canFallInto((World)worldIn, (BlockPos)pos.down()) && pos.getY() >= 0) {
            int i = 32;
            if (!BlockFalling.fallInstantly && worldIn.isAreaLoaded(pos.add(-i, -i, -i), pos.add(i, i, i))) {
                worldIn.spawnEntityInWorld((Entity)new EntityFallingBlock(worldIn, (double)((float)pos.getX() + 0.5f), (double)pos.getY(), (double)((float)pos.getZ() + 0.5f), this.getDefaultState()));
            } else {
                worldIn.setBlockToAir(pos);
                BlockPos blockpos = pos;
                while (BlockFalling.canFallInto((World)worldIn, (BlockPos)blockpos) && blockpos.getY() > 0) {
