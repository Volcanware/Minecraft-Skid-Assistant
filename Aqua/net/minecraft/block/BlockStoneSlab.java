...
extends BlockSlab {
    ...
    public BlockStoneSlab() {
        ...
        this.setDefaultState(iblockstate.withProperty(VARIANT, (Comparable)EnumType.STONE));
    ...
    public void getSubBlocks(Item itemIn, CreativeTabs tab, List<ItemStack> list) {
        if (itemIn != Item.getItemFromBlock((Block)Blocks.double_stone_slab)) {
            for (EnumType blockstoneslab$enumtype : EnumType.values()) {
                if (blockstoneslab$enumtype == EnumType.WOOD) continue;
                list.add((Object)new ItemStack(itemIn, 1, blockstoneslab$enumtype.getMetadata()));
            }
        }
    }
