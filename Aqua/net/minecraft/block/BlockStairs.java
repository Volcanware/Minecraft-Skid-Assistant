...
extends Block {
    ...
    public static final PropertyEnum<EnumHalf> HALF = PropertyEnum.create((String)"half", EnumHalf.class);
    public static final PropertyEnum<EnumShape> SHAPE = PropertyEnum.create((String)"shape", EnumShape.class);
    ...
    protected BlockStairs(IBlockState modelState) {
        ...
        this.setDefaultState(this.blockState.getBaseState().withProperty((IProperty)FACING, (Comparable)EnumFacing.NORTH).withProperty(HALF, (Comparable)EnumHalf.BOTTOM).withProperty(SHAPE, (Comparable)EnumShape.STRAIGHT));
    ...
    public static boolean isSameStair(IBlockAccess worldIn, BlockPos pos, IBlockState state) {
        ...
        return BlockStairs.isBlockStairs(block) && iblockstate.getValue(HALF) == state.getValue(HALF) && iblockstate.getValue((IProperty)FACING) == state.getValue((IProperty)FACING);
    }
    ...
    public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
        ...
        iblockstate = iblockstate.withProperty((IProperty)FACING, (Comparable)placer.getHorizontalFacing()).withProperty(SHAPE, (Comparable)EnumShape.STRAIGHT);
        return facing != EnumFacing.DOWN && (facing == EnumFacing.UP || (double)hitY <= 0.5) ? iblockstate.withProperty(HALF, (Comparable)EnumHalf.BOTTOM) : iblockstate.withProperty(HALF, (Comparable)EnumHalf.TOP);
    }
    ...
    public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vec3 start, Vec3 end) {
        ...
        for (int j = 0; j < 8; ++j) {
            this.rayTracePass = j;
            if (Arrays.binarySearch((int[])aint, (int)j) >= 0) continue;
    ...
    public IBlockState getStateFromMeta(int meta) {
        IBlockState iblockstate = this.getDefaultState().withProperty(HALF, (Comparable)((meta & 4) > 0 ? EnumHalf.TOP : EnumHalf.BOTTOM));
        iblockstate = iblockstate.withProperty((IProperty)FACING, (Comparable)EnumFacing.getFront((int)(5 - (meta & 3))));
        return iblockstate;
    }
    ...
    public int getMetaFromState(IBlockState state) {
        ...
        return i |= 5 - ((EnumFacing)state.getValue((IProperty)FACING)).getIndex();
    }
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        if (this.func_176306_h(worldIn, pos)) {
            switch (this.func_176305_g(worldIn, pos)) {
                case 0: {
                    state = state.withProperty(SHAPE, (Comparable)EnumShape.STRAIGHT);
                    break;
                }
                case 1: {
                    state = state.withProperty(SHAPE, (Comparable)EnumShape.INNER_RIGHT);
                    break;
                }
                case 2: {
                    state = state.withProperty(SHAPE, (Comparable)EnumShape.INNER_LEFT);
                }
            }
        } else {
            switch (this.func_176307_f(worldIn, pos)) {
                ...
                case 1: {
                    state = state.withProperty(SHAPE, (Comparable)EnumShape.OUTER_RIGHT);
                    break;
                }
                case 2: {
                    state = state.withProperty(SHAPE, (Comparable)EnumShape.OUTER_LEFT);
                }
            }
        }
        return state;
