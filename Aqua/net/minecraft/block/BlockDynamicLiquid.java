...
extends BlockLiquid {
    ...
    private void placeStaticBlock(World worldIn, BlockPos pos, IBlockState currentState) {
        worldIn.setBlockState(pos, BlockDynamicLiquid.getStaticBlock((Material)this.blockMaterial).getDefaultState().withProperty((IProperty)LEVEL, currentState.getValue((IProperty)LEVEL)), 2);
    }
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (i > 0) {
            ...
            if (this.getLevel((IBlockAccess)worldIn, pos.up()) >= 0) {
                int j1 = this.getLevel((IBlockAccess)worldIn, pos.up());
                i1 = j1 >= 8 ? j1 : j1 + 8;
            }
            if (this.adjacentSourceBlocks >= 2 && this.blockMaterial == Material.water) {
                ...
                if (iblockstate1.getBlock().getMaterial().isSolid()) {
                    i1 = 0;
                } else if (iblockstate1.getBlock().getMaterial() == this.blockMaterial && (Integer)iblockstate1.getValue((IProperty)LEVEL) == 0) {
                    i1 = 0;
                }
            }
            ...
            if (i1 == i) {
                ...
                if (i1 < 0) {
                    ...
                    state = state.withProperty((IProperty)LEVEL, (Comparable)Integer.valueOf((int)i1));
                    worldIn.setBlockState(pos, state, 2);
                    worldIn.scheduleUpdate(pos, (Block)this, k);
    ...
    private void tryFlowInto(World worldIn, BlockPos pos, IBlockState state, int level) {
        if (this.canFlowInto(worldIn, pos, state)) {
            ...
            worldIn.setBlockState(pos, this.getDefaultState().withProperty((IProperty)LEVEL, (Comparable)Integer.valueOf((int)level)), 3);
        }
    }
    ...
    private int func_176374_a(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            BlockPos blockpos;
            if (enumfacing == calculateFlowCost || this.isBlocked(worldIn, blockpos = pos.offset(enumfacing), iblockstate = worldIn.getBlockState(blockpos)) || iblockstate.getBlock().getMaterial() == this.blockMaterial && (Integer)iblockstate.getValue((IProperty)LEVEL) <= 0) continue;
    ...
    private Set<EnumFacing> getPossibleFlowDirections(World worldIn, BlockPos pos) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            if (this.isBlocked(worldIn, blockpos, iblockstate = worldIn.getBlockState(blockpos)) || iblockstate.getBlock().getMaterial() == this.blockMaterial && (Integer)iblockstate.getValue((IProperty)LEVEL) <= 0) continue;
    ...
    protected int checkAdjacentBlock(World worldIn, BlockPos pos, int currentMinLevel) {
        int i = this.getLevel((IBlockAccess)worldIn, pos);
        if (i < 0) {
