...
implements Predicate<IBlockState> {
    ...
    public boolean apply(IBlockState p_apply_1_) {
        if (p_apply_1_ != null && p_apply_1_.getBlock().equals((Object)this.blockstate.getBlock())) {
            for (Map.Entry entry : this.propertyPredicates.entrySet()) {
                Comparable object = p_apply_1_.getValue((IProperty)entry.getKey());
                if (((Predicate)entry.getValue()).apply((Object)object)) continue;
                return false;
            }
            return true;
        }
