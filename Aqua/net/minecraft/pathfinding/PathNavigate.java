...
public abstract class PathNavigate {
    ...
    public final PathEntity getPathToXYZ(double x, double y, double z) {
        return this.getPathToPos(new BlockPos(MathHelper.floor_double((double)x), (int)y, MathHelper.floor_double((double)z)));
    }
    ...
    public boolean tryMoveToXYZ(double x, double y, double z, double speedIn) {
        PathEntity pathentity = this.getPathToXYZ(MathHelper.floor_double((double)x), (int)y, MathHelper.floor_double((double)z));
    ...
    public void onUpdateNavigation() {
        ...
        if (!this.noPath()) {
            ...
            if (this.canNavigate()) {
                ...
                if (vec3.yCoord > vec31.yCoord && !this.theEntity.onGround && MathHelper.floor_double((double)vec3.xCoord) == MathHelper.floor_double((double)vec31.xCoord) && MathHelper.floor_double((double)vec3.zCoord) == MathHelper.floor_double((double)vec31.zCoord)) {
            ...
            if (!this.noPath() && (vec32 = this.currentPath.getPosition((Entity)this.theEntity)) != null) {
                ...
                List list = this.worldObj.getCollidingBoundingBoxes((Entity)this.theEntity, axisalignedbb1.addCoord(0.0, -1.0, 0.0));
                double d0 = -1.0;
    ...
    protected void pathFollow() {
        ...
        int j1 = MathHelper.ceiling_float_int((float)this.theEntity.width);
        int k1 = (int)this.theEntity.height + 1;
        int l = j1;
        for (int i1 = i - 1; i1 >= this.currentPath.getCurrentPathIndex(); --i1) {
            if (!this.isDirectPathBetweenPoints(vec3, this.currentPath.getVectorFromIndex((Entity)this.theEntity, i1), j1, k1, l)) continue;
