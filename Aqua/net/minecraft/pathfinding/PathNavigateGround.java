...
extends PathNavigate {
    ...
    private int getPathablePosY() {
        if (this.theEntity.isInWater() && this.getCanSwim()) {
            ...
            Block block = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double((double)this.theEntity.posX), i, MathHelper.floor_double((double)this.theEntity.posZ))).getBlock();
            int j = 0;
            while (block == Blocks.flowing_water || block == Blocks.water) {
                block = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double((double)this.theEntity.posX), ++i, MathHelper.floor_double((double)this.theEntity.posZ))).getBlock();
                if (++j <= 16) continue;
    ...
    protected void removeSunnyPath() {
        ...
        if (this.shouldAvoidSun) {
            if (this.worldObj.canSeeSky(new BlockPos(MathHelper.floor_double((double)this.theEntity.posX), (int)(this.theEntity.getEntityBoundingBox().minY + 0.5), MathHelper.floor_double((double)this.theEntity.posZ)))) {
                return;
            }
    ...
    protected boolean isDirectPathBetweenPoints(Vec3 posVec31, Vec3 posVec32, int sizeX, int sizeY, int sizeZ) {
        int i = MathHelper.floor_double((double)posVec31.xCoord);
        int j = MathHelper.floor_double((double)posVec31.zCoord);
        ...
        double d3 = 1.0 / Math.sqrt((double)d2);
        if (!this.isSafeToStandAt(i, (int)posVec31.yCoord, j, sizeX += 2, sizeY, sizeZ += 2, posVec31, d0 *= d3, d1 *= d3)) {
            return false;
        }
        sizeX -= 2;
        sizeZ -= 2;
        double d4 = 1.0 / Math.abs((double)d0);
        double d5 = 1.0 / Math.abs((double)d1);
        ...
        int k = d0 < 0.0 ? -1 : 1;
        int l = d1 < 0.0 ? -1 : 1;
        int i1 = MathHelper.floor_double((double)posVec32.xCoord);
        int j1 = MathHelper.floor_double((double)posVec32.zCoord);
        int k1 = i1 - i;
        int l1 = j1 - j;
        while (k1 * k > 0 || l1 * l > 0) {
            if (d6 < d7) {
    ...
    private boolean isPositionClear(int p_179692_1_, int p_179692_2_, int p_179692_3_, int p_179692_4_, int p_179692_5_, int p_179692_6_, Vec3 p_179692_7_, double p_179692_8_, double p_179692_10_) {
        for (BlockPos blockpos : BlockPos.getAllInBox((BlockPos)new BlockPos(p_179692_1_, p_179692_2_, p_179692_3_), (BlockPos)new BlockPos(p_179692_1_ + p_179692_4_ - 1, p_179692_2_ + p_179692_5_ - 1, p_179692_3_ + p_179692_6_ - 1))) {
            Block block;
            double d1;
            double d0 = (double)blockpos.getX() + 0.5 - p_179692_7_.xCoord;
            if (!(d0 * p_179692_8_ + (d1 = (double)blockpos.getZ() + 0.5 - p_179692_7_.zCoord) * p_179692_10_ >= 0.0) || (block = this.worldObj.getBlockState(blockpos).getBlock()).isPassable((IBlockAccess)this.worldObj, blockpos)) continue;
            return false;
        }
        return true;
    }
