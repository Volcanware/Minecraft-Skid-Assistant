    ...
    public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale) {
        ...
        if (i > 0) {
            ...
            for (int j = 0; j < i; ++j) {
                ...
                f1 = f1 * 2.0f - 1.0f;
                f2 = f2 * 2.0f - 1.0f;
                float f6 = MathHelper.sqrt_float((float)((f *= -1.0f) * f + (f2 *= -1.0f) * f2));
                entity.prevRotationYaw = entity.rotationYaw = (float)(Math.atan2((double)f, (double)f2) * 180.0 / Math.PI);
                entity.prevRotationPitch = entity.rotationPitch = (float)(Math.atan2((double)(f1 *= -1.0f), (double)f6) * 180.0 / Math.PI);
                double d0 = 0.0;
                double d1 = 0.0;
                double d2 = 0.0;
                this.field_177168_a.getRenderManager().renderEntityWithPosYaw((Entity)entity, d0, d1, d2, 0.0f, partialTicks);
