...
implements IEntityRenderer {
    ...
    private void renderShadow(Entity entityIn, double x, double y, double z, float shadowAlpha, float partialTicks) {
        if (!Config.isShaders() || !Shaders.shouldSkipDefaultShadow) {
            ...
            int i = MathHelper.floor_double((double)(d5 - (double)f));
            int j = MathHelper.floor_double((double)(d5 + (double)f));
            int k = MathHelper.floor_double((double)(d0 - (double)f));
            int l = MathHelper.floor_double((double)d0);
            int i1 = MathHelper.floor_double((double)(d1 - (double)f));
            int j1 = MathHelper.floor_double((double)(d1 + (double)f));
            double d2 = x - d5;
            double d3 = y - d0;
            double d4 = z - d1;
            Tessellator tessellator = Tessellator.getInstance();
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance) {
        ...
        if (d0 <= (double)(maxDistance * maxDistance)) {
            ...
            GlStateManager.pushMatrix();
            GlStateManager.translate((float)((float)x + 0.0f), (float)((float)y + ((Entity)entityIn).height + 0.5f), (float)((float)z));
