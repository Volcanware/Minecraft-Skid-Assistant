...
public class ActiveRenderInfo {
    ...
    private static final FloatBuffer OBJECTCOORDS = GLAllocation.createDirectFloatBuffer((int)3);
    ...
    public static void updateRenderInfo(EntityPlayer entityplayerIn, boolean p_74583_1_) {
        ...
        rotationX = MathHelper.cos((float)(f3 * (float)Math.PI / 180.0f)) * (float)(1 - i * 2);
        rotationZ = MathHelper.sin((float)(f3 * (float)Math.PI / 180.0f)) * (float)(1 - i * 2);
        rotationYZ = -rotationZ * MathHelper.sin((float)(f2 * (float)Math.PI / 180.0f)) * (float)(1 - i * 2);
        rotationXY = rotationX * MathHelper.sin((float)(f2 * (float)Math.PI / 180.0f)) * (float)(1 - i * 2);
        rotationXZ = MathHelper.cos((float)(f2 * (float)Math.PI / 180.0f));
    }
    ...
    public static Block getBlockAtEntityViewpoint(World worldIn, Entity p_180786_1_, float p_180786_2_) {
        ...
        if (block.getMaterial().isLiquid()) {
            ...
            if (iblockstate.getBlock() instanceof BlockLiquid) {
                f = BlockLiquid.getLiquidHeightPercent((int)((Integer)iblockstate.getValue((IProperty)BlockLiquid.LEVEL))) - 0.11111111f;
            }
