...
extends ModelBase {
    ...
    public ModelEnderMite() {
        ...
        for (int i = 0; i < this.field_178713_d.length; ++i) {
            this.field_178713_d[i] = new ModelRenderer((ModelBase)this, field_178714_b[i][0], field_178714_b[i][1]);
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        for (int i = 0; i < this.field_178713_d.length; ++i) {
            this.field_178713_d[i].rotateAngleY = MathHelper.cos((float)(ageInTicks * 0.9f + (float)i * 0.15f * (float)Math.PI)) * (float)Math.PI * 0.01f * (float)(1 + Math.abs((int)(i - 2)));
            this.field_178713_d[i].rotationPointX = MathHelper.sin((float)(ageInTicks * 0.9f + (float)i * 0.15f * (float)Math.PI)) * (float)Math.PI * 0.1f * (float)Math.abs((int)(i - 2));
        }
    }
}
