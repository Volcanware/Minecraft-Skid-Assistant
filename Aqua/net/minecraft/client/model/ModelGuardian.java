...
extends ModelBase {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            this.guardianSpines[i].rotationPointX = afloat3[i] * (1.0f + MathHelper.cos((float)(ageInTicks * 1.5f + (float)i)) * 0.01f - f1);
            this.guardianSpines[i].rotationPointY = 16.0f + afloat4[i] * (1.0f + MathHelper.cos((float)(ageInTicks * 1.5f + (float)i)) * 0.01f - f1);
            this.guardianSpines[i].rotationPointZ = afloat5[i] * (1.0f + MathHelper.cos((float)(ageInTicks * 1.5f + (float)i)) * 0.01f - f1);
        }
        ...
        if (entity != null) {
            ...
            this.guardianEye.rotationPointX = MathHelper.sqrt_float((float)((float)Math.abs((double)d1))) * 2.0f * (float)Math.signum((double)d1);
        }
        this.guardianEye.showModel = true;
        float f2 = entityguardian.func_175471_a(f);
        this.guardianTail[0].rotateAngleY = MathHelper.sin((float)f2) * (float)Math.PI * 0.05f;
        this.guardianTail[1].rotateAngleY = MathHelper.sin((float)f2) * (float)Math.PI * 0.1f;
        this.guardianTail[1].rotationPointX = -1.5f;
        this.guardianTail[1].rotationPointY = 0.5f;
        this.guardianTail[1].rotationPointZ = 14.0f;
        this.guardianTail[2].rotateAngleY = MathHelper.sin((float)f2) * (float)Math.PI * 0.15f;
