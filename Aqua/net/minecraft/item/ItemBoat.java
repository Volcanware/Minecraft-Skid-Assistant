...
extends Item {
    ...
    public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn) {
        ...
        boolean flag = false;
        float f9 = 1.0f;
        List list = worldIn.getEntitiesWithinAABBExcludingEntity((Entity)playerIn, playerIn.getEntityBoundingBox().addCoord(vec32.xCoord * d3, vec32.yCoord * d3, vec32.zCoord * d3).expand((double)f9, (double)f9, (double)f9));
        ...
        if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            entityboat.rotationYaw = ((MathHelper.floor_double((double)((double)(playerIn.rotationYaw * 4.0f / 360.0f) + 0.5)) & 3) - 1) * 90;
            if (!worldIn.getCollidingBoundingBoxes((Entity)entityboat, entityboat.getEntityBoundingBox().expand(-0.1, -0.1, -0.1)).isEmpty()) {
                return itemStackIn;
            }
            if (!worldIn.isRemote) {
