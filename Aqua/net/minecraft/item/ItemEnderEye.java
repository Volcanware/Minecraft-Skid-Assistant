...
extends Item {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.canPlayerEdit(pos.offset(side), side, stack) && iblockstate.getBlock() == Blocks.end_portal_frame && !((Boolean)iblockstate.getValue((IProperty)BlockEndPortalFrame.EYE)).booleanValue()) {
            ...
            worldIn.setBlockState(pos, iblockstate.withProperty((IProperty)BlockEndPortalFrame.EYE, (Comparable)Boolean.valueOf((boolean)true)), 2);
            ...
            if (flag && j == l + 2) {
                ...
                for (int i1 = l; i1 <= j; ++i1) {
                    ...
                    if (iblockstate3.getBlock() == Blocks.end_portal_frame && ((Boolean)iblockstate3.getValue((IProperty)BlockEndPortalFrame.EYE)).booleanValue()) continue;
                    flag = false;
                    break;
                }
                block3: for (int j1 = l - 1; j1 <= j + 1; j1 += 4) {
                    ...
                    for (int l1 = 1; l1 <= 3; ++l1) {
                        ...
                        if (iblockstate2.getBlock() == Blocks.end_portal_frame && ((Boolean)iblockstate2.getValue((IProperty)BlockEndPortalFrame.EYE)).booleanValue()) continue;
                        flag = false;
                        continue block3;
                    }
                }
    ...
    public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn) {
        ...
        if (!worldIn.isRemote && (blockpos = worldIn.getStrongholdPos("Stronghold", new BlockPos((Entity)playerIn))) != null) {
