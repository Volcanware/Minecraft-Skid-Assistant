...
extends Item {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        List list = worldIn.getEntitiesWithinAABBExcludingEntity((Entity)null, AxisAlignedBB.fromBounds((double)d0, (double)(d1 = (double)blockpos.getY()), (double)(d2 = (double)blockpos.getZ()), (double)(d0 + 1.0), (double)(d1 + 2.0), (double)(d2 + 1.0)));
        if (list.size() > 0) {
            return false;
        }
        if (!worldIn.isRemote) {
            ...
            float f = (float)MathHelper.floor_float((float)((MathHelper.wrapAngleTo180_float((float)(playerIn.rotationYaw - 180.0f)) + 22.5f) / 45.0f)) * 45.0f;
