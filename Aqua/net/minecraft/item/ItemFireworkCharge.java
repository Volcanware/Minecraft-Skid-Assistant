...
extends Item {
    ...
    public static void addExplosionInfo(NBTTagCompound nbt, List<String> tooltip) {
        ...
        if (b0 >= 0 && b0 <= 4) {
            tooltip.add((Object)StatCollector.translateToLocal((String)("item.fireworksCharge.type." + b0)).trim());
        } else {
            tooltip.add((Object)StatCollector.translateToLocal((String)"item.fireworksCharge.type").trim());
        }
        ...
        if (aint.length > 0) {
            ...
            for (int i : aint) {
                ...
                for (int j = 0; j < ItemDye.dyeColors.length; ++j) {
                    ...
                    flag1 = true;
                    s = s + StatCollector.translateToLocal((String)("item.fireworksCharge." + EnumDyeColor.byDyeDamage((int)j).getUnlocalizedName()));
                    break;
                }
                if (flag1) continue;
                s = s + StatCollector.translateToLocal((String)"item.fireworksCharge.customColor");
            }
            tooltip.add((Object)s);
        }
        if ((aint1 = nbt.getIntArray("FadeColors")).length > 0) {
            boolean flag2 = true;
            String s1 = StatCollector.translateToLocal((String)"item.fireworksCharge.fadeTo") + " ";
            for (int l : aint1) {
                if (!flag2) {
                    s1 = s1 + ", ";
                }
                ...
                for (int k = 0; k < 16; ++k) {
                    ...
                    flag5 = true;
                    s1 = s1 + StatCollector.translateToLocal((String)("item.fireworksCharge." + EnumDyeColor.byDyeDamage((int)k).getUnlocalizedName()));
                    break;
                }
                if (flag5) continue;
                s1 = s1 + StatCollector.translateToLocal((String)"item.fireworksCharge.customColor");
            }
            tooltip.add((Object)s1);
        }
        if (flag3 = nbt.getBoolean("Trail")) {
            tooltip.add((Object)StatCollector.translateToLocal((String)"item.fireworksCharge.trail"));
        }
        if (flag4 = nbt.getBoolean("Flicker")) {
            tooltip.add((Object)StatCollector.translateToLocal((String)"item.fireworksCharge.flicker"));
        }
    }
}
