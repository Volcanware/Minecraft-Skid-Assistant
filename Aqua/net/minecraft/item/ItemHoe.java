    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (side != EnumFacing.DOWN && worldIn.getBlockState(pos.up()).getBlock().getMaterial() == Material.air) {
            ...
            if (block == Blocks.dirt) {
                switch (1.$SwitchMap$net$minecraft$block$BlockDirt$DirtType[((BlockDirt.DirtType)iblockstate.getValue((IProperty)BlockDirt.VARIANT)).ordinal()]) {
                    case 1: {
                        return this.useHoe(stack, playerIn, worldIn, pos, Blocks.farmland.getDefaultState());
                    }
                    case 2: {
                        return this.useHoe(stack, playerIn, worldIn, pos, Blocks.dirt.getDefaultState().withProperty((IProperty)BlockDirt.VARIANT, (Comparable)BlockDirt.DirtType.DIRT));
                    }
                }
            }
        }
