...
public final class ItemStack {
    ...
    public ItemStack(Block blockIn, int amount, int meta) {
        this(Item.getItemFromBlock((Block)blockIn), amount, meta);
    }
    ...
    public boolean onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (flag) {
            playerIn.triggerAchievement(StatList.objectUseStats[Item.getIdFromItem((Item)this.item)]);
        }
        return flag;
    }
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
        ResourceLocation resourcelocation = (ResourceLocation)Item.itemRegistry.getNameForObject((Object)this.item);
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        this.item = nbt.hasKey("id", 8) ? Item.getByNameOrId((String)nbt.getString("id")) : Item.getItemById((int)nbt.getShort("id"));
    ...
    public boolean attemptDamageItem(int amount, Random rand) {
        ...
        if (amount > 0) {
            int i = EnchantmentHelper.getEnchantmentLevel((int)Enchantment.unbreaking.effectId, (ItemStack)this);
            int j = 0;
            for (int k = 0; i > 0 && k < amount; ++k) {
                if (!EnchantmentDurability.negateDamage((ItemStack)this, (int)i, (Random)rand)) continue;
                ++j;
            }
            if ((amount -= j) <= 0) {
                return false;
    ...
    public void damageItem(int amount, EntityLivingBase entityIn) {
        if ((!(entityIn instanceof EntityPlayer) || !((EntityPlayer)entityIn).capabilities.isCreativeMode) && this.isItemStackDamageable() && this.attemptDamageItem(amount, entityIn.getRNG())) {
            ...
            if (entityIn instanceof EntityPlayer) {
                ...
                entityplayer.triggerAchievement(StatList.objectBreakStats[Item.getIdFromItem((Item)this.item)]);
    ...
    public static boolean areItemStackTagsEqual(ItemStack stackA, ItemStack stackB) {
        return stackA == null && stackB == null ? true : (stackA != null && stackB != null ? (stackA.stackTagCompound == null && stackB.stackTagCompound != null ? false : stackA.stackTagCompound == null || stackA.stackTagCompound.equals((Object)stackB.stackTagCompound)) : false);
    }
    ...
    private boolean isItemStackEqual(ItemStack other) {
        return this.stackSize != other.stackSize ? false : (this.item != other.item ? false : (this.itemDamage != other.itemDamage ? false : (this.stackTagCompound == null && other.stackTagCompound != null ? false : this.stackTagCompound == null || this.stackTagCompound.equals((Object)other.stackTagCompound))));
    }
    ...
    public void onCrafting(World worldIn, EntityPlayer playerIn, int amount) {
        playerIn.addStat(StatList.objectCraftStats[Item.getIdFromItem((Item)this.item)], amount);
    ...
    public NBTTagCompound getSubCompound(String key, boolean create) {
        ...
        if (create) {
            ...
            this.setTagInfo(key, (NBTBase)nbttagcompound);
    ...
    public List<String> getTooltip(EntityPlayer playerIn, boolean advanced) {
        ...
        if (advanced) {
            ...
            int i = Item.getIdFromItem((Item)this.item);
            s = this.getHasSubtypes() ? s + String.format((String)"#%04d/%d%s", (Object[])new Object[]{i, this.itemDamage, s1}) : s + String.format((String)"#%04d%s", (Object[])new Object[]{i, s1});
        ...
        if ((i1 & 0x20) == 0) {
            this.item.addInformation(this, playerIn, (List)list, advanced);
        }
        if (this.hasTagCompound()) {
            ...
            if ((i1 & 1) == 0 && (nbttaglist = this.getEnchantmentTagList()) != null) {
                for (int j = 0; j < nbttaglist.tagCount(); ++j) {
                    ...
                    if (Enchantment.getEnchantmentById((int)k) == null) continue;
                    list.add((Object)Enchantment.getEnchantmentById((int)k).getTranslatedName((int)l));
                }
            }
            if (this.stackTagCompound.hasKey("display", 10)) {
                ...
                if (nbttagcompound.hasKey("color", 3)) {
                    if (advanced) {
                        list.add((Object)("Color: #" + Integer.toHexString((int)nbttagcompound.getInteger("color")).toUpperCase()));
                    } else {
                        list.add((Object)(EnumChatFormatting.ITALIC + StatCollector.translateToLocal((String)"item.dyed")));
                    }
                }
                if (nbttagcompound.getTagId("Lore") == 9 && (nbttaglist1 = nbttagcompound.getTagList("Lore", 8)).tagCount() > 0) {
                    for (int j1 = 0; j1 < nbttaglist1.tagCount(); ++j1) {
                        list.add((Object)(EnumChatFormatting.DARK_PURPLE + "" + EnumChatFormatting.ITALIC + nbttaglist1.getStringTagAt(j1)));
                    }
                }
            }
        }
        if (!(multimap = this.getAttributeModifiers()).isEmpty() && (i1 & 2) == 0) {
            ...
            for (Map.Entry entry : multimap.entries()) {
                ...
                if (attributemodifier.getID() == Item.itemModifierUUID) {
                    d0 += (double)EnchantmentHelper.getModifierForCreature((ItemStack)this, (EnumCreatureAttribute)EnumCreatureAttribute.UNDEFINED);
                }
                double d1 = attributemodifier.getOperation() != 1 && attributemodifier.getOperation() != 2 ? d0 : d0 * 100.0;
                if (d0 > 0.0) {
                    list.add((Object)(EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted((String)("attribute.modifier.plus." + attributemodifier.getOperation()), (Object[])new Object[]{DECIMALFORMAT.format(d1), StatCollector.translateToLocal((String)("attribute.name." + (String)entry.getKey()))})));
                    continue;
                }
                if (!(d0 < 0.0)) continue;
                list.add((Object)(EnumChatFormatting.RED + StatCollector.translateToLocalFormatted((String)("attribute.modifier.take." + attributemodifier.getOperation()), (Object[])new Object[]{DECIMALFORMAT.format(d1 *= -1.0), StatCollector.translateToLocal((String)("attribute.name." + (String)entry.getKey()))})));
            }
        }
        if (this.hasTagCompound() && this.getTagCompound().getBoolean("Unbreakable") && (i1 & 4) == 0) {
            list.add((Object)(EnumChatFormatting.BLUE + StatCollector.translateToLocal((String)"item.unbreakable")));
        }
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanDestroy", 9) && (i1 & 8) == 0 && (nbttaglist2 = this.stackTagCompound.getTagList("CanDestroy", 8)).tagCount() > 0) {
            list.add((Object)"");
            list.add((Object)(EnumChatFormatting.GRAY + StatCollector.translateToLocal((String)"item.canBreak")));
            for (int k1 = 0; k1 < nbttaglist2.tagCount(); ++k1) {
                Block block = Block.getBlockFromName((String)nbttaglist2.getStringTagAt(k1));
                if (block != null) {
                    list.add((Object)(EnumChatFormatting.DARK_GRAY + block.getLocalizedName()));
                    continue;
                }
                list.add((Object)(EnumChatFormatting.DARK_GRAY + "missingno"));
            }
        }
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanPlaceOn", 9) && (i1 & 0x10) == 0 && (nbttaglist3 = this.stackTagCompound.getTagList("CanPlaceOn", 8)).tagCount() > 0) {
            list.add((Object)"");
            list.add((Object)(EnumChatFormatting.GRAY + StatCollector.translateToLocal((String)"item.canPlace")));
            for (int l1 = 0; l1 < nbttaglist3.tagCount(); ++l1) {
                Block block1 = Block.getBlockFromName((String)nbttaglist3.getStringTagAt(l1));
                if (block1 != null) {
                    list.add((Object)(EnumChatFormatting.DARK_GRAY + block1.getLocalizedName()));
                    continue;
                }
                list.add((Object)(EnumChatFormatting.DARK_GRAY + "missingno"));
            }
        ...
        if (advanced) {
            if (this.isItemDamaged()) {
                list.add((Object)("Durability: " + (this.getMaxDamage() - this.getItemDamage()) + " / " + this.getMaxDamage()));
            }
            list.add((Object)(EnumChatFormatting.DARK_GRAY + ((ResourceLocation)Item.itemRegistry.getNameForObject((Object)this.item)).toString()));
            if (this.hasTagCompound()) {
                list.add((Object)(EnumChatFormatting.DARK_GRAY + "NBT: " + this.getTagCompound().getKeySet().size() + " tag(s)"));
            }
        }
        return list;
    }
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers() {
        ...
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("AttributeModifiers", 9)) {
            ...
            for (int i = 0; i < nbttaglist.tagCount(); ++i) {
                ...
                AttributeModifier attributemodifier = SharedMonsterAttributes.readAttributeModifierFromNBT((NBTTagCompound)nbttagcompound);
    ...
    public IChatComponent getChatComponent() {
        ...
        if (this.hasDisplayName()) {
            chatcomponenttext.getChatStyle().setItalic(Boolean.valueOf((boolean)true));
        }
    ...
    public boolean canDestroy(Block blockIn) {
        ...
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanDestroy", 9)) {
            ...
            for (int i = 0; i < nbttaglist.tagCount(); ++i) {
                Block block = Block.getBlockFromName((String)nbttaglist.getStringTagAt(i));
