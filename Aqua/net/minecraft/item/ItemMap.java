...
extends ItemMapBase {
    ...
    public MapData getMapData(ItemStack stack, World worldIn) {
        ...
        if (mapdata == null && !worldIn.isRemote) {
            ...
            mapdata.calculateMapCenter((double)worldIn.getWorldInfo().getSpawnX(), (double)worldIn.getWorldInfo().getSpawnZ(), (int)mapdata.scale);
    ...
    public void updateMapData(World worldIn, Entity viewer, MapData data) {
        if (worldIn.provider.getDimensionId() == data.dimension && viewer instanceof EntityPlayer) {
            ...
            int j = data.xCenter;
            int k = data.zCenter;
            int l = MathHelper.floor_double((double)(viewer.posX - (double)j)) / i + 64;
            int i1 = MathHelper.floor_double((double)(viewer.posZ - (double)k)) / i + 64;
            int j1 = 128 / i;
                ...
                for (int l1 = i1 - j1 - 1; l1 < i1 + j1; ++l1) {
                    ...
                    if (worldIn.provider.getHasNoSky()) {
                        ...
                        if (((l3 = l3 * l3 * 31287121 + l3 * 11) >> 20 & 1) == 0) {
                            multiset.add((Object)Blocks.dirt.getMapColor(Blocks.dirt.getDefaultState().withProperty((IProperty)BlockDirt.VARIANT, (Comparable)BlockDirt.DirtType.DIRT)), 10);
                        } else {
                            multiset.add((Object)Blocks.stone.getMapColor(Blocks.stone.getDefaultState().withProperty((IProperty)BlockStone.VARIANT, (Comparable)BlockStone.EnumType.STONE)), 100);
                        }
                        d1 = 100.0;
                    } else {
                        ...
                        for (int i4 = 0; i4 < i; ++i4) {
                            for (int j4 = 0; j4 < i; ++j4) {
                                ...
                                if (k4 > 1) {
                                    while ((iblockstate = chunk.getBlockState((BlockPos)blockpos$mutableblockpos.set(i4 + i3, --k4, j4 + j3))).getBlock().getMapColor(iblockstate) == MapColor.airColor && k4 > 0) {
                                    }
    ...
    public void onCreated(ItemStack stack, World worldIn, EntityPlayer playerIn) {
        if (stack.hasTagCompound() && stack.getTagCompound().getBoolean("map_is_scaling")) {
            ...
            mapdata1.calculateMapCenter((double)mapdata.xCenter, (double)mapdata.zCenter, (int)mapdata1.scale);
    ...
    public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
        ...
        if (advanced) {
            if (mapdata == null) {
                tooltip.add((Object)"Unknown map");
            } else {
                tooltip.add((Object)("Scaling at 1:" + (1 << mapdata.scale)));
                tooltip.add((Object)("(Level " + mapdata.scale + "/" + 4 + ")"));
            }
        }
    }
}
