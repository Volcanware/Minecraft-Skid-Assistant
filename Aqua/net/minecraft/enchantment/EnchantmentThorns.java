...
extends Enchantment {
    ...
    public void onUserHurt(EntityLivingBase user, Entity attacker, int level) {
        ...
        ItemStack itemstack = EnchantmentHelper.getEnchantedItem((Enchantment)Enchantment.thorns, (EntityLivingBase)user);
        if (EnchantmentThorns.func_92094_a(level, random)) {
            if (attacker != null) {
                attacker.attackEntityFrom(DamageSource.causeThornsDamage((Entity)user), (float)EnchantmentThorns.func_92095_b(level, random));
