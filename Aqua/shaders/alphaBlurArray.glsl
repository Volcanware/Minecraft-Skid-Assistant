...
#define radius 1.75 * sigmaArray

float gauss(float x, float sigmaArray) {
    float sq = x / sigmaArray;
    return 1.0 / (abs(sigmaArray) * 2.50662827) * exp(-0.5 * sq * sq);
}
...
void main() {
    ...
    for (float i = -radius; i <= radius; i++) {
        ...
        float alpha = smpl.a * gauss(i, sigmaArray);
