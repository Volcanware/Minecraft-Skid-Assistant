...
float map(vec3 p){
    ...
    vec4 n = noised(vec3(p.xz*0.07, u_time*0.5));
    float nn = n.x * (length((n.yzw)));
    n = noised(vec3(p.xz*0.173, u_time*0.639));
...
void main()
{
    ...
    vec2 u2 = -1.0+2.0*uv;
    u2.x *= u_texelSize.x/u_texelSize.y;
    ...
    gl_FragColor.rgb = a*(0.99+0.02*hash(vec3(uv,u_time)));
    gl_FragColor.a = 1.0;
}