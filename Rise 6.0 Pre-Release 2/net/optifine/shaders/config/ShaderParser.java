...
public class ShaderParser {
    ...
    public static ShaderLine parseLine(final String line) {
        final Matcher matcher = PATTERN_UNIFORM.matcher(line);

        if (matcher.matches()) {
            return new ShaderLine(1, matcher.group(1), "", line);
        } else {
            final Matcher matcher1 = PATTERN_ATTRIBUTE.matcher(line);

            if (matcher1.matches()) {
                return new ShaderLine(2, matcher1.group(1), "", line);
            } else {
                final Matcher matcher2 = PATTERN_PROPERTY.matcher(line);

                if (matcher2.matches()) {
                    return new ShaderLine(6, matcher2.group(2), matcher2.group(3), line);
                } else {
                    final Matcher matcher3 = PATTERN_CONST_INT.matcher(line);

                    if (matcher3.matches()) {
                        return new ShaderLine(3, matcher3.group(1), matcher3.group(2), line);
                    } else {
                        final Matcher matcher4 = PATTERN_CONST_FLOAT.matcher(line);

                        if (matcher4.matches()) {
                            return new ShaderLine(4, matcher4.group(1), matcher4.group(2), line);
                        } else {
                            final Matcher matcher5 = PATTERN_CONST_BOOL.matcher(line);

                            if (matcher5.matches()) {
                                return new ShaderLine(5, matcher5.group(1), matcher5.group(2), line);
                            } else {
                                final Matcher matcher6 = PATTERN_EXTENSION.matcher(line);

                                if (matcher6.matches()) {
                                    return new ShaderLine(7, matcher6.group(1), matcher6.group(2), line);
                                } else {
                                    final Matcher matcher7 = PATTERN_CONST_VEC4.matcher(line);
    ...
    public static int getIndex(final String uniform, final String prefix, final int minIndex, final int maxIndex) {
        if (uniform.length() != prefix.length() + 1) {
            ...
            final int i = uniform.charAt(prefix.length()) - 48;
