    ...
    public boolean clickBlock(final BlockPos loc, final EnumFacing face) {
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(loc)) {
            ...
            if (this.currentGameType.isCreative()) {
                ...
                final BlockDamageEvent bdEvent = new BlockDamageEvent(block1, this.mc.thePlayer, this.mc.thePlayer.worldObj, loc);
                Client.INSTANCE.getEventBus().handle(bdEvent);
                if (flag && bdEvent.getRelativeBlockHardness() >= 1.0F) {
    ...
    public boolean onPlayerDamageBlock(final BlockPos posBlock, final EnumFacing directionFacing) {
        ...
        if (this.blockHitDelay > 0) {
            ...
            if (block.getMaterial() == Material.air) {
                ...
                final BlockDamageEvent bdEvent = new BlockDamageEvent(block, this.mc.thePlayer, this.mc.thePlayer.worldObj, posBlock);
                Client.INSTANCE.getEventBus().handle(bdEvent);
                this.curBlockDamageMP += bdEvent.getRelativeBlockHardness();
