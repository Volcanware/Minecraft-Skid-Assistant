...
public class CommandFill extends CommandBase {
    ...
    public void processCommand(final ICommandSender sender, final String[] args) throws CommandException {
        if (args.length < 7) {
            ...
            final BlockPos blockpos2 = new BlockPos(Math.min(blockpos.getX(), blockpos1.getX()), Math.min(blockpos.getY(), blockpos1.getY()), Math.min(blockpos.getZ(), blockpos1.getZ()));
            final BlockPos blockpos3 = new BlockPos(Math.max(blockpos.getX(), blockpos1.getX()), Math.max(blockpos.getY(), blockpos1.getY()), Math.max(blockpos.getZ(), blockpos1.getZ()));
            ...
            if (j > 32768) {
                ...
                if (args.length >= 10 && block.hasTileEntity()) {
                    final String s = getChatComponentFromNthArg(sender, args, 9).getUnformattedText();
                ...
                for (int i1 = blockpos2.getZ(); i1 <= blockpos3.getZ(); ++i1) {
                    for (int j1 = blockpos2.getY(); j1 <= blockpos3.getY(); ++j1) {
                        for (int k1 = blockpos2.getX(); k1 <= blockpos3.getX(); ++k1) {
                            final BlockPos blockpos4 = new BlockPos(k1, j1, i1);
                            ...
                            if (args.length >= 9) {
                                if (!args[8].equals("outline") && !args[8].equals("hollow")) {
                                    if (args[8].equals("destroy")) {
                                        ...
                                        if (args.length > 9) {
                                            final Block block1 = CommandBase.getBlockByText(sender, args[9]);
                                        ...
                                        if (args.length > 10) {
                                            final int l1 = CommandBase.parseInt(args[10]);
                            ...
                            final TileEntity tileentity1 = world.getTileEntity(blockpos4);
                            ...
                            final IBlockState iblockstate1 = block.getStateFromMeta(i);
                ...
                for (final BlockPos blockpos5 : list) {
                    final Block block2 = world.getBlockState(blockpos5).getBlock();
