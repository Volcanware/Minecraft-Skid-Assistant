...
public abstract class PathNavigate {
    ...
    public void onUpdateNavigation() {
        ...
        if (!this.noPath()) {
            if (this.canNavigate()) {
                ...
                final Vec3 vec3 = this.getEntityPosition();
                final Vec3 vec31 = this.currentPath.getVectorFromIndex(this.theEntity, this.currentPath.getCurrentPathIndex());
            ...
            if (!this.noPath()) {
                final Vec3 vec32 = this.currentPath.getPosition(this.theEntity);
                ...
                if (vec32 != null) {
                    ...
                    final List<AxisAlignedBB> list = this.worldObj.getCollidingBoundingBoxes(this.theEntity, axisalignedbb1.addCoord(0.0D, -1.0D, 0.0D));
                    double d0 = -1.0D;
    ...
    protected void pathFollow() {
        ...
        for (int k = this.currentPath.getCurrentPathIndex(); k < i; ++k) {
            final Vec3 vec31 = this.currentPath.getVectorFromIndex(this.theEntity, k);
        ...
        final int j1 = MathHelper.ceiling_float_int(this.theEntity.width);
        final int k1 = (int) this.theEntity.height + 1;
        final int l = j1;
