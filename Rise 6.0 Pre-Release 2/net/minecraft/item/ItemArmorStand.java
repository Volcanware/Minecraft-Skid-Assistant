...
public class ItemArmorStand extends Item {
        ...
        if (side == EnumFacing.DOWN) {
            ...
            if (!playerIn.canPlayerEdit(blockpos, side, stack)) {
                ...
                if (flag1) {
                    ...
                    final double d1 = blockpos.getY();
                    final double d2 = blockpos.getZ();
                    final List<Entity> list = worldIn.getEntitiesWithinAABBExcludingEntity(null, AxisAlignedBB.fromBounds(d0, d1, d2, d0 + 1.0D, d1 + 2.0D, d2 + 1.0D));
                    ...
                    if (list.size() > 0) {
                        ...
                        if (!worldIn.isRemote) {
                            ...
                            final EntityArmorStand entityarmorstand = new EntityArmorStand(worldIn, d0 + 0.5D, d1, d2 + 0.5D);
                            final float f = (float) MathHelper.floor_float((MathHelper.wrapAngleTo180_float(playerIn.rotationYaw - 180.0F) + 22.5F) / 45.0F) * 45.0F;
    ...
    private void applyRandomRotations(final EntityArmorStand armorStand, final Random rand) {
        ...
        final float f1 = rand.nextFloat() * 20.0F - 10.0F;
