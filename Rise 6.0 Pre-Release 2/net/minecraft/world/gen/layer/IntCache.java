    ...
    public static synchronized int[] getIntCache(final int p_76445_0_) {
        if (p_76445_0_ <= 256) {
            if (freeSmallArrays.isEmpty()) {
                ...
                final int[] aint3 = freeSmallArrays.remove(freeSmallArrays.size() - 1);
            ...
            final int[] aint2 = new int[intCacheSize];
            inUseLargeArrays.add(aint2);
            return aint2;
        } else if (freeLargeArrays.isEmpty()) {
            final int[] aint1 = new int[intCacheSize];
            inUseLargeArrays.add(aint1);
            return aint1;
        } else {
            final int[] aint = freeLargeArrays.remove(freeLargeArrays.size() - 1);
