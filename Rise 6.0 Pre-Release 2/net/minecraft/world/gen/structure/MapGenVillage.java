...
public class MapGenVillage extends MapGenStructure {
    ...
    public MapGenVillage(final Map<String, String> p_i2093_1_) {
        ...
        for (final Entry<String, String> entry : p_i2093_1_.entrySet()) {
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        if (i == k && j == l) {
            final boolean flag = this.worldObj.getWorldChunkManager().areBiomesViable(i * 16 + 8, j * 16 + 8, 0, villageSpawnBiomes);
    ...
    public static class Start extends StructureStart {
        ...
        public Start(final World worldIn, final Random rand, final int x, final int z, final int p_i2092_5_) {
            ...
            final List<StructureComponent> list1 = structurevillagepieces$start.field_74930_j;
            final List<StructureComponent> list2 = structurevillagepieces$start.field_74932_i;

            while (!list1.isEmpty() || !list2.isEmpty()) {
                if (list1.isEmpty()) {
                    final int i = rand.nextInt(list2.size());
                    final StructureComponent structurecomponent = list2.remove(i);
                    structurecomponent.buildComponent(structurevillagepieces$start, this.components, rand);
                } else {
                    final int j = rand.nextInt(list1.size());
                    final StructureComponent structurecomponent2 = list1.remove(j);
            ...
            for (final StructureComponent structurecomponent1 : this.components) {
