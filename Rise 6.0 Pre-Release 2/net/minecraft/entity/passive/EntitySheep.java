...
public class EntitySheep extends EntityAnimal {
    ...
    protected void dropFewItems(final boolean p_70628_1_, final int p_70628_2_) {
        ...
        final int i = this.rand.nextInt(2) + 1 + this.rand.nextInt(1 + p_70628_2_);
    ...
    public float getHeadRotationAngleX(final float p_70890_1_) {
        if (this.sheepTimer > 4 && this.sheepTimer <= 36) {
            final float f = ((float) (this.sheepTimer - 4) - p_70890_1_) / 32.0F;
    ...
    public boolean interact(final EntityPlayer player) {
        ...
        if (itemstack != null && itemstack.getItem() == Items.shears && !this.getSheared() && !this.isChild()) {
            if (!this.worldObj.isRemote) {
                ...
                for (int j = 0; j < i; ++j) {
                    final EntityItem entityitem = this.entityDropItem(new ItemStack(Item.getItemFromBlock(Blocks.wool), 1, this.getFleeceColor().getMetadata()), 1.0F);
    ...
    private EnumDyeColor getDyeColorMixFromParents(final EntityAnimal father, final EntityAnimal mother) {
        ...
        final ItemStack itemstack = CraftingManager.getInstance().findMatchingRecipe(this.inventoryCrafting, father.worldObj);
        final int k;
