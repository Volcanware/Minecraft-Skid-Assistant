...
public class EntityWither extends EntityMob implements IBossDisplayData, IRangedAttackMob {
        ...
        if (this.getInvulTime() > 0) {
            ...
            for (int i = 1; i < 3; ++i) {
                if (this.ticksExisted >= this.field_82223_h[i - 1]) {
                    ...
                    final int k1 = this.getWatchedTargetId(i);

                    if (k1 > 0) {
                        final Entity entity = this.worldObj.getEntityByID(k1);
                        ...
                        final List<EntityLivingBase> list = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(20.0D, 8.0D, 20.0D), Predicates.and(attackEntitySelector, EntitySelectors.NOT_SPECTATING));
            ...
            if (this.blockBreakCounter > 0) {
                ...
                if (this.blockBreakCounter == 0 && this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing")) {
                    final int i1 = MathHelper.floor_double(this.posY);
                    ...
                    for (int k2 = -1; k2 <= 1; ++k2) {
                        for (int l2 = -1; l2 <= 1; ++l2) {
                            for (int j = 0; j <= 3; ++j) {
                                ...
                                final int k = i1 + j;
                                final int l = i2 + l2;
                                final BlockPos blockpos = new BlockPos(i3, k, l);
    ...
    private double func_82214_u(final int p_82214_1_) {
        if (p_82214_1_ <= 0) {
            ...
            final float f = (this.renderYawOffset + (float) (180 * (p_82214_1_ - 1))) / 180.0F * (float) Math.PI;
    ...
    private double func_82213_w(final int p_82213_1_) {
        if (p_82213_1_ <= 0) {
            ...
            final float f = (this.renderYawOffset + (float) (180 * (p_82213_1_ - 1))) / 180.0F * (float) Math.PI;
    ...
    private void launchWitherSkullToCoords(final int p_82209_1_, final double x, final double y, final double z, final boolean invulnerable) {
        ...
        final double d1 = this.func_82208_v(p_82209_1_);
        final double d2 = this.func_82213_w(p_82209_1_);
        final double d3 = x - d0;
        final double d4 = y - d1;
        final double d5 = z - d2;
        final EntityWitherSkull entitywitherskull = new EntityWitherSkull(this.worldObj, this, d3, d4, d5);
    ...
    public boolean attackEntityFrom(final DamageSource source, final float amount) {
        if (this.isEntityInvulnerable(source)) {
            ...
            if (this.getInvulTime() > 0 && source != DamageSource.outOfWorld) {
                ...
                final Entity entity1 = source.getEntity();
    ...
    protected void dropFewItems(final boolean p_70628_1_, final int p_70628_2_) {
        ...
        if (!this.worldObj.isRemote) {
            for (final EntityPlayer entityplayer : this.worldObj.getEntitiesWithinAABB(EntityPlayer.class, this.getEntityBoundingBox().expand(50.0D, 100.0D, 50.0D))) {
