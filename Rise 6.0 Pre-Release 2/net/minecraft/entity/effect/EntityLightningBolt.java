...
public class EntityLightningBolt extends EntityWeatherEffect {
    ...
    public EntityLightningBolt(final World worldIn, final double posX, final double posY, final double posZ) {
        ...
        if (!worldIn.isRemote && worldIn.getGameRules().getGameRuleBooleanValue("doFireTick") && (worldIn.getDifficulty() == EnumDifficulty.NORMAL || worldIn.getDifficulty() == EnumDifficulty.HARD) && worldIn.isAreaLoaded(blockpos, 10)) {
            ...
            for (int i = 0; i < 4; ++i) {
                final BlockPos blockpos1 = blockpos.add(this.rand.nextInt(3) - 1, this.rand.nextInt(3) - 1, this.rand.nextInt(3) - 1);
    ...
    public void onUpdate() {
        ...
        if (this.lightningState >= 0) {
            if (this.worldObj.isRemote) {
                ...
                final List<Entity> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, new AxisAlignedBB(this.posX - d0, this.posY - d0, this.posZ - d0, this.posX + d0, this.posY + 6.0D + d0, this.posZ + d0));
