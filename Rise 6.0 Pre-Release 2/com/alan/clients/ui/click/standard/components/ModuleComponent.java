...
//        RenderUtil.dropShadow(3, (float) position.x, (float) position.y, scale.x, scale.y, 42, 1);
        ...
        if (clickGUI.getSelectedScreen() instanceof SearchScreen) {
            SearchScreen screen = (SearchScreen) clickGUI.getSelectedScreen();

            if (!screen.searchBar.getText().isEmpty()) {
                FontManager.getNunito(15).drawString("(" + Localization.get(module.getModuleInfo().category().getName()) + ")",
                        (float) (position.getX() + FontManager.getNunito(20).width(this.module.getModuleInfo().name()) + 10F),
                        (float) position.getY() + 10, ColorUtil.withAlpha(fontColor, 64).hashCode());
            }
        }

        // Draw module name
        FontManager.getNunito(20).drawString(this.module.getModuleInfo().name(), (float) position.x + 6f, (float) position.y + 8,
