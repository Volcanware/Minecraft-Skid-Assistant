...
public interface InstanceAccess {
    ...
    static void render2DRunnables(float partialTicks, boolean shaders) {
        ...
        if (shaders) {
            ...
            render2dProfiler.start();
            NORMAL_ABOVE_BLOOM_RUNNABLES.forEach(Runnable::run);
            render2dProfiler.stop();
        }
        ...
        NORMAL_UI_RENDER_RUNNABLES.forEach(Runnable::run);
//        RiseShaders.UI_BLOOM_SHADER.run(ShaderRenderType.OVERLAY, partialTicks, InstanceAccess.NORMAL_UI_BLOOM_RUNNABLES);
