 ...
 * @since 10/13/2022
 */
@ModuleInfo(name = "Session Stats", description = "module.render.sessionstats.description", category = Category.RENDER)
public class SessionStats extends Module {
    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        ...
        NORMAL_POST_RENDER_RUNNABLES.add(() -> {
            ...
            String walkDistance = session.distanceWalked < 1000 ? Math.round(session.distanceWalked) + " meters" : Math.round(session.distanceWalked / 10D) / 100D + " km";
            String flyDistance = session.distanceFlown < 1000 ? Math.round(session.distanceFlown) + " meters" : Math.round(session.distanceFlown / 10D) / 100D + " km";

            FontManager.getProductSansBold(24).drawString(Localization.get("ui.sessionstats.name"), position.position.x + padding, position.position.y + padding, -1);
            FontManager.getProductSansBold(18).drawString(time, position.position.x + padding,
                    position.position.y + padding + 15, new Color(255, 255, 255, 200).getRGB());

            FontManager.getProductSansMedium(18).drawString(Localization.get("ui.sessionstats.kills") + " " + session.kills, position.position.x + padding,
                    position.position.y + padding + 35, new Color(255, 255, 255, 200).getRGB());
            FontManager.getProductSansMedium(18).drawString(Localization.get("ui.sessionstats.wins") + " " + session.wins, position.position.x + padding + 80,
                    position.position.y + padding + 35, new Color(255, 255, 255, 200).getRGB());

            FontManager.getProductSansMedium(18).drawString(Localization.get("ui.sessionstats.distanceran") + " " + walkDistance, position.position.x + padding,
                    position.position.y + padding + 52, new Color(255, 255, 255, 200).getRGB());
            FontManager.getProductSansMedium(18).drawString(Localization.get("ui.sessionstats.distanceflown") + " " +  flyDistance, position.position.x + padding,
                    position.position.y + padding + 64, new Color(255, 255, 255, 200).getRGB());
        });
...
//        if (true) return;
        ...
        NORMAL_POST_BLOOM_RUNNABLES.add(() -> {
            FontManager.getProductSansBold(24).drawString(Localization.get("ui.sessionstats.name"), position.position.x + padding, position.position.y + padding, 0xFFFFFFFF);
            FontManager.getProductSansBold(18).drawString(time, position.position.x + padding, position.position.y + padding + 15, 0xFFFFFFFF);
        });
    };
