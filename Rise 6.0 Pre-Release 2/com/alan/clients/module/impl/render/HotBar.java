...
@ModuleInfo(name = "Hotbar", description = "module.render.hotbar.description", category = Category.RENDER, autoEnabled = true, allowDisable = false)
public class HotBar extends Module {

    private final BooleanValue blur = new BooleanValue("Blur", this, true);
    private final BooleanValue shadow = new BooleanValue("Shadow", this, true);
    private final NumberValue rounding = new NumberValue("Rounding Radius", this, 5, 1, 7, 1);
    ...
    private void renderHotBar(final ScaledResolution scaledResolution, final Color color) {
        if (mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            structure.position = new Vector2d(scaledResolution.getScaledWidth() / 2.0F - 91, scaledResolution.getScaledHeight() - 22 - 4);
            structure.scale = new Vector2d(91 * 2, 22);

            RenderUtil.roundedRectangle(structure.position.x, structure.position.y, structure.scale.x, structure.scale.y, this.rounding.getValue().intValue(), color);
        }
    }
    ...
    private void renderHotBarScroll() {
        ...
        if (mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            RenderUtil.roundedRectangle(rPosX + 1, scaledResolution.getScaledHeight() - 22 - 4, 22,
                    22, this.rounding.getValue().intValue(), getTheme().getBackgroundShade());
        }
    }
