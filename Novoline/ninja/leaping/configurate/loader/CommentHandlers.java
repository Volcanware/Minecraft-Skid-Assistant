...
public enum CommentHandlers implements CommentHandler {
    ...
    private static final class AbstractDelineatedHandler implements CommentHandler {
        ...
        public Collection<String> toComment(@NonNull Collection<String> lines) {
            if (lines.size() == 1) {
                return lines.stream().map(i -> startSequence + " " + i + " " + endSequence).collect(Collectors.toCollection(ObjectArrayList::new));
            } else {
                Collection<String> ret = new ArrayList<>();
                ret.add(startSequence);
                ret.addAll(lines.stream().map(i -> " " + lineIndentSequence + " " + i).collect(Collectors.toCollection(ObjectArrayList::new)));
                ret.add(" " + endSequence);
                return ret;
            }
        }
