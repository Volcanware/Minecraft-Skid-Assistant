...
public class ArmorStandReplacement implements EntityReplacement {
	...
	public void setYawPitch(float yaw, float pitch) {
		if(this.yaw != yaw && this.pitch != pitch || headYaw != yaw) {
			this.yaw = yaw;
			this.headYaw = yaw;
			this.pitch = pitch;
			updateLocation();
	...
	public void updateLocation() {
		...
		if(currentState == State.ZOMBIE) {
			...
			teleportSkull.write(Type.INT,
					(int) ((locY + (marker ? 54.85 : small ? 56 : 57)) * 32.0));  //Don't ask me where this offset is coming from
	...
	public void spawn() {
		...
		if(currentState == State.ZOMBIE) {
			...
			int[] entityIds = { entityId, ENTITY_ID-- };
	...
	public AABB getBoundingBox() {
		double w = small ? 0.25 : 0.5;
		double h = small ? 0.9875 : 1.975;
		Vector3d min = new Vector3d(locX - w / 2, locY, locZ - w / 2);
		Vector3d max = new Vector3d(locX + w / 2, locY + h, locZ + w / 2);
