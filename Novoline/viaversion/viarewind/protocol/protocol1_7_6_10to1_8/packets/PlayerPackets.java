...
import viaversion.viarewind.protocol.protocol1_7_6_10to1_8.storage.*;
	...
	public static void register(Protocol protocol) {
					...
					for(int i = 0; i < count; i++) {
						...
						if(action == 0) {
							...
							while(propertyCount-- > 0) {
								gameProfile.properties.add(new GameProfileStorage.Property(packetWrapper.read(Type.STRING),
							...
							String displayName =
									packetWrapper.read(Type.BOOLEAN) ? ChatUtil.jsonToLegacy(packetWrapper.read(Type.STRING)) : null;

							GameProfileStorage.GameProfile gameProfile = gameProfileStorage.get(uuid);
							if(gameProfile == null || gameProfile.displayName == null && displayName == null) continue;

							if(gameProfile.displayName == null && displayName != null || gameProfile.displayName != null && displayName == null || !gameProfile.displayName
									.equals(displayName)) {
				...
				handler(packetWrapper -> {
					...
					switch(action) {
						...
						case 1:
							...
							break;
						case 2:
							titleRenderProvider.setTimings(uuid, packetWrapper.read(Type.INT), packetWrapper.read(Type.INT),
		...
		protocol.registerIncoming(State.PLAY, 0x04, 0x04, new PacketRemapper() {
			...
			public void registerMap() {
				map(Type.DOUBLE);  //X
				map(Type.DOUBLE);  //Y
				handler(packetWrapper -> packetWrapper.read(Type.DOUBLE));
				map(Type.DOUBLE);  //Z
			...
			public void registerMap() {
				...
				handler(packetWrapper -> {
					...
					if(action == 3 || action == 4) {
						...
						abilitiesPacket
								.write(Type.FLOAT, abilities.isSprinting() ? abilities.getFlySpeed() * 2.0f : abilities.getFlySpeed());
		...
		protocol.registerIncoming(State.PLAY, 0x14, 0x14, new PacketRemapper() {
			...
			public void registerMap() {
				map(Type.STRING);
				create(packetWrapper -> packetWrapper.write(Type.OPTIONAL_POSITION, null));
				handler(packetWrapper -> {
					String msg = packetWrapper.get(Type.STRING, 0);
