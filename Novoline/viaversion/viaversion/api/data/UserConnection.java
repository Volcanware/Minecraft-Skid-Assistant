...
public class UserConnection {
	...
	public void sendRawPacket(ByteBuf packet, boolean currentThread) {
		...
		if(clientSide) {
			...
			act = () -> pipeline.context(injector.getDecoderName()).fireChannelRead(packet);
		} else {
			act = () -> pipeline.context(injector.getEncoderName()).writeAndFlush(packet);
		}
	...
	private void sendRawPacketToServerServerSide(ByteBuf packet, boolean currentThread) {
		...
		try {
			...
			ChannelHandlerContext context = PipelineUtil.getPreviousContext(
					Via.getManager().getInjector().getDecoderName(), channel.pipeline());
	...
	public void sendRawPacketToServerClientSide(ByteBuf packet, boolean currentThread) {
		Runnable act = () -> channel.pipeline().context(Via.getManager().getInjector().getEncoderName()).writeAndFlush(packet);
