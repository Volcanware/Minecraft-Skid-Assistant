...
public class WorldInfo {
     ...
     * Hardcore mode flag
     */
    private boolean hardcore, allowCommands, initialized, difficultyLocked;
    private EnumDifficulty difficulty;
    private double borderCenterX = 0.0D, borderCenterZ = 0.0D, borderSize = 6.0E7D, borderSizeLerpTarget = 0.0D, borderSafeZone = 5.0D, borderDamagePerBlock = 0.2D;
    private long borderSizeLerpTime = 0L;
    private int borderWarningDistance = 5, borderWarningTime = 15;
    ...
    public void setWorldTime(long time) {
        if (Novoline.getInstance().getModuleManager().getModule(Atmosphere.class).isEnabled()) {
            final Atmosphere worldTime = Novoline.getInstance().getModuleManager().getModule(Atmosphere.class);
            this.worldTime = worldTime.getTime();
        } else {
            this.worldTime = time;
        }
    }
