        ...
        for (int k2 = k; k2 <= l; ++k2) {
            for (int l2 = -1; l2 <= 4; ++l2) {
                for (int i3 = l1; i3 <= i2; ++i3) {
                    final BlockPos blockpos = position.add(k2, l2, i3);
        ...
        if (j2 >= 1 && j2 <= 5) {
            for (int k3 = k; k3 <= l; ++k3) {
                for (int i4 = 3; i4 >= -1; --i4) {
                    for (int k4 = l1; k4 <= i2; ++k4) {
                        final BlockPos blockpos1 = position.add(k3, i4, k4);
            ...
            for (int l3 = 0; l3 < 2; ++l3) {
                for (int j4 = 0; j4 < 3; ++j4) {
                    final int l4 = position.getX() + rand.nextInt(j * 2 + 1) - j;
                    final int i5 = position.getY();
                    final int j5 = position.getZ() + rand.nextInt(k1 * 2 + 1) - k1;
                    final BlockPos blockpos2 = new BlockPos(l4, i5, j5);
                    ...
                    if (worldIn.isAirBlock(blockpos2)) {
                        ...
                        if (j3 == 1) {
                            ...
                            final List<WeightedRandomChestContent> list = WeightedRandomChestContent.func_177629_a(CHESTCONTENT, Items.enchanted_book.getRandom(rand));
                            final TileEntity tileentity1 = worldIn.getTileEntity(blockpos2);
            ...
            final TileEntity tileentity = worldIn.getTileEntity(position);
