            ...
            if ((this.ticksExisted + this.getEntityID()) % 1200 == 0) {
                ...
                for (EntityPlayerMP entityplayermp : this.worldObj.getPlayers(EntityPlayerMP.class, p_apply_1_ -> EntityGuardian.this.getDistanceSqToEntity(p_apply_1_) < 2500.0D && p_apply_1_.theItemInWorldManager.survivalOrAdventure())) {
                    if (!entityplayermp.isPotionActive(potion) || entityplayermp.getActivePotionEffect(potion).getAmplifier() < 2 || entityplayermp.getActivePotionEffect(potion).getDuration() < 1200) {
                        ...
                        entityplayermp.addPotionEffect(new PotionEffect(potion.getId(), 6000, 2));
                    }
                }
            }
    ...
    static class GuardianMoveHelper extends EntityMoveHelper {
        ...
        public void onUpdateMoveHelper() {
            if (this.update && !this.entityGuardian.getNavigator().noPath()) {
                ...
                double d4 = MathHelper.sin((double) (this.entityGuardian.ticksExisted + this.entityGuardian.getEntityID()) * 0.5D) * 0.05D;
                double d5 = MathHelper.cos((double) (this.entityGuardian.rotationYaw * (float) Math.PI / 180.0F));
                double d6 = MathHelper.sin((double) (this.entityGuardian.rotationYaw * (float) Math.PI / 180.0F));
                this.entityGuardian.motionX += d4 * d5;
                this.entityGuardian.motionZ += d4 * d6;
                d4 = MathHelper.sin((double) (this.entityGuardian.ticksExisted + this.entityGuardian.getEntityID()) * 0.75D) * 0.05D;
