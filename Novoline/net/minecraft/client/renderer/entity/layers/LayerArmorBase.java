...
public abstract class LayerArmorBase<T extends ModelBase> implements LayerRenderer<EntityLivingBase> {
        ...
        if (!Config.isCustomItems() || CustomItems.isUseGlint()) {
            if (!Config.isShaders() || !Shaders.isShadowPass) {
                ...
                for (int i = 0; i < 2; ++i) {
                    ...
                    float f2 = 0.76F;
                    float r = 0.5F * f2, g = 0.25F * f2, b = 0.8F * f2;
                    setupColor(r, g, b);
    ...
    private void setupColor(float r, float g, float b) {
        if (Novoline.getInstance().getModuleManager().getModule(GlintColorize.class).isEnabled()) {
            ...
            ColorProperty colorSet = glintColorize.getColor();
            if (glintColorize.getRainbow().get()) {
                float hue = glintColorize.getOffset() / 255.0F;
                float[] hsb = colorSet.getHSB(), rgba = RenderUtils.getRGBAs(Color.getHSBColor(hue, hsb[1], hsb[2]).getRGB());
                GlStateManager.color(rgba[0], rgba[1], rgba[2], 1.0F);
            } else {
                Color color = colorSet.getAwtColor();
