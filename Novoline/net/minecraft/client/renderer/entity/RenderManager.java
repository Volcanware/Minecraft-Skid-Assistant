...
public class RenderManager {
    ...
    public Render<?> getEntityRenderObject(Entity entityIn) {
        ...
        if (entityIn instanceof AbstractClientPlayer) {
            ...
            RenderPlayer renderPlayer = (RenderPlayer) this.skinMap.get(skinType);
    ...
    public void cacheActiveRenderInfo(World world, FontRenderer textRenderer, Entity livingPlayer, Entity pointedEntity, GameSettings options, float partialTicks) {
        this.worldObj = world;
        ...
        if (livingPlayer instanceof EntityLivingBase && ((EntityLivingBase) livingPlayer).isPlayerSleeping()) {
            IBlockState iblockstate = world.getBlockState(new BlockPos(livingPlayer));
            Block block = iblockstate.getBlock();

            if (Reflector.callBoolean(Reflector.ForgeBlock_isBed, world, new BlockPos(livingPlayer), livingPlayer)) {
                final EnumFacing enumfacing = (EnumFacing) Reflector.call(block, Reflector.ForgeBlock_getBedDirection, new Object[]{world, new BlockPos(livingPlayer)});
            ...
            this.playerViewY = livingPlayer.prevRotationYaw + (livingPlayer.rotationYaw - livingPlayer.prevRotationYaw) * partialTicks;
            this.playerViewX = livingPlayer.prevRotationPitch + (livingPlayer.rotationPitch - livingPlayer.prevRotationPitch) * partialTicks;
        }
        ...
        this.viewerPosX = livingPlayer.lastTickPosX + (livingPlayer.posX - livingPlayer.lastTickPosX) * (double) partialTicks;
        this.viewerPosY = livingPlayer.lastTickPosY + (livingPlayer.posY - livingPlayer.lastTickPosY) * (double) partialTicks;
        this.viewerPosZ = livingPlayer.lastTickPosZ + (livingPlayer.posZ - livingPlayer.lastTickPosZ) * (double) partialTicks;
    }
    ...
    public boolean doRenderEntity(Entity entity, double x, double y, double z, float entityYaw, float partialTicks, boolean p_147939_10_) {
        ...
        try {
            ...
            CrashReport crashreport = CrashReport.makeCrashReport(t, "Rendering entity in world");
