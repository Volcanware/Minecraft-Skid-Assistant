...
public class TwitchStream implements BroadcastController.BroadcastListener, ChatController.ChatListener, IngestServerTester.IngestTestListener, IStream {
            ...
            final Thread thread = new Thread("Twitch authenticator") {
                ...
                public void run() {
                    try {
                        ...
                        final JsonObject responseObject = JsonUtils.getJsonObject(new JsonParser().parse(s), "Response");
                        final JsonObject tokenObject = JsonUtils.getJsonObject(responseObject, "token");

                        if (JsonUtils.getBoolean(tokenObject, "valid")) {
                            final String s1 = JsonUtils.getString(tokenObject, "user_name");
    ...
    public void func_152935_j() {
        ...
        final boolean flag1 = this.chatController.func_153000_j() == ChatController.ChatState.Initialized && (this.field_176029_e == null || this.chatController.func_175989_e(this.field_176029_e) == ChatController.EnumChannelState.Disconnected);
    ...
    protected void func_152942_I() {
        final ChatController.ChatState chatState = this.chatController.func_153000_j();
        ...
        if (chatState != ChatController.ChatState.Initialized) {
            LOGGER.warn("Invalid twitch chat state {}", chatState);
    ...
    public void func_152922_k() {
        if (this.broadcastController.isBroadcasting() && !this.broadcastController.isBroadcastPaused()) {
            ...
            if (flag) {
                ...
                final Framebuffer framebuffer1 = this.mc.getFramebuffer();
                ...
                final float f1 = (float) this.framebuffer.framebufferHeight;
                final float f2 = (float) framebuffer1.framebufferWidth / (float) framebuffer1.framebufferTextureWidth;
                final float f3 = (float) framebuffer1.framebufferHeight / (float) framebuffer1.framebufferTextureHeight;
    ...
    public void func_176026_a(Metadata p_176026_1_, long p_176026_2_, long p_176026_4_) {
        if (this.isBroadcasting() && this.field_152957_i) {
            ...
            final String s1 = p_176026_1_.func_152806_b();
            final long j = this.broadcastController.func_177946_b(p_176026_1_.func_152810_c(), i + p_176026_2_, s, s1);
    ...
    public void func_152930_t() {
        ...
        final VideoParams videoparams = this.broadcastController.func_152834_a(formatStreamKbps(gamesettings.streamKbps), formatStreamFps(gamesettings.streamFps), formatStreamBps(gamesettings.streamBytesPerPixel), (float) this.mc.displayWidth / (float) this.mc.displayHeight);
        ...
        if (gamesettings.streamPreferredServer != null && !gamesettings.streamPreferredServer.isEmpty()) {
    ...
    public void func_152892_c(ErrorCode p_152892_1_) {
        if (p_152892_1_ == ErrorCode.TTV_EC_SOUNDFLOWER_NOT_INSTALLED) {
            final IChatComponent chatComponent0 = new ChatComponentTranslation("stream.unavailable.soundflower.chat.link");
            final ChatStyle chatStyle = chatComponent0.getChatStyle();
            chatStyle.setChatClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://help.mojang.com/customer/portal/articles/1374877-configuring-soundflower-for-streaming-on-apple-computers"));
            chatStyle.setUnderlined(Boolean.TRUE);

            final IChatComponent chatComponent1 = new ChatComponentTranslation("stream.unavailable.soundflower.chat", chatComponent0);
            chatComponent1.getChatStyle().setColor(EnumChatFormatting.DARK_RED);

            this.mc.ingameGUI.getChatGUI().printChatMessage(chatComponent1);
        } else {
            final IChatComponent chatComponent = new ChatComponentTranslation("stream.unavailable.unknown.chat", ErrorCode.getString(p_152892_1_));
    ...
    public void func_180605_a(String p_180605_1_, ChatRawMessage[] p_180605_2_) {
        for (ChatRawMessage chatrawmessage : p_180605_2_) {
            ...
            if (this.func_176028_a(chatrawmessage.modes, chatrawmessage.subscriptions, this.mc.gameSettings.streamChatUserFilter)) {
                final IChatComponent chatComponent0 = new ChatComponentText(chatrawmessage.userName);
                final IChatComponent chatComponent1 = new ChatComponentTranslation("chat.stream." + (chatrawmessage.action ? "emote" : "text"), this.twitchComponent, chatComponent0, EnumChatFormatting.getTextWithoutFormattingCodes(chatrawmessage.message));

                if (chatrawmessage.action) {
                    chatComponent1.getChatStyle().setItalic(Boolean.TRUE);
                }

                final IChatComponent chatComponent3 = new ChatComponentText("");
                chatComponent3.appendSibling(new ChatComponentTranslation("stream.userinfo.chatTooltip"));

                for (IChatComponent chatComponent : GuiTwitchUserMode.func_152328_a(chatrawmessage.modes, chatrawmessage.subscriptions, null)) {
                    chatComponent3.appendText("\n");
                    chatComponent3.appendSibling(chatComponent);
                }

                final ChatStyle chatStyle = chatComponent0.getChatStyle();
                chatStyle.setChatHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, chatComponent3));
                chatStyle.setChatClickEvent(new ClickEvent(ClickEvent.Action.TWITCH_USER_INFO, chatrawmessage.userName));
    ...
    public void func_176018_a(String p_176018_1_, ChatUserInfo[] p_176018_2_, ChatUserInfo[] p_176018_3_, ChatUserInfo[] p_176018_4_) {
        for (ChatUserInfo chatUserInfo : p_176018_3_) this.field_152955_g.remove(chatUserInfo.displayName);
        for (ChatUserInfo chatUserInfo : p_176018_4_) this.field_152955_g.put(chatUserInfo.displayName, chatUserInfo);
        for (ChatUserInfo chatUserInfo : p_176018_2_) this.field_152955_g.put(chatUserInfo.displayName, chatUserInfo);
    }
