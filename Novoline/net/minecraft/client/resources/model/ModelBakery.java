...
public class ModelBakery {
            ...
            try {
                for (IResource iresource : this.resourceManager.getAllResources(resourcelocation)) {
                    ...
                    try {
                        ...
                        final ModelBlockDefinition modelBlockDefinition = ModelBlockDefinition.parseFromReader(new InputStreamReader(inputstream, Charsets.UTF_8));
                        list.add(modelBlockDefinition);
                    } catch (Exception exception) {
    ...
    private void bakeBlockModels() {
        ...
        for (Entry<String, ResourceLocation> entry : this.itemLocations.entrySet()) {
            ...
            final ModelResourceLocation modelResourceLocation = new ModelResourceLocation(entry.getKey(), "inventory");
    ...
    private Set<ResourceLocation> getVariantsTextureLocations() {
        ...
        for (ModelResourceLocation modelresourcelocation : list) {
            final ModelBlockDefinition.Variants variants = this.variants.get(modelresourcelocation);
    ...
    private IBakedModel bakeModel(ModelBlock modelBlockIn, ModelRotation modelRotationIn, boolean uvLocked) {
        ...
        final SimpleBakedModel.Builder builder = new SimpleBakedModel.Builder(modelBlockIn).setTexture(textureatlassprite);

        for (BlockPart blockpart : modelBlockIn.getElements()) {
            for (EnumFacing enumfacing : blockpart.mapFaces.keySet()) {
                final BlockPartFace blockpartface = blockpart.mapFaces.get(enumfacing);
                final TextureAtlasSprite textureAtlasSprite = this.sprites.get(new ResourceLocation(modelBlockIn.resolveTextureName(blockpartface.texture)));

                if (blockpartface.cullFace == null) {
                    builder.addGeneralQuad(this.makeBakedQuad(blockpart, blockpartface, textureAtlasSprite, enumfacing, modelRotationIn, uvLocked));
                } else {
                    builder.addFaceQuad(modelRotationIn.rotateFace(blockpartface.cullFace), this.makeBakedQuad(blockpart, blockpartface, textureAtlasSprite, enumfacing, modelRotationIn, uvLocked));
                }
            }
        }
    ...
    private BakedQuad makeBakedQuad(BlockPart part, BlockPartFace partFace, TextureAtlasSprite sprite, EnumFacing facing, ModelRotation rotation, boolean uvLocked) {
        return this.faceBakery.makeBakedQuad(part.positionFrom, part.positionTo, partFace, sprite, facing, rotation, part.partRotation, uvLocked, part.shade);
    }
    ...
    private void loadModels() {
        ...
        for (ResourceLocation resourceLocation : this.models.keySet()) {
            ...
            if (parentLocation != null) {
                deque.add(parentLocation);
            }
        }

        while (!deque.isEmpty()) {
            final ResourceLocation resourceLocation = deque.pop();
            ...
            try {
                ...
                final ModelBlock modelblock = this.loadModel(resourceLocation);
                this.models.put(resourceLocation, modelblock);
                final ResourceLocation parentLocation = modelblock.getParentLocation();

                if (parentLocation != null && !set.contains(parentLocation)) {
                    deque.add(parentLocation);
                }
            } catch (Exception exception) {
                LOGGER.warn("In parent chain: " + JOINER.join(this.getParentPath(resourceLocation)) + "; unable to load model: '" + resourceLocation + "'", exception);
            }
    ...
    private Set<ResourceLocation> getTextureLocations(ModelBlock modelBlock) {
        ...
        for (BlockPart part : modelBlock.getElements()) {
            for (BlockPartFace blockpartface : part.mapFaces.values()) {
                set.add(new ResourceLocation(modelBlock.resolveTextureName(blockpartface.texture)));
            }
        }
    ...
    private void loadSprites() {
        ...
        final IIconCreator iiconcreator = iconRegistry -> {
            for (ResourceLocation resourcelocation : set) {
                ModelBakery.this.sprites.put(resourcelocation, iconRegistry.registerSprite(resourcelocation));
            }
        };
            ...
            if (modelblock != null) {
                ...
                if (this.hasItemModel(modelblock)) {
                    for (String s : ItemModelGenerator.LAYERS) {
                        final ResourceLocation resourceLocation = new ResourceLocation(modelblock.resolveTextureName(s));
    ...
    private boolean hasItemModel(ModelBlock modelBlock) {
        if (modelBlock == null) {
            ...
            final ModelBlock modelblock = modelBlock.getRootModel();
    ...
    private void bakeItemModels() {
        for (ResourceLocation resourcelocation : this.itemLocations.values()) {
            ...
            if (this.hasItemModel(modelblock)) {
                final ModelBlock modelBlock = this.makeItemModel(modelblock);
