...
public class BlockFire extends Block {
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !Blocks.fire.canCatchFire(worldIn, pos.down())) {
            final boolean flag = (i + j + k & 1) == 1;
            final boolean flag1 = (i / 2 + j / 2 + k / 2 & 1) == 1;
            int l = 0;
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (worldIn.getGameRules().getBoolean("doFireTick")) {
            ...
            if (!flag && worldIn.isRaining() && this.canDie(worldIn, pos)) {
                ...
                final boolean flag1 = worldIn.isBlockinHighHumidity(pos);
                int j = 0;
                    ...
                    for (int l = -1; l <= 1; ++l) {
                        for (int i1 = -1; i1 <= 4; ++i1) {
                            if (k != 0 || i1 != 0 || l != 0) {
                                ...
                                final int k1 = this.getNeighborEncouragement(worldIn, blockpos);
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !Blocks.fire.canCatchFire(worldIn, pos.down())) {
            if (Blocks.fire.canCatchFire(worldIn, pos.west())) {
                for (int j = 0; j < 2; ++j) {
                    final double d3 = (double) pos.getX() + rand.nextDouble() * 0.10000000149011612D;
                    final double d8 = (double) pos.getY() + rand.nextDouble();
                    final double d13 = (double) pos.getZ() + rand.nextDouble();
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.east())) {
                for (int k = 0; k < 2; ++k) {
                    final double d4 = (double) (pos.getX() + 1) - rand.nextDouble() * 0.10000000149011612D;
                    final double d9 = (double) pos.getY() + rand.nextDouble();
                    final double d14 = (double) pos.getZ() + rand.nextDouble();
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.north())) {
                for (int l = 0; l < 2; ++l) {
                    final double d5 = (double) pos.getX() + rand.nextDouble();
                    final double d10 = (double) pos.getY() + rand.nextDouble();
                    final double d15 = (double) pos.getZ() + rand.nextDouble() * 0.10000000149011612D;
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.south())) {
                for (int i1 = 0; i1 < 2; ++i1) {
                    final double d6 = (double) pos.getX() + rand.nextDouble();
                    final double d11 = (double) pos.getY() + rand.nextDouble();
                    final double d16 = (double) (pos.getZ() + 1) - rand.nextDouble() * 0.10000000149011612D;
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.up())) {
                for (int j1 = 0; j1 < 2; ++j1) {
                    final double d7 = (double) pos.getX() + rand.nextDouble();
                    final double d12 = (double) (pos.getY() + 1) - rand.nextDouble() * 0.10000000149011612D;
                    final double d17 = (double) pos.getZ() + rand.nextDouble();
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d7, d12, d17, 0.0D, 0.0D, 0.0D);
                }
            }
        } else {
            for (int i = 0; i < 3; ++i) {
                final double d0 = (double) pos.getX() + rand.nextDouble();
                final double d1 = (double) pos.getY() + rand.nextDouble() * 0.5D + 0.5D;
                final double d2 = (double) pos.getZ() + rand.nextDouble();
