...
public abstract class MinecraftServer implements Runnable, ICommandSender, IThreadListener, IPlayerUsage {
    ...
    public List<String> getTabCompletions(ICommandSender sender, String input, BlockPos pos) {
        ...
        if (input.startsWith("/")) {
            complete(sender, input, pos, list, this.commandManager, "/");
        } else if (input.startsWith(".")) {
            complete(sender, input, pos, list, Novoline.getInstance().getNovoCommandHandler(), ".");
        } else {
    ...
    private void complete(ICommandSender sender, String input, BlockPos pos, List<String> list, ICommandManager novoCommandHandler, String s3) {
        input = input.substring(1);
        final boolean flag = !input.contains(" ");
        final List<String> list1 = novoCommandHandler.getTabCompletionOptions(sender, input, pos);
