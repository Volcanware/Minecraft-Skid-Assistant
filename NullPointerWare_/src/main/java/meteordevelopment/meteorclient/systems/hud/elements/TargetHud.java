...
public class TargetHud extends HudElement {
    ...
    public static final HudElementInfo<TargetHud> INFO = new HudElementInfo<>(Hud.GROUP, "TargetHud", "Displays information about your combat target.", TargetHud::new);
    ...
    private final Setting<Boolean> winning = sgGeneral.add(new BoolSetting.Builder()
        .name("Winning")
    ...
    private final Setting<winningMode> winningModeSetting = sgGeneral.add(new EnumSetting.Builder<winningMode>()
        .name("winnning-mode")
        .description("How to determine if you are winning.")
        .defaultValue(winningMode.Armor)
        .visible(winning::get)
        .build()
    );

    private final Setting<SettingColor> winningColor1 = sgGeneral.add(new ColorSetting.Builder()
        .name("Winning-color")
        .description("Color of the winning text when winning.")
        .defaultValue(new SettingColor(15, 255, 15))
        .visible(winning::get)
        .build()
    );

    private final Setting<SettingColor> losingColor = sgGeneral.add(new ColorSetting.Builder()
        .name("losing-color")
        .description("Color of winning text when losing.")
    ...
    private boolean getWinning() {

        if (mc.player.getMainHandStack() == null || playerEntity.getMainHandStack() == null || mc.player == null || playerEntity == null) {
            return true;
        }

        // Get own armor dura and enemy armor dura
        int ownHelmetDura = mc.player.getInventory().getArmorStack(3).getDamage();
        int enemyHelmetDura = playerEntity.getInventory().getArmorStack(3).getDamage();

        float ownHealth = mc.player.getHealth();
        float enemyHealth = playerEntity.getHealth();

        Item ownItem = mc.player.getMainHandStack().getItem();
        Item enemyItem = playerEntity.getMainHandStack().getItem();

        boolean ownHasStrength = mc.player.hasStatusEffect(StatusEffects.STRENGTH);
        boolean enemyHasStrength = playerEntity.hasStatusEffect(StatusEffects.STRENGTH);

        boolean strength = (ownHasStrength && !enemyHasStrength) || (ownHasStrength && enemyHasStrength);


        if (ownItem == null || enemyItem == null) {
            return true;
        }
        if (ownItem instanceof SwordItem item && (winningModeSetting.get().equals(winningMode.Sword) || winningModeSetting.get().equals(winningMode.All))) {
            if (!(enemyItem instanceof SwordItem )) {
                return true;
            }
            SwordItem enemySword = (SwordItem) enemyItem;
            return (item.getAttackDamage() > enemySword.getAttackDamage());
        }

        // Check winning mode
        if (winningModeSetting.get().equals(winningMode.Armor)) {
            return ownHelmetDura >= enemyHelmetDura;
        } else if (winningModeSetting.get().equals(winningMode.Health)) {
            return ownHealth >= enemyHealth;
        } else if (winningModeSetting.get().equals(winningMode.HealthAndArmor)) {
            // Check both helmet durability and health
        return (ownHelmetDura == enemyHelmetDura) ? (ownHealth >= enemyHealth) : (ownHelmetDura >= enemyHelmetDura);
        } else if (winningModeSetting.get().equals(winningMode.All)) {
            // Check all conditions (helmet durability, health, and sword damage and str)
            return (ownHelmetDura >= enemyHelmetDura) && (ownHealth >= enemyHealth) && (ownItem instanceof SwordItem) && strength;
        }
    ...
    public void render(final HudRenderer renderer) {
        renderer.post(() -> {
            ...
            double winningWidth = TextRenderer.get().getWidth(winningText);
            double distTextWidth = TextRenderer.get().getWidth(distText);
            if (!displayDistance.get()) distTextWidth = 0;
            ...
            if (winning.get()) {
                ...
                Color winColor = getWinning() ? winningColor1.get() : losingColor.get();

                TextRenderer.get().render(breakText, x + friendWidth + breakWidth + pingWidth, y, secondaryColor);
                // TextRenderer.get().render(nameText, x, y, nameColor != null ? nameColor : primaryColor);

                TextRenderer.get().render(winningText, x + friendWidth + breakWidth + pingWidth - 10 + breakWidth + winningWidth, y - 10 * scale.get(), winColor);
            }
