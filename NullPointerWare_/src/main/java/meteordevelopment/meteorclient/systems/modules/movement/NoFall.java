...
public final class NoFall extends Module {
    ...
    private final Setting<Boolean> packetpos = sgGeneral.add(new BoolSetting.Builder()
        .name("packet-position")
        .description("Sends position instead of onground.")
        .defaultValue(true)
        .visible(() -> mode.get() == Mode.VulcanPacket)
        .build()
    );
    ...
    private final Setting<Boolean> alwayspacket = sgGeneral.add(new BoolSetting.Builder()
        .name("always")
        .description("Always sends the ground packet.")
        .defaultValue(true)
        .visible(() -> mode.get() == Mode.VulcanPacket && !packetpos.get())
        .build()
    );

    private final Setting<Boolean> clientsidevulcan = sgGeneral.add(new BoolSetting.Builder()
        .name("client-side")
        .description("Makes the player on ground clientside.")
        .defaultValue(false)
        .visible(() -> mode.get() == Mode.VulcanPacket && !packetpos.get())
        .build()
    );
    ...
    private final Setting<Boolean> onground = sgGeneral.add(new BoolSetting.Builder()
        .name("on-ground")
        .description("What the onground state should be.")
        .defaultValue(true)
        .visible(() -> mode.get() == Mode.VulcanPacket && !packetpos.get())
        .build()
    );
    ...
    private void onSendPacket(final PacketEvent.Send event) {
        ...
        if (mode.get().equals(Mode.Cancel) && event.packet instanceof PlayerMoveC2SPacket movePacket) {
            if (mc.player.fallDistance > 5) {
                mc.player.fallDistance = 0;
                sendNoEvent(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), mc.player.getYaw(), mc.player.getPitch(), true));
            }
        }
    }
    ...
    private void onTick(final TickEvent.Pre event) {
        ...
        if (mode.get() == Mode.AirPlace) {
            ...
            Rotations.rotate(mc.player.getYaw(), 90, Integer.MAX_VALUE, () -> {
                ...
                BlockUtils.place(mc.player.getBlockPos().down(), InvUtils.findInHotbar(itemStack -> itemStack.getItem() instanceof BlockItem), BlockUtils.RotationPacket.None, 0, true);
        ...
        // Bucket mode
         ...
         if (mode.get() == Mode.VulcanPacket) {
             if (!packetpos.get()) {
                 if ((mc.player.fallDistance < 2 || mc.player.getVelocity().y > 0 || mc.player.isOnGround()) && !alwayspacket.get()) return;
                 sendNoEvent(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), mc.player.getYaw(), mc.player.getPitch(), onground.get()));
                 if (clientsidevulcan.get())
                 ...
                 if (mc.player != null && !mc.player.isOnGround() && mc.player.fallDistance > 2f) {
                     for (Hand hand : Hand.values()) {
                         ...
                         if (!stack.isEmpty()) {
                             ...
                             Vec3d originVec = mc.player.getPos();
    ...
    public boolean isVulcanMode() {
        return mode.get().equals(Mode.VulcanPacket);
    }
