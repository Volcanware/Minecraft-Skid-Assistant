...
public final class BedNuker extends Module {
    ...
    private final Setting<BedNuker.Mode> mode = sgGeneral.add(new EnumSetting.Builder<BedNuker.Mode>()
            .name("mode")
    ...
    private final Setting<Boolean> breakOwnTeamBed = sgGeneral.add(new BoolSetting.Builder()
        .name("Team-bed")
        .description("Makes the nuker not break own teams beds.")
        .defaultValue(false)
        .build()
    );
    ...
    private final Setting<BedNuker.SortMode> sortMode = sgGeneral.add(new EnumSetting.Builder<BedNuker.SortMode>()
            .name("sort-mode")
            .description("The blocks you want to mine first.")
            .defaultValue(BedNuker.SortMode.Closest)
            .build()
    );
    ...
    public BedNuker() {
        super(Categories.World, "BedNuker", "Breaks blocks around you.");
    }
    ...
    private void onTickPre(final TickEvent.Pre event) {
        ...
        BlockIterator.after(() -> {
            ...
            for (BlockPos block : blocks) {
                ...
                bpos = block;
                if (rotate.get() == RotationMode.Vanilla) Rotations.rotate(Rotations.getYaw(block), Rotations.getPitch(block), () -> breakBlock(block));
    ...
    private void breakBlock(final BlockPos blockPos) {
        ...
        if (BlockUtils.isBedOwn(blockPos, true) && breakOwnTeamBed.get())
            return;
