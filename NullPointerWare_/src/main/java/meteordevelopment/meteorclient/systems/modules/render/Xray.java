...
public final class Xray extends Module {
    ...
    @EventHandler
    private void onRenderBlockEntity(final RenderBlockEntityEvent event) {
        if (isBlocked(event.blockEntity.getCachedState().getBlock(), event.blockEntity.getPos())) event.cancel();
    }

    @EventHandler
    private void onChunkOcclusion(final ChunkOcclusionEvent event) {
        event.cancel();
    }

    @EventHandler
    private void onAmbientOcclusion(final AmbientOcclusionEvent event) {
        event.lightLevel = 1;
    }
