...
public final class AutoAnvil extends Module {
    ...
    private void onTick(final TickEvent.Pre event){
        ...
        if (placeButton.get()) {
            ...
            BlockUtils.place(target.getBlockPos(), floorBlock, rotate.get() ? BlockUtils.RotationPacket.Vanilla : BlockUtils.RotationPacket.None, 0, false);
        }
        ...
        if (timer >= delay.get()) {
            ...
            for (int i = height.get(); i > 1; i--) {
                ...
                if (BlockUtils.place(blockPos, anvil, rotate.get() ? BlockUtils.RotationPacket.Vanilla : BlockUtils.RotationPacket.None, 0) && !multiPlace.get()) break;
            }
        } else {
            timer++;
        }
