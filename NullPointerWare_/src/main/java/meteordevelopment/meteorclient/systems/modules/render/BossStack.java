...
public final class BossStack extends Module {
    ...
    @EventHandler
    private void onFetchText(final RenderBossBarEvent.BossText event) {
        if (hideName.get()) {
            event.name = Text.of("");
            return;
    ...
    @EventHandler
    private void onSpaceBars(final RenderBossBarEvent.BossSpacing event) {
        event.spacing = spacing.get().intValue();
    }

    @EventHandler
    private void onGetBars(final RenderBossBarEvent.BossIterator event) {
        if (stack.get()) {
