...
public final class Flight extends Module {
    ...
    private final Setting<Boolean> tickTimerBool= sgGeneral.add(new BoolSetting.Builder()
        .name("Fly Yick Delay")
        .description("Turns off the fly after a specific amount of ticks")
        .defaultValue(false)
        .build()
    );

    private final Setting<Boolean> noTickOnDisabler = sgGeneral.add(new BoolSetting.Builder()
        .name("No Tick on Disabler")
        .description("Dont count ticks while disabler is on")
        .defaultValue(false)
        .visible(tickTimerBool::get)
        .build()
    );

    private final Setting<Integer> tickTimer = sgGeneral.add(new IntSetting.Builder()
        .name("fly-delay-ticks")
        .description("The amount of delay, in ticks, that you can fly")
        .defaultValue(80)
        .min(1)
        .sliderMax(200)
        .visible(tickTimerBool::get)
    ...
    private void onPreTick(final TickEvent.Pre event) {
        ...
        if (tickTimerBool.get() && !(noTickOnDisabler.get() && Modules.get().isActive(Disabler.class)))
            ticks++;
