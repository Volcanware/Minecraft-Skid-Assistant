...
public final class PacketFly extends Module {
    public PacketFly() {
        super(Categories.Movement, "Packet Fly", "Flies with packets.");
    }
    ...
    private final Setting<Double> yBound = sgGeneral.add(new DoubleSetting.Builder()
        .name("Y Bound")
    ...
    private void onReceive(PacketEvent.Receive e) {
        if (e.packet instanceof PlayerPositionLookS2CPacket packet) {
            ...
            if (validPos.containsKey(packet.getTeleportId()) && validPos.get(packet.getTeleportId()).equals(vec)) {
                ...
                e.cancel();
                if (!predictID.get()) {
                    sendNoEvent(new TeleportConfirmC2SPacket(packet.getTeleportId()));
                }
    ...
    private boolean onGround() {
        return mc.player.isOnGround() || (mc.player.getBlockY() - mc.player.getY() == 0 && BlockUtils.collidable(mc.player.getBlockPos().down()));
    }
    ...
    private boolean isPhasing() {
        return BlockUtils.inside(mc.player, mc.player.getBoundingBox().shrink(0.0625, 0, 0.0625));
    }

    private boolean isSemiPhase() {
        return BlockUtils.inside(mc.player, mc.player.getBoundingBox().expand(0.01, 0, 0.01));
    }
    ...
    private void send(Vec3d pos, Vec3d bounds, boolean onGround) {
        ...
        id++;
        if (predictID.get()) {
            sendNoEvent(new TeleportConfirmC2SPacket(id));
        }
    }
