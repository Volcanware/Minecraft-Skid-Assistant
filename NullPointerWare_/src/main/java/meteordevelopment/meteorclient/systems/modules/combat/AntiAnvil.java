    ...
    private void onTick(final TickEvent.Pre event){
        for (int i = 0; i <= mc.interactionManager.getReachDistance(); i++) {
            ...
            if (mc.world.getBlockState(pos).getBlock() == Blocks.ANVIL && mc.world.getBlockState(pos.down()).isAir()) {
                if (BlockUtils.place(pos.down(), InvUtils.findInHotbar(Items.OBSIDIAN), rotate.get() ? BlockUtils.RotationPacket.Vanilla : BlockUtils.RotationPacket.None, 15, swing.get(), true))
                    break;
            }
        }
    }
