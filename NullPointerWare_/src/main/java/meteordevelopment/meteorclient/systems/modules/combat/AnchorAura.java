...
public final class AnchorAura extends Module {
    ...
    private void onTick(final TickEvent.Post event){
        ...
        if (placeDelayLeft >= placeDelay.get() && place.get()) {
            ...
            if (placePos != null) {
                placeDelayLeft = 0;
                BlockUtils.place(placePos, anchor, (rotationMode.get() == RotationMode.Place || rotationMode.get() == RotationMode.Both) ? BlockUtils.RotationPacket.Vanilla : BlockUtils.RotationPacket.None, 50);
            }
        }
