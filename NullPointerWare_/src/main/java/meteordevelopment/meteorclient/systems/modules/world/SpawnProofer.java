...
public final class SpawnProofer extends Module {
    ...
    private void onTickPost(TickEvent.Post event) {
        ...
        // Place blocks
        if (delay.get() == 0) {
            for (BlockPos blockPos : spawns) BlockUtils.place(blockPos, block, rotate.get() ? BlockUtils.RotationPacket.Vanilla : BlockUtils.RotationPacket.None, -50, false);
        }
        else {
            ...
            if (isLightSource(Block.getBlockFromItem(mc.player.getInventory().getStack(block.slot()).getItem()))) {
                ...
                BlockUtils.place(selectedBlockPos, block, rotate.get() ? BlockUtils.RotationPacket.Vanilla : BlockUtils.RotationPacket.None, -50, false);
            }
            else {
                BlockUtils.place(spawns.get(0), block, rotate.get() ? BlockUtils.RotationPacket.Vanilla : BlockUtils.RotationPacket.None, -50, false);
            }
        }
