...
import meteordevelopment.meteorclient.events.entity.player.UpdateEvent;
...
public final class NoSlow extends Module {
    ...
    private final Setting<ConsumableMode> mode = sgGeneral.add(new EnumSetting.Builder<ConsumableMode>()
        .name("items")
        .description("Whether or not using items will slow you.")
        .defaultValue(ConsumableMode.None)
        .build()
    );
    ...
    public boolean grimCobweb() { return isActive() && web.get() == WebMode.Grim; }
    ...
    private void onUpdate(final UpdateEvent event) {
        if (mode.get() == ConsumableMode.Grim && mc.player.isUsingItem()) {
            if (mc.player.getActiveHand() == Hand.OFF_HAND) {
                ...
                PendingUpdateManager pum = ((ClientWorldAccessor) mc.world).acquirePendingUpdateManager();
                mc.player.networkHandler.sendPacket(new PlayerInteractItemC2SPacket(Hand.OFF_HAND, pum.getSequence()));
                pum.close();
            }
        }
        if (mode.get() == ConsumableMode.OldGrim && mc.player.isUsingItem()) {
            mc.player.networkHandler.sendPacket(new UpdateSelectedSlotC2SPacket(mc.player.getInventory().selectedSlot % 8 + 1));
            mc.player.networkHandler.sendPacket(new UpdateSelectedSlotC2SPacket(mc.player.getInventory().selectedSlot));
        }
    }
