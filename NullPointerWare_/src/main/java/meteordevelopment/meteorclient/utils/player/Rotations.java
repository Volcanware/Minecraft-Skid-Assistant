...
public final class Rotations {
    ...
    private static class Rotation {
        ...
        public void sendPacket() {
            Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), (float) yaw, (float) pitch, mc.player.isOnGround()));
            runCallback();
        }
    ...
    public static float[] lookAtEntity(Entity targetEntity) {
        ...
        float yaw = (float) (((Math.atan2(diffZ, diffX) * 180.0 / Math.PI) - 90.0f)) + Rotations.random(-2, 2);
        float pitch = (float) (-(Math.atan2(diffY, dist) * 180.0 / Math.PI)) + Rotations.random(-2, 2);
