...
public final class EChestMemory {
    ...
    @EventHandler
    private static void onBlockActivate(final BlockActivateEvent event) {
        if (event.blockState.getBlock() instanceof EnderChestBlock && echestOpenedState == 0) echestOpenedState = 1;
    }

    @EventHandler
    private static void onOpenScreenEvent(final OpenScreenEvent event) {
    ...
    @EventHandler
    private static void onLeaveEvent(final GameLeftEvent event) {
        ITEMS.clear();
        isKnown = false;
    }
