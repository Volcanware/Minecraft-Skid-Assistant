...
public final class BlockUtils {
    ...
    public static boolean place(BlockPos blockPos, FindItemResult findItemResult, RotationPacket rotationPacket, int rotationPriority) {
        return place(blockPos, findItemResult, rotationPacket, rotationPriority, true);
    }

    public static boolean place(BlockPos blockPos, FindItemResult findItemResult, RotationPacket rotationPacket, int rotationPriority, boolean checkEntities) {
        return place(blockPos, findItemResult, rotationPacket, rotationPriority, true, checkEntities);
    }

    public static boolean place(BlockPos blockPos, FindItemResult findItemResult, int rotationPriority, boolean checkEntities) {
        return place(blockPos, findItemResult, RotationPacket.Vanilla, rotationPriority, true, checkEntities);
    }

    public static boolean place(BlockPos blockPos, FindItemResult findItemResult, RotationPacket rotationPacket, int rotationPriority, boolean swingHand, boolean checkEntities) {
        return place(blockPos, findItemResult, rotationPacket, rotationPriority, swingHand, checkEntities, true);
    }

    public static boolean place(BlockPos blockPos, FindItemResult findItemResult, RotationPacket rotationPacket, int rotationPriority, boolean swingHand, boolean checkEntities, boolean swapBack) {
        if (findItemResult.isOffhand()) {
            return place(blockPos, Hand.OFF_HAND, mc.player.getInventory().selectedSlot, rotationPacket, rotationPriority, swingHand, checkEntities, swapBack);
        } else if (findItemResult.isHotbar()) {
            return place(blockPos, Hand.MAIN_HAND, findItemResult.slot(), rotationPacket, rotationPriority, swingHand, checkEntities, swapBack);
        }
        return false;
    }

    public static boolean place(BlockPos blockPos, Hand hand, int slot, RotationPacket rotationPacket, int rotationPriority, boolean swingHand, boolean checkEntities, boolean swapBack) {
        ...
        switch (rotationPacket) {
            ...
            case Packet -> {
                ...
                mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.LookAndOnGround((float) Rotations.getYaw(hitPos), (float) Rotations.getPitch(hitPos), mc.player.isOnGround()));
            ...
            case FullPacket -> {
                ...
                mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), (float) Rotations.getYaw(hitPos), (float) Rotations.getPitch(hitPos), mc.player.isOnGround()));

                interact(bhr, hand, swingHand);

                // Grim bypass, isnt required.
    ...
    public static boolean isBedOwn(final BlockPos blockPos, boolean debug) {
        ...
        if (debug) {
            System.out.println("Bed color is " + bedcolor + "\n Team Color is " + teamColor);
        }
    ...
    public static boolean isVoidBelow(final BlockPos pos) {
        int air = 0;
    ...
    public static boolean isVoidBelow2(final BlockPos blockPos, final boolean isOneNineteen) {
        int startY = blockPos.getY(); // Starting Y value
        int endY; // = Math.max(startY - 60, 0); // Ending Y value, minimum 0
        if(isOneNineteen)
            endY = -60;
        else endY=0;

        // Iterate over the Y values from the starting position to the ending position
        for (int y = startY; y >= endY; y--) {
            BlockPos currentPos = new BlockPos(blockPos.getX(), y, blockPos.getZ());
            if (!isAir(currentPos)) {
                // If it's not air, return false
                return false;
            }
        }
    ...
    public static double getDistanceToVoid() {
        double distance = 0.0D;

        double i;

        for (i = mc.player.getY(); i > 0.0D; --i) {
            BlockPos blockPos = new BlockPos((int) mc.player.getX(), MathHelper.floor(i), (int) mc.player.getZ());
            Block block = mc.world.getBlockState(blockPos).getBlock();

            if (!isAir(blockPos) && block.isShapeFullCube(block.getDefaultState(), mc.world, blockPos) && block.collidable) {
             distance = i;
            }
        ...
        i = mc.player.getY() - distance;
        return i;
    }


    public static int getBlocksTillground(BlockPos pos) {
        int air = 0;
