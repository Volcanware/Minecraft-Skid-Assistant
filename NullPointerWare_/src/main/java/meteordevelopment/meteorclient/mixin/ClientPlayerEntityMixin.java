...
public abstract class ClientPlayerEntityMixin extends AbstractClientPlayerEntity {
    ...
    public void swingHand(Hand hand, CallbackInfo ci) {
        MeteorClient.EVENT_BUS.post(HandSwingEvent.get(hand));
        if (MeteorClient.EVENT_BUS.post(HandSwingEvent.get(hand).isCancelled())) ci.cancel();
    }
    ...
    private void onIsSneaking(CallbackInfoReturnable<Boolean> info) {
        if (Modules.get().get(Scaffold.class).scaffolding() && Modules.get().get(Scaffold.class).down.get()) info.setReturnValue(false);
    }
    ...
    @Inject(at = @At(value = "INVOKE", target = "Lnet/minecraft/client/network/AbstractClientPlayerEntity;tick()V", ordinal = 0), method = "tick()V", cancellable = true)
    private void onUpdate(CallbackInfo ci) {
        if (MeteorClient.EVENT_BUS.post(UpdateEvent.get()).isCancelled())
            ci.cancel();
    }
