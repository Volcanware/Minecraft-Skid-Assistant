...
import tech.dort.dortware.impl.utils.combat.rotation.AngleUtility;
...
public class KillAura extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        try {
            ...
            if (entities.isEmpty()) {
                index = 0;
                if (blocking && blockMode.getValue() != BlockMode.VANILLA && blockMode.getValue() != BlockMode.AGC)
                    unblock();
            } else {
                ...
                if (canBlock && blockMode.getValue() != BlockMode.FAKE) {
                    if (event.isPre() && blockMode.getValue() != BlockMode.VANILLA && blockMode.getValue() != BlockMode.AGC) {
                        unblock();
                    } else if (!event.isPre()) {
    ...
    private void rotateToEntity(EntityLivingBase entity, RotationMode type, UpdateEvent event) {
        ...
        switch (type) {
            ...
            case DOWN:
                ...
                break;
            case NON_SILENT:
                mc.thePlayer.rotationYaw = temp.getRotationYaw();
                mc.thePlayer.rotationPitch = temp.getRotationPitch();
                break;
            case GWEN:
                temp = Rotation.fromArray(AngleUtility.getRotations(entity));
                event.setRotationYaw(lastRotation.getRotationYaw());
                event.setRotationPitch(lastRotation.getRotationPitch());
                break;
            case AAC:
