...
public class SettingArgumentType implements ArgumentType<String> {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        Stream<String> stream = Streams.stream(context.getArgument("module", Module.class).settings.iterator()).flatMap(settings -> Streams.stream(settings.iterator())).map(setting -> setting.name);
