...
import xyz.mathax.mathaxclient.events.render.RenderItemEntityEvent;
import xyz.mathax.mathaxclient.mixininterface.IItemEntity;
...
public class ItemPhysics extends Module {
    public ItemPhysics(Category category) {
        super(category, "Item Physics", "Applies physics to items on the ground.");
    }

    @EventHandler
    private void onRenderItemEntity(RenderItemEntityEvent event) {
        ItemStack itemStack = event.itemEntity.getStack();
        event.random.setSeed(itemStack.isEmpty() ? 187 : Item.getRawId(itemStack.getItem()) + itemStack.getDamage());
