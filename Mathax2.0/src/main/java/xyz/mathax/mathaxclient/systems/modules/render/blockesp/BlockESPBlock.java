...
public class BlockESPBlock {
    ...
    public void update() {
        ...
        neighbours = 0;
        if (isNeighbour(Direction.SOUTH)) {
            neighbours |= FO;
        }

        if (isNeighbourDiagonal(1, 0, 1)) {
            neighbours |= FO_RI;
        }

        if (isNeighbour(Direction.EAST)) {
            neighbours |= RI;
        }
        ...
        if (isNeighbour(Direction.NORTH)) {
            neighbours |= BA;
        }
        ...
        if (isNeighbour(Direction.UP)) {
            neighbours |= TO;
        }

        if (isNeighbourDiagonal(0, 1, 1)) {
            neighbours |= TO_FO;
        }
    ...
    private boolean isNeighbour(Direction direction) {
        blockPos.set(x + direction.getOffsetX(), y + direction.getOffsetY(), z + direction.getOffsetZ());
