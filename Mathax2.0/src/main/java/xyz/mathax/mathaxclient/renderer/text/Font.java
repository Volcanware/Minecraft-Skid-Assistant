...
public class Font {
    ...
    public Font(ByteBuffer buffer, int height) {
        ...
        for (int i = 0; i < charData[1].length; i++) {
            ...
            float ipw = 1f / 2048;
            float iph = 1f / 2048;
            charData[1][i] = new CharData(packedChar.xoff(), packedChar.yoff(), packedChar.xoff2(), packedChar.yoff2(), packedChar.x0() * ipw, packedChar.y0() * iph, packedChar.x1() * ipw, packedChar.y1() * iph, packedChar.xadvance());
        }
    }
    ...
    public double getWidth(String string, int length) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (cp >= 32 && cp <= 127) {
                c = charData[0][cp - 32];
            } else if (cp >= 1024 && cp <= 1279) {
                c = charData[1][cp - 1024]; // cp - 1024 (rus lang)
            }
    ...
    public double render(Mesh mesh, String string, double x, double y, Color color, double scale) {
        ...
        for (int i = 0; i < string.length(); i++) {
            ...
            if (cp >= 32 && cp <= 127) {
                ...
                c = charData[1][cp - 1024];
            }

            int i1 = mesh.vec2(x + c.x0 * scale, y + c.y0 * scale).vec2(c.u0, c.v0).color(color).next();
            int i2 = mesh.vec2(x + c.x0 * scale, y + c.y1 * scale).vec2(c.u0, c.v1).color(color).next();
            int i3 = mesh.vec2(x + c.x1 * scale, y + c.y1 * scale).vec2(c.u1, c.v1).color(color).next();
            int i4 = mesh.vec2(x + c.x1 * scale, y + c.y0 * scale).vec2(c.u1, c.v0).color(color).next();
