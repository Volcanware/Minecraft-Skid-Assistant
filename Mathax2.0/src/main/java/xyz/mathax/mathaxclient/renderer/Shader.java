...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            String error = "Failed to compile vertex shader (" + vertPath + "): " + vertError;
            MatHax.LOG.error(error);
        ...
        if (fragError != null) {
            String error = "Failed to compile fragment shader (" + fragPath + "): " + fragError;
            MatHax.LOG.error(error);
            throw new RuntimeException(error);
        }
        ...
        if (programError != null) {
            String error = "Failed to link program: " + programError;
            MatHax.LOG.error(error);
            throw new RuntimeException(error);
        }
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(MatHax.mc.getResourceManager().getResource(new MatHaxIdentifier("shaders/" + path)).get().getInputStream(), StandardCharsets.UTF_8);
