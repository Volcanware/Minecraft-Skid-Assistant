...
import xyz.mathax.mathaxclient.mixin.BufferRendererAccessor;
import xyz.mathax.mathaxclient.mixininterface.ICapabilityTracker;
...
public class GL {
    ...
    public static void bindTexture(Identifier id) {
        ...
        MatHax.mc.getTextureManager().bindTexture(id);
    }
    ...
    private static ICapabilityTracker getTracker(String fieldName) {
        try {
            ...
            for (Field field1 : state.getClass().getDeclaredFields()) {
                if (field1.getType().getName().equals(trackerName)) {
                    capStateField = field1;
                    break;
                }
            }
