...
import xyz.mathax.mathaxclient.gui.renderer.packer.TextureRegion;
...
public class Renderer2D {
    ...
    public Renderer2D(boolean texture) {
        triangles = new ShaderMesh(texture ? Shaders.POS_TEXTURED_COLOR : Shaders.POS_COLOR, DrawMode.Triangles, texture ? new Mesh.Attrib[]{Mesh.Attrib.Vec2, Mesh.Attrib.Vec2, Mesh.Attrib.Color} : new Mesh.Attrib[]{Mesh.Attrib.Vec2, Mesh.Attrib.Color});
    ...
    public void quad(double x, double y, double width, double height, Color cTopLeft, Color cTopRight, Color cBottomRight, Color cBottomLeft) {
        int i1 = triangles.vec2(x, y).color(cTopLeft).next();
    ...
    public void texturedQuad(double x, double y, double width, double height, Color color) {
        int i1 = triangles.vec2(x, y).vec2(0, 0).color(color).next();
    ...
    public void texturedQuad(double x, double y, double width, double height, TextureRegion texture, Color color) {
        int i1 = triangles.vec2(x, y).vec2(texture.x1, texture.y1).color(color).next();
    ...
    public void texturedQuad(double x, double y, double width, double height, double rotation, double texX1, double texY1, double texX2, double texY2, Color color) {
    ...
    public void texturedQuad(double x, double y, double width, double height, double rotation, TextureRegion region, Color color) {
        texturedQuad(x, y, width, height, rotation, region.x1, region.y1, region.x2, region.y2, color);
    }
}