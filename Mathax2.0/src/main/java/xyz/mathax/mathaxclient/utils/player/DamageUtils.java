...
import xyz.mathax.mathaxclient.mixininterface.IExplosion;
...
public class DamageUtils {
    ...
    public static double crystalDamage(LivingEntity livingEntity, Vec3d crystal, boolean predictMovement, BlockPos obsidianPos, boolean ignoreTerrain) {
        ...
        if (predictMovement) {
            ((IVec3d) vec3d).set(vec3d.x + livingEntity.getVelocity().x, vec3d.y + livingEntity.getVelocity().y, vec3d.z + livingEntity.getVelocity().z);
        }
    ...
    public static double crystalDamage(LivingEntity livingEntity, Vec3d crystal) {
        return crystalDamage(livingEntity, crystal, false, null, false);
    }
    ...
    private static double normalProtReduction(Entity player, double damage) {
        int protectionLevel = EnchantmentHelper.getProtectionAmount(player.getArmorItems(), DamageSource.GENERIC);
        if (protectionLevel > 20) {
            protectionLevel = 20;
        }

        damage *= 1 - (protectionLevel / 25.0);
        return damage < 0 ? 0 : damage;
    }
