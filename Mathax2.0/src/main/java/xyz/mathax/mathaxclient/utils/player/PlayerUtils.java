...
import xyz.mathax.mathaxclient.utils.misc.BaritoneUtils;
import xyz.mathax.mathaxclient.utils.text.TextUtils;
...
public class PlayerUtils {
    ...
    public static Color getPlayerColor(PlayerEntity entity, Color defaultColor) {
        if (Friends.get().contains(entity)) {
            return color.set(Friends.get().colorSetting.get()).a(defaultColor.a);
        }

        if (Enemies.get().contains(entity)) {
            return color.set(Enemies.get().colorSetting.get()).a(defaultColor.a);
        }

        if (!color.set(TextUtils.getMostPopularColor(entity.getDisplayName())).equals(Color.WHITE) && Config.get().useTeamColorSetting.get()) {
    ...
    public static double possibleHealthReductions(boolean entities, boolean fall) {
        ...
        if (entities) {
            for (Entity entity : mc.world.getEntities()) {
                if (entity instanceof EndCrystalEntity && damageTaken < DamageUtils.crystalDamage(mc.player, entity.getPos())) {
                    ...
                    if (!Friends.get().contains((PlayerEntity) entity) && isWithin(entity, 5)) {
        ...
        if (fall) {
            if (/*!Modules.get().isEnabled(NoFall.class) &&*/ mc.player.fallDistance > 3) {
