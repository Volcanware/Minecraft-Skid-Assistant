...
public class ChamsShader extends EntityShader {
    ...
    public ChamsShader() {
        MatHax.EVENT_BUS.subscribe(ChamsShader.class);
    }
    ...
    public static void load() {
        try {
            ...
            for (String fileFormat : FILE_FORMATS) {
                Optional<Resource> optional = mc.getResourceManager().getResource(new MatHaxIdentifier("textures/chams." + fileFormat));
    ...
    protected void setUniforms() {
        shader.set("u_Color", chams.shaderColorSetting.get());

        if (chams.isShader() && chams.shaderSetting.get() == Chams.Shader.Image && IMAGE_TEX != null && IMAGE_TEX.isValid()) {
            IMAGE_TEX.bind(1);
    ...
    public boolean shouldDraw(Entity entity) {
        ...
        if (entity instanceof PlayerEntity player) {
            if (player == mc.player && chams.ignoreSelfDepthSetting.get()) {
                return false;
            } else if (Friends.get().contains(player) && chams.ignoreFriendsDepthSetting.get()) {
                return false;
            } else if (Enemies.get().contains(player) && chams.ignoreEnemiesDepthSetting.get()) {
                return false;
            }
        }

        return chams.entitiesSetting.get().getBoolean(entity.getType());
    }
}