...
import xyz.mathax.mathaxclient.mixin.ClientChunkManagerAccessor;
import xyz.mathax.mathaxclient.mixin.ClientChunkMapAccessor;
...
public class ChunkIterator implements Iterator<Chunk> {
    ...
    private Chunk getNext() {
        ...
        while (i < map.getChunks().length()) {
            ...
            if (chunk != null && (!onlyWithLoadedNeighbours || isInRadius(chunk))) {
                break;
            }
        }
