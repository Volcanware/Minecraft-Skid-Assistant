...
import xyz.mathax.mathaxclient.settings.EntityTypeListSetting;
...
public class EntityTypeListSettingScreen extends WindowScreen {
    ...
    public EntityTypeListSettingScreen(Theme theme, EntityTypeListSetting setting) {
    ...
    public void initWidgets() {
        ...
        for (EntityType<?> entityType : setting.get().keySet()) {
            if (!setting.get().getBoolean(entityType)) {
                continue;
            }
        ...
        if (filterText.isEmpty()) {
            ...
            Registries.ENTITY_TYPE.forEach(entity -> {
                ...
                if (words > 0 || diff < Names.get(entity).length() / 2) {
                    entities.add(new Pair<>(entity, -diff));
                }
            });
        ...
        if (animalsT.cells.size() == 0) {
            list.cells.remove(animalsCell);
        }

        if (waterAnimalsT.cells.size() == 0) {
            list.cells.remove(waterAnimalsCell);
        }

        if (monstersT.cells.size() == 0) {
            list.cells.remove(monstersCell);
        }

        if (ambientT.cells.size() == 0) {
            list.cells.remove(ambientCell);
        }

        if (miscT.cells.size() == 0) {
            list.cells.remove(miscCell);
        }
        ...
        if (first) {
            ...
            if (totalCount <= 20) {
                ...
                if (waterAnimalsT.cells.size() > 0) {
                    waterAnimals.setExpanded(true);
                }

                if (monstersT.cells.size() > 0) {
                    monsters.setExpanded(true);
                }
                ...
                if (animalsT.cells.size() > 0) {
                    animals.setExpanded(false);
                }

                if (waterAnimalsT.cells.size() > 0) {
                    waterAnimals.setExpanded(false);
                }

                if (monstersT.cells.size() > 0) {
                    monsters.setExpanded(false);
                }
    ...
    private void addEntityType(WTable table, WCheckbox tableCheckbox, EntityType<?> entityType) {
        ...
        WCheckbox checkbox = table.add(theme.checkbox(setting.get().getBoolean(entityType))).expandCellX().right().widget();
        checkbox.action = () -> {
            if (checkbox.checked) {
                setting.get().put(entityType, true);
                switch (entityType.getSpawnGroup()) {
