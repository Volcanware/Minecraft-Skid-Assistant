...
import xyz.mathax.mathaxclient.gui.screens.settings.*;
...
public class DefaultSettingsWidgetFactory extends SettingsWidgetFactory {
    ...
    public DefaultSettingsWidgetFactory(Theme theme) {
        super(theme);
    ...
    @Override
    public WWidget create(Theme theme, Settings settings, String filter) {
    ...
    private void group(WVerticalList list, SettingGroup group, String filter, List<RemoveInfo> removeInfoList) {
        ...
        for (Setting<?> setting : group) {
            if (!StringUtils.containsIgnoreCase(setting.name, filter)) {
                continue;
            }
    ...
    private static class RemoveInfo {
        ...
        public void remove(WVerticalList list) {
            ...
            if (table.cells.isEmpty()) {
                list.cells.removeIf(cell -> cell.widget() == section);
            }
        }
    }
    ...
    private void selectW(WContainer container, Setting<?> setting, Runnable action) {
        boolean addCount = WSelectedCountLabel.getSize(setting) != -1;

        WContainer container2 = container;
        if (addCount) {
            container2 = container.add(theme.horizontalList()).expandCellX().widget();
            ((WHorizontalList) container2).spacing *= 2;
        }

        WButton button = container2.add(theme.button("Select")).expandCellX().widget();
        button.action = action;

        if (addCount) {
            container2.add(new WSelectedCountLabel(setting).color(theme.textSecondaryColor()));
        }
    ...
    private static class WSelectedCountLabel extends WLabel {
        ...
        public static int getSize(Setting<?> setting) {
            ...
            if (setting.get() instanceof Map<?, ?> map) {
                return map.size();
            }
