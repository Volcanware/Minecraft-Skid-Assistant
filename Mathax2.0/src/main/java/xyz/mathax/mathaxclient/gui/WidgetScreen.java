...
import xyz.mathax.mathaxclient.gui.widgets.WRoot;
...
import xyz.mathax.mathaxclient.utils.input.CursorStyle;
...
public abstract class WidgetScreen extends Screen {
    ...
    public WidgetScreen(Theme theme, String title) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float delta) {
        ...
        boolean tooltip = RENDERER.renderTooltip(mouseX, mouseY, delta / 20, matrixStack);

        if (debug) {
            DEBUG_RENDERER.render(root, matrixStack);
            if (tooltip) {
                DEBUG_RENDERER.render(RENDERER.tooltipWidget, matrixStack);
            }
        }
