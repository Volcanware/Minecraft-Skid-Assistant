package xyz.mathax.mathaxclient.gui.widgets.input;
...
public abstract class WTextBox extends WWidget {
    ...
    public boolean onKeyPressed(int key, int mods) {
        ...
        if (control && key == GLFW_KEY_C) {
            ...
            stringBuilder.append(text, cursor, text.length());
    ...
    public boolean onKeyRepeated(int key, int mods) {
        ...
        if (control && key == GLFW_KEY_V) {
            ...
            StringBuilder stringBuilder = new StringBuilder(text.length() + clipboard.length());
            stringBuilder.append(text, 0, cursor);
    ...
    private void calculateTextWidths() {
        ...
        for (int i = 0; i <= text.length(); i++) {
            textWidths.add(theme.textWidth(text, i, false, false));
        }
    }
