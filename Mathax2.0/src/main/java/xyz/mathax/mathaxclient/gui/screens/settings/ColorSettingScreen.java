...
public class ColorSettingScreen extends WindowScreen {
    ...
    public ColorSettingScreen(Theme theme, Setting<SettingColor> setting) {
    ...
    private void setFromSetting() {
        SettingColor settingColor = setting.get();
        if (settingColor.r != rItb.get()) {
            rItb.set(settingColor.r);
        }

        if (settingColor.g != gItb.get()) {
            gItb.set(settingColor.g);
        }

        if (settingColor.b != bItb.get()) {
            bItb.set(settingColor.b);
        }

        if (settingColor.a != aItb.get()) {
            aItb.set(settingColor.a);
        }

        rainbow.checked = settingColor.rainbow;
    ...
    private class WBrightnessQuad extends WWidget {
        ...
        protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
            ...
            renderer.quad(x + handleX - scale / 2, y + handleY - scale / 2, scale, scale, WHITE);
        }
    }
    ...
    private class WHueQuad extends WWidget {
        ...
        void calculateFromSetting(boolean calculateNow) {
            ...
            if (!calculated) {
                ...
                if (!calculated) {
                    if (color.r >= max) {
                        hueAngle = (color.g - color.b) / delta;
                    } else if (color.g >= max) {
                        hueAngle = 2.0 + (color.b - color.r) / delta;
                    } else {
                        hueAngle = 4.0 + (color.r - color.g) / delta;
                    }
        ...
        protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
            ...
            double scale = theme.scale(2);
            renderer.quad(x + handleX - scale / 2, y, scale, height, WHITE);
        }
    }
}